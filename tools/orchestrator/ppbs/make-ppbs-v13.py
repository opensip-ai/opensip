#!/usr/bin/env python3
"""Generator for `preview-product-boundary-successor.v13.json`.
...
SELF = "preview-product-boundary-successor.v13"
SPEAKER = f"This {SELF}"
VERSION = 13
REQUIRED_NOW = 28
REGISTER_ROW = "DR-117"
...
def collect_finding_ids(review_doc):
    """Every reviewer finding identifier in a Stage A verdict, whatever the shape."""
    ids = []
...
def rejected_intermediates(pred_version):
    ...
    for v in range(pred_version + 1, VERSION):
        ...
        for who, fn in (("claude", "claude2"), ("codex", "codex")):
            ...
            ids += [i for i in collect_finding_ids(doc) if i not in ids]
...
def build_recorded_inputs(m, based_on):  # noqa: D401
    ...
    for rj in m["rejected"]:
        ri[rj["path"]] = rj["sha256"]
        for r in rj["reviews"].values():
            ri[r["path"]] = r["sha256"]
    for j in m["joins"]:
        ri[j["path"]] = j["sha256"]
...
def build_doc(m):
    ...
    based_on["predecessorPinningShape"] = (
        ...
        f"measured at this dispatch, and each verdict object additionally carries the "
        f"gradeRuling its verdict file states where that file states one. "
        f"preview-product-boundary-successor.v6, preview-product-boundary-successor.v9 and "
        + ", ".join(f"preview-product-boundary-successor.v{r['version']}" for r in m["rejected"])
        + f" were rejected at Stage A and never recorded, so none carries a recording; "
        f"preview-product-boundary-successor.v6 carries only a Claude verdict, because no Codex "
        f"verdict for it exists on disk.")
    v10_reviews = {}
    for who, fn in (("claude", "claude2"), ("codex", "codex")):
        rel = f"{ART}/preview-product-boundary-successor.v10.review-independent.{fn}.json"
        need(os.path.exists(os.path.join(REPO, rel)), f"missing {rel}")
    ...
    for rj in m["rejected"]:
        based_on[f"predecessorV{rj['version']}"] = {
            "path": rj["path"], "sha256": rj["sha256"],
            "recording": None,
            "standing": "REJECTED-AT-STAGE-A-NEVER-RECORDED",
            "reviews": rj["reviews"],
            "role": (
                f"preview-product-boundary-successor.v{rj['version']} is the immediate predecessor "
                f"of {SELF} and was REJECTED at Stage A by both independent reviewers. It carries "
                f"no recording and never became current; {PRED} ({PRED_RECORDING}) stayed the "
                f"current recorded DR-117 leftover remasurement throughout. It stays frozen and is "
                f"not to be recorded. Its {word(len(rj['findingIds']))} reviewer findings are "
                f"disposed in findingDisposition; {SELF} exists to land them."),
        }
    for j in m["joins"]:
    ...
    rej_note = ("" if not m["rejected"] else
                " Its immediate predecessor is "
                + ", ".join(f"preview-product-boundary-successor.v{r['version']}"
                            for r in m["rejected"])
                + ", rejected at Stage A by both reviewers and never recorded; the current"
                  f" recorded baseline this successor is built from remains {PRED}"
                  f" ({PRED_RECORDING}).")
    based_on["relation"] = (
...
def rebuild_does_not(m, pred):
    ...
    for s in pred["doesNot"]:
        ...
        elif "stays frozen" in s or "do not record" in s.lower():
            extra = ", ".join(f"preview-product-boundary-successor.v{r['version']}"
                              for r in m["rejected"])
...
def assemble(m):
    ...
    doc = {
        ...
        "predecessorStanding": {
            ...
            **{f"predecessorV{r['version']}Standing": (
                f"preview-product-boundary-successor.v{r['version']} is frozen at {r['path']}, "
                f"sha256 {r['sha256']}, rejected at Stage A by both independent reviewers and "
                f"unrecorded. It is not current and never was. It stays frozen; do not record it. "
                f"Its {word(len(r['findingIds']))} findings are disposed in findingDisposition.")
               for r in m["rejected"]},
        ...
        "authorityClaim": (
            ...
            + ", ".join(f"preview-product-boundary-successor.v{r['version']} was rejected at "
                        f"Stage A by both reviewers and never recorded" for r in m["rejected"])
        ...
        "findingDisposition": [{
            ...
            {"id": fid, "severity": "LANDED",
             ...
             "disposition": f"landed at {SELF}: " + (
                 V11_DISPOSITIONS.get(fid) or V12_DISPOSITIONS.get(fid) or "")}
            for rj in m["rejected"] for fid in rj["findingIds"]
        ] + [{
            "id": "PPBS-V13-VENUE-NOTE",
            "severity": "NOTE",
            "disposition": (
...
def main(argv=None):
    ...
    try:
        ...
        print(f"make-ppbs-v13.py: FAILED: {e}", file=sys.stderr)
        return 2
    except subprocess.CalledProcessError as e:
        print(f"make-ppbs-v13.py: FAILED: git: {e}", file=sys.stderr)
        return 2
