...
adds the current public `permissionPolicyDigest`: the domain-separated digest
of the effective permission-policy document under
`opensip.metadata.policy-effective.1`, including both raw-policy source pins
(or the explicit absent-layer representation). It identifies effective policy
...
checks. `compatibility-matrix.completed.v5.json` retains all eight inherited
...
`compatibility-matrix.completed.v5.json` adds S-DOCTOR: writer schema major 1,
...
`compatibility-matrix.completed.v5.json` is the current normative matrix. Its
history is exact: v3 replaces v2's illustrative exact initial host constraint
with the patch-compatible interval above; v4 adds S-DOCTOR; v5 changes only
`/lockGrammar` from component-lock-schema.completed.v2.json to the completed
v3 schema already selected in §3. Release/state separation and every support
window remain. Exact-version constraints remain a valid grammar choice for
explicitly constrained requests and fixtures, not the initial preview provider's
release policy. Historical solver corpora pin their original matrix inputs;
the final signed-selection integration pins this v5 matrix for current locks.
