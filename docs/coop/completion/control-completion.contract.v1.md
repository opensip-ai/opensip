...
Status: PROPOSED. Author: Codex protocol fixture author. No readiness register
change, permission grant, journal carrier activation, or product qualification is
made by these files. The complete schema is `control-completion.schema.v2.json`;
...
violations are RF7. Schema-valid attempts to make the control plane adjudicate
provider meaning are RF5; unauthorized host effects are RF6 with PR detail.
The `semanticActionRequested` test-seam input is an owning host check for a real
semantic act, not a classifier over arbitrary strings.
...
For an unsupported-major hello, the frozen envelope permits direction and replay
...
`control-completion.protocol-join.v1.json` names exact old/new corpus hashes and
...
The new schema makes their legacy unknown action tokens RF2, while new valid
semantic-action and denied-authorization cases retain RF5 and RF6 respectively.
...
selection contains 9,755 cases. Corpus size is finite evidence, not a universal
proof of all streams or all scheduler behavior.

## Replay and limits

`control-completion.check.v1.py` exercises the actual JSON decoder, closed schema,
UTF-8 byte bounds and reference state/correlation checks. With Python3.12 and
jsonschema4.25.1:

```
python docs/coop/completion/control-completion.check.v1.py --report /tmp/control-review.json
```

The retained report is 434/434 PASS over 431 cases, two accepted source pins and
the complete matrix check. It captures exact checker/schema/corpus hashes.
Platform aliases remain macOS arm64/x86_64 and Linux arm64/x86_64. No shipping
