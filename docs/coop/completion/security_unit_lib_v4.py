"""Shared library for the security completion unit v4 (design evidence only; WORKING COPY, not adopted).
...
def witness_shape_refusals(w):
    ...
    if w.get('witnessSchema') != 1 or isinstance(w.get('witnessSchema'), bool): r.append('SCHEMA')
    ...
    if 'SEQ' not in r and 'STATE' not in r:
        sha_ = w.get('bodySha256')
        if w['seq'] == 0:
...
def verify_envelope(stored_bytes, env, root, expected_kind, publisher_namespace=None):
    ...
    if not isinstance(root, AdmittedRoot):
        return 'ROOT-NOT-ADMITTED', 'verify_envelope requires an AdmittedRoot from admit_root_document'
    root = root.doc
    if admit_root(root):
...
_SCHEMA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'security-schemas.v4')
...
def verify_effect_request(body, ctx):
    """Section 7.4 ENFORCING BOUNDARY (SEC3-M4). ctx is one required record
    {connectionMap, currentBinding, journalState, snapshotMembers}; a missing or malformed member refuses
    RF-6:AUTHORIZATION.CONTEXT_ABSENT before anything else. connectionMap: the handles minted for THIS
    connection (one per GRANT record; several may share the spawn operationRef). journalState:
    {locator: {'status', 'token', 'binding', 'scope'}}. snapshotMembers: the sealed snapshot manifest's
    member-path set for the grant's snapshot. Returns None (proceed to RA) or an RF-6 refusal reason."""
    ...
    if not (isinstance(connection_map, dict) and isinstance(current_binding, dict) and isinstance(journal_state, dict) and isinstance(snapshot_members, (set, frozenset, list, tuple))):
        return 'RF-6:AUTHORIZATION.CONTEXT_ABSENT'
...
def resolve_result(result_ref, read_scratch, ctx):
    """SDK-side ENFORCING BOUNDARY (SEC3-M4). ctx is one required record with every member of
    RESULT_CONTEXT_KEYS present and non-null; absence refuses RESULT.CONTEXT_ABSENT. Order: (1) the carrying
    effectResult is COMPLETED, its requestSeq is the SDK's own request and its resultRef is the one being
    resolved; (2) the held scratch directory is a directory owned by selfUid at mode 0700; (3) ref grammar
    (full match) and per-result cap; (4) aggregate per-spawn cap; (5) the file opened by openat(held,
    '<rid>.bin', O_RDONLY|O_NOFOLLOW|O_CLOEXEC) is regular, nlink 1, owned by selfUid, size == ref length;
    (6) bytes read equal length and digest. Returns ('OK', bytes) or ('RESULT.<reason>', None)."""
    ...
    if not isinstance(er, dict) or er.get('effectOutcome') != 'COMPLETED':
        return 'RESULT.EFFECT_NOT_COMPLETED', None
    if er.get('requestSeq') != want_seq:
        return 'RESULT.REQUEST_SEQ_MISMATCH', None
    if er.get('resultRef') != result_ref:
        return 'RESULT.REF_NOT_FROM_RESULT', None
    if dst.get('type') != 'directory' or dst.get('uid') != uid or (dst.get('mode', 0o777) & 0o077):
    ...
    if not _strict_int(so_far, 0, RESULT_BYTES_MAX_PER_SPAWN) or so_far + length > RESULT_BYTES_MAX_PER_SPAWN:
        return 'RESULT.SPAWN_CAP_EXCEEDED', None
    if fst.get('type') != 'regular' or fst.get('nlink') != 1 or fst.get('uid') != uid or fst.get('size') != length:
...
# ---- SEC3-M5: HE-1 ordered algorithm with reversible / irreversible intent boundaries --------------
...
def he1_recover(commit_class, durable_records, footprint):
    """Recovery after a crash during HE-1. durable_records: the ordered journal records present (subset of
    HE1_ORDER without EFFECT/UNLINK-STAGE/RESULT). footprint: 'present' | 'absent' | 'unknown' for the
    committed object (present means bytes with the recorded digest exist at the staged target).
    Returns the outcome record recovery appends and the effect outcome the host reports.
    Rule: a missing outcome record never means not-performed; the footprint decides; irreversible
    intent without a determinable footprint is INDETERMINATE."""
    if commit_class not in HE1_ORDER:
        raise ValueError('commit class')
    intent, outcome = ('RCI', 'RCO') if commit_class == 'REVERSIBLE' else ('ICI', 'ICO')
    recs = list(durable_records)
    if 'RA' not in recs:
        return None, 'NOT-ACCEPTED'                        # no authorization was recorded: nothing to recover
    if intent not in recs:
        return (outcome, 'FAILED'), 'FAILED'              # accepted, no intent: the effect never began; close it
    if outcome in recs:
        return None, 'ALREADY-RECORDED'
    if commit_class == 'REVERSIBLE':
        return (outcome, 'REVERTED'), 'FAILED'            # intent without outcome: revert the staged commit
    if footprint == 'present':
        return (outcome, 'COMPLETED'), 'COMPLETED'
    if footprint == 'absent':
        return (outcome, 'FAILED'), 'FAILED'
    return (outcome, 'INDETERMINATE'), 'INDETERMINATE'
...
class AdmittedRoot(object):
    """The only root value verify_envelope accepts. Constructed solely by admit_root_document."""
    __slots__ = ('doc', 'digest')
    def __init__(self, doc, digest):
        self.doc, self.digest = doc, digest
