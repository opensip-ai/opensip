#!/usr/bin/env python3
"""Retained checker for the security completion unit v5 (design evidence only): bounded corrective successor of v4.
...
import security_unit_lib_v5 as L  # noqa: E402

FX = os.path.join(HERE, 'security-fixtures.v5')
SCH = os.path.join(HERE, 'security-schemas.v5')
...
def main():
    ...
    vec = rj('security-vectors.v5.json')
    for s in ('envelope', 'root', 'revocation', 'catalog', 'registry', 'registry-view', 'payload', 'journal-record', 'permission-policy', 'tcb-profile-template'):
        rb('security-schemas.v5/%s.schema.json' % s)
    rb('security-schemas.v5/grant-journal.sql')
    rb('security_unit_lib_v5.py')
    ...
    for sch, fn in examples.items():
        obj = rj('security-fixtures.v5/' + fn); objs[sch] = obj
        try:
    ...
    rbc = rj('security-fixtures.v5/root-boundary-cases.json'); env_ok = rj('security-fixtures.v5/typescript-analyzer.envelope.json'); raw_man = rb('security-fixtures.v5/typescript-analyzer.manifest.json')
    for c in rbc['cases']:
    ...
    for name in vec['rootNegatives']:
        neg = rj('security-fixtures.v5/root.negative.%s.json' % name)
        shape_ok = True
        try:
    ...
    weak = rj('security-fixtures.v5/root.negative.role-threshold-1.json')
    single = rj('security-fixtures.v5/typescript-analyzer.envelope.json'); single = json.loads(json.dumps(single)); single['signatures'] = single['signatures'][:1]
    weak_adm = L.admit_root_document(weak)[0]
    outcome, detail = L.verify_envelope(rb('security-fixtures.v5/typescript-analyzer.manifest.json'), single, weak_adm if weak_adm is not None else weak, 'manifest', publisher_namespace='opensip')
    ...
    for vf in ('registry-view.project-a.example.json', 'registry-view.project-b.example.json'):
        view = rj('security-fixtures.v5/' + vf)
        try:
    ...
    va = rj('security-fixtures.v5/registry-view.project-a.example.json'); vb = rj('security-fixtures.v5/registry-view.project-b.example.json')
    ...
    stored = rb('security-fixtures.v5/typescript-analyzer.manifest.json')
    ...
    rec('manifest.canonicalFile', L.sha256_hex(rb('security-fixtures.v5/typescript-analyzer.manifest.canonical.bin')) == m['canonicalSha256'])
    ...
    for neg, expect in (('typescript-analyzer.manifest.wrong-rung.json', 'relation/rung mismatch'), ('typescript-analyzer.manifest.unknown-capability.json', 'unknown capability')):
        r = admit_manifest(L.load_json_strict(rb('security-fixtures.v5/' + neg)), m['registry'])
    ...
    for fn, expect in vec['envelopes'].items():
        env = rj('security-fixtures.v5/' + fn)
    ...
    ok_env = rj('security-fixtures.v5/typescript-analyzer.envelope.json')
    ...
    wc = rj('security-fixtures.v5/witness-reconciliation-cases.json')
    for c in wc['cases']:
    ...
    joins = rj('security-fixtures.v5/d9-joins.example.json')
    ...
    gj = rj('security-fixtures.v5/grant-journal.example.json')
    ...
    lb = rj('security-fixtures.v5/locator-bounds.json')
    ...
    bh = rj('security-fixtures.v5/broker-handles.example.json')
    for c in bh['cases']:
    ...
    du = rj('security-fixtures.v5/durability-io-failures.json')
    ...
    dr = rj('security-fixtures.v5/doctor-trust-report.example.json')
    ...
    pe = rj('security-fixtures.v5/permission-policy.effective.example.json')
    ...
    lv = rj('security-fixtures.v5/launch-verification-classes.json')
    ...
    qr = rj('security-fixtures.v5/qualification-runners.json')
    ...
    rec('runners.templates-exist-and-name-their-class', all(os.path.exists(os.path.join(HERE, 'security-fixtures.v5', c['developmentTemplate'])) and rj('security-fixtures.v5/' + c['developmentTemplate'])['qualificationRunnerClass'] == c['class'] for c in qr['classes']))
    # 5d. G22 observed-event admission probes and the G07 loader/TOCTOU crosswalk
    ge = rj('security-fixtures.v5/g22-observed-events.json')
    profs = {fn: rj('security-fixtures.v5/' + fn) for fn in ('profile.P-MACOS-ARM64-25G83-APFS.json', 'profile.P-MACOS-X86_64-25G83-APFS.json', 'profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json')}
    ...
    rb('security-fixtures.v5/grant-journal.example.sqlite')
    ...
    m6 = rj('security-fixtures.v5/linux-os-abi-predicate-cases.json')
    for c in m6['cases']:
    ...
    for fn in ('profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json'):
        pay = rj('security-fixtures.v5/' + fn)['signedEntries'][0]['identityEvidence']['value']['authenticityCore']['payload']
    ...
    for fn in ('profile.P-MACOS-ARM64-25G83-APFS.json', 'profile.P-MACOS-X86_64-25G83-APFS.json', 'profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json'):
        prof = rj('security-fixtures.v5/' + fn)
        try:
    ...
    report = {'checker': 'check-security-unit.v5.py', 'checkerSha256': L.sha256_hex(open(os.path.abspath(__file__), 'rb').read()), 'ranAt': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
