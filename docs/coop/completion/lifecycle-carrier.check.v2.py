...
class Store:
    def __init__(self, directory, initialize=True):
        ...
        self.register()
        if initialize:
            self.connection.executescript((B/'lifecycle-carrier.schema.v2.sql').read_text())
            with self.writer():
        ...
        assert self.connection.execute('PRAGMA recursive_triggers').fetchone()[0]==1
    ...
    def register(self):
        ...
        def scope_match(key,gid,digest):
            ...
            lock=json.loads(raw);context=lock['scopeContext']
            global_tuples=all(t['scope']=='global' for t in lock['resolved'])
            return int((context['projectKey'] is None and context['allowedScopes']==['global'] and global_tuples) or (context['projectKey']==key and context['allowedScopes']==['project','global']))
    ...
    def insert_generation(self,gid=G1,lockProjectKey=None,resolvedScopes=None,**overrides):
        context={'projectKey':lockProjectKey,'allowedScopes':['global'] if lockProjectKey is None else ['project','global']}
        self.fixture_locks[gid]=json.dumps({'fixtureGeneration':gid,'scopeContext':context,'resolved':[{'scope':v} for v in (resolvedScopes if resolvedScopes is not None else ['global'] if lockProjectKey is None else ['project'])]},sort_keys=True,separators=(',',':')).encode()
    ...
    def lease(self,lid=L1,project=P1,gid=G1,token='fixture-process-start:1',acquire=True,boot=BOOT):
...
def act(store,action):
    ...
    if op=='replace-registry':
        row=list(store.execute('SELECT * FROM project_registry WHERE projectKey=?',(action['key'],)).fetchone());row[-1]='ACTIVE'
        return store.execute('INSERT OR REPLACE INTO project_registry VALUES(?,?,?,?,?,?,?,?,?)',row)
    if op=='registry-ticket-values':
        store.root_tickets[action['values'][0]]=action['values'];return
...
def run_case(case):
    with tempfile.TemporaryDirectory(prefix='opensip-lifecycle-carrier-') as td:
        ...
        store.close()
        refusal_matches=not case.get('refusalContains') or (error is not None and case['refusalContains'] in error)
        return {'id':case['id'],'expected':case['expected'],'observed':observed,'passed':observed==case['expected'] and refusal_matches and all(x['passed'] for x in assertions),'sqliteRefusal':error,'postconditions':assertions,**({'expectedRefusalContains':case['refusalContains']} if case.get('refusalContains') else {})}
...
def run(args):
    corpus_path=B/'lifecycle-carrier.cases.v2.json';contract_path=B/'lifecycle-carrier.contract.v2.json'
    ...
    report={'status':'DESIGN-DDL-RESULT-NOT-PRODUCT-QUALIFICATION','sqliteVersion':sqlite3.sqlite_version,'passed':sum(r['passed'] for r in results),'total':len(results),
            'sourcePins':[{'path':str(p.relative_to(ROOT)),'sha256':SHA(p.read_bytes())} for p in [Path(__file__),corpus_path,contract_path,B/'lifecycle-carrier.schema.v2.sql']],
...
if __name__=='__main__':
    ...
    parser.add_argument('--report',default=str(B/'lifecycle-carrier.report.v2.json'))
