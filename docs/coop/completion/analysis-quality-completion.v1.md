...
AL-3 core release byte rollback is explicitly deferred with DR-110 under the
existing D-002 fresh-download scope. Generation rollback remains active under
...
The quality matrix is `language-quality-matrix.completed.v1.json`: 20 explicit
...
claiming an equivalent full-product behavior/performance baseline. The 5.9.3
exploratory run is not the accepted baseline. Performance definitions, runner
...
are in `sarif-preservation-cases.v1.json`, checked by `check_quality_design.py`.
...
G13's strict result grammar is `g13-result-schema.v1.json`. Its semantic
