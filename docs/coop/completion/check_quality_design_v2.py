...
def run():
    matrix = json.loads((BASE / 'language-quality-matrix.completed.v2.json').read_text())
    ps = matrix['platformsSource']
    source = BASE.parents[2] / ps['path']
    assert SHA(source) == ps['sha256']
    assert json.loads(source.read_text())['platforms'] == ps['quotedValue']
    assert matrix['platforms'] == [x.replace('/', '-') for x in ps['quotedValue']]
    schema = json.loads((BASE / 'g13-result-schema.v2.json').read_text())
    def enums(x):
        if isinstance(x, dict):
            if 'enum' in x and set(x['enum']) == set(matrix['platforms']):
                assert x['enum'] == matrix['platforms']
            for y in x.values(): enums(y)
    ...
    report = {'kind': 'quality-and-preservation-design-check', 'productQualification': False,
              ...
              'pins': {p.name: SHA(p) for p in [Path(__file__), BASE / 'language-quality-matrix.completed.v2.json',
    ...
    parser.add_argument('--report', type=Path, default=BASE / 'quality-design-report.v2.json')
