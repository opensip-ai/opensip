# Security, trust and platform-TCB completion — lead-owned corrective integration v7
...
Retained with this document, all under `docs/coop/completion/`: `security-schemas.v7/` (eleven closed
JSON Schemas plus `grant-journal.sql`, byte-identical to v4), `security-fixtures.v7/` (54 files),
`security-vectors.v7.json`, `security_unit_lib_v7.py`, `check-security-unit.v7.py`, and the frozen report
`security-unit.v7.report.json` (864 checks, 0 failures; cross-validated through the review
...
`check-security-unit.v7.py --report security-unit.v7.report.json` (864 checks) reproduces every vector,
...
`security_boundary_mutations_v7.py` applies type-changing mutations at every
