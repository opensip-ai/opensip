# Security, trust and platform-TCB completion — corrective successor v5

Author: Claude (Herdr `wH:p2`). Status: PROPOSED under the adopted D-368 case addendum "delegated
security remediation after failed confirmation" (`security-owner-remediation.v1.md` `1efce11f…`,
independent procedural consent `83a3473e…`): the one authorized corrective successor, bounded to the
four findings the first bounded confirmation left unresolved (`security-repair-confirmation.v1.json`
`4c7f3c29…`: SEC3-M1, SEC3-M3, SEC3-M4 with the protocol author's malformed-snapshot probe, SEC3-M5 with
its inseparable regression SEC4-REG-M5-OUTCOME). SEC3-M2 and SEC3-M6 keep their confirmed dispositions;
SEC-POLICY-N1 and the cap clarification are intact. Awaiting the same adjudicator's one additional
bounded confirmation; a further failure escalates to the user. Scope: DR-112, DR-105, DR-114, DR-124, DR-126, DR-101 OD-101-2, the metadata
...
History, preserved byte-for-byte: three ordinary exchanges (v1, v2, v3 `9486f6c4…`), the UPHOLD ruling
(`security-adjudication.v1.json` `7030a258…`), the v4 repair (`security-freeze.v4.json` `6219340d…`,
283/283) and its failed bounded confirmation (four unresolved MUST-FIX). The counts stay three ordinary
exchanges plus one failed confirmation; v5 is not a fourth exchange and resets nothing. v5 lives entirely
under `.v5` names and `security-freeze.v5.json` carries the exact per-file diff against v4. It binds NOTHING, edits no register cell, authorizes no
...
Retained with this document, all under `docs/coop/completion/`: `security-schemas.v5/` (eleven closed
JSON Schemas plus `grant-journal.sql`, byte-identical to v4), `security-fixtures.v5/` (54 files),
`security-vectors.v5.json`, `security_unit_lib_v5.py`, `check-security-unit.v5.py`, and the frozen report
`security-unit.v5.report.json` (332 checks, 0 failures; cross-validated through the review
...
**Corrective successor (v5), bounded to the four unresolved IDs; every failed probe retained as a
...
**One admission boundary (SEC3-M1, corrected in v5).** `admit_root_document(bytes)` = parse → closed
...
- **HE-1 HOST-STAGED-STATE-WRITE (ordered algorithm, SEC3-M5).** The SDK writes
  ...
  `uncertainTailLoss` and appends nothing. RA without intent closes as `FAILED` (the effect never began).
...
`check-security-unit.v5.py --report security-unit.v5.report.json` (332 checks) reproduces every vector,
