...
authority. Per-spawn broker scratch lives under the lifecycle UUID project
namespace; the host removes it after child exit while holding the operation
lease, and recovery only reaps it once that lease is proven released. Journal
and witness files use the same UUID namespace; their digest-based carrier locator
is never a path. Preview state and project roots require local APFS on macOS or ext4
