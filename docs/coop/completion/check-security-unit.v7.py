...
import security_unit_lib_v7 as L  # noqa: E402

FX = os.path.join(HERE, 'security-fixtures.v7')
SCH = os.path.join(HERE, 'security-schemas.v7')
...
def main():
    ...
    vec = rj('security-vectors.v7.json')
    for s in ('envelope', 'root', 'revocation', 'catalog', 'registry', 'registry-view', 'payload', 'journal-record', 'permission-policy', 'tcb-profile-template'):
        rb('security-schemas.v7/%s.schema.json' % s)
    rb('security-schemas.v7/grant-journal.sql')
    rb('security_unit_lib_v7.py')
    ...
    for sch, fn in examples.items():
        obj = rj('security-fixtures.v7/' + fn); objs[sch] = obj
        try:
    ...
    rbc = rj('security-fixtures.v7/root-boundary-cases.json'); env_ok = rj('security-fixtures.v7/typescript-analyzer.envelope.json'); raw_man = rb('security-fixtures.v7/typescript-analyzer.manifest.json')
    for c in rbc['cases']:
    ...
    for name in vec['rootNegatives']:
        neg = rj('security-fixtures.v7/root.negative.%s.json' % name)
        shape_ok = True
        try:
    ...
    weak = rj('security-fixtures.v7/root.negative.role-threshold-1.json')
    single = rj('security-fixtures.v7/typescript-analyzer.envelope.json'); single = json.loads(json.dumps(single)); single['signatures'] = single['signatures'][:1]
    weak_adm = L.admit_root_document(weak)[0]
    outcome, detail = L.verify_envelope(rb('security-fixtures.v7/typescript-analyzer.manifest.json'), single, weak_adm if weak_adm is not None else weak, 'manifest', publisher_namespace='opensip')
    ...
    for vf in ('registry-view.project-a.example.json', 'registry-view.project-b.example.json'):
        view = rj('security-fixtures.v7/' + vf)
        try:
    ...
    va = rj('security-fixtures.v7/registry-view.project-a.example.json'); vb = rj('security-fixtures.v7/registry-view.project-b.example.json')
    ...
    rec('manifest.canonicalFile', L.sha256_hex(rb('security-fixtures.v7/typescript-analyzer.manifest.canonical.bin')) == m['canonicalSha256'])
    ...
    for neg, expect in (('typescript-analyzer.manifest.wrong-rung.json', 'relation/rung mismatch'), ('typescript-analyzer.manifest.unknown-capability.json', 'unknown capability')):
        r = admit_manifest(L.load_json_strict(rb('security-fixtures.v7/' + neg)), m['registry'])
    ...
    for fn, expect in vec['envelopes'].items():
        env = rj('security-fixtures.v7/' + fn)
    ...
    ok_env = rj('security-fixtures.v7/typescript-analyzer.envelope.json')
    ...
    wc = rj('security-fixtures.v7/witness-reconciliation-cases.json')
    for c in wc['cases']:
    ...
    joins = rj('security-fixtures.v7/d9-joins.example.json')
    ...
    gj = rj('security-fixtures.v7/grant-journal.example.json')
    ...
    lb = rj('security-fixtures.v7/locator-bounds.json')
    ...
    bh = rj('security-fixtures.v7/broker-handles.example.json')
    for c in bh['cases']:
    ...
    du = rj('security-fixtures.v7/durability-io-failures.json')
    ...
    dr = rj('security-fixtures.v7/doctor-trust-report.example.json')
    ...
    pe = rj('security-fixtures.v7/permission-policy.effective.example.json')
    ...
    lv = rj('security-fixtures.v7/launch-verification-classes.json')
    ...
    qr = rj('security-fixtures.v7/qualification-runners.json')
    ...
    rec('runners.templates-exist-and-name-their-class', all(os.path.exists(os.path.join(HERE, 'security-fixtures.v7', c['developmentTemplate'])) and rj('security-fixtures.v7/' + c['developmentTemplate'])['qualificationRunnerClass'] == c['class'] for c in qr['classes']))
    # 5d. G22 observed-event admission probes and the G07 loader/TOCTOU crosswalk
    ge = rj('security-fixtures.v7/g22-observed-events.json')
    profs = {fn: rj('security-fixtures.v7/' + fn) for fn in ('profile.P-MACOS-ARM64-25G83-APFS.json', 'profile.P-MACOS-X86_64-25G83-APFS.json', 'profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json')}
    ...
    rb('security-fixtures.v7/grant-journal.example.sqlite')
    ...
    m6 = rj('security-fixtures.v7/linux-os-abi-predicate-cases.json')
    for c in m6['cases']:
    ...
    for fn in ('profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json'):
        pay = rj('security-fixtures.v7/' + fn)['signedEntries'][0]['identityEvidence']['value']['authenticityCore']['payload']
    ...
    for fn in ('profile.P-MACOS-ARM64-25G83-APFS.json', 'profile.P-MACOS-X86_64-25G83-APFS.json', 'profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json'):
        prof = rj('security-fixtures.v7/' + fn)
        try:
    ...
    rb('security_boundary_mutations_v7.py')
    import security_boundary_mutations_v7 as boundary_mutations
    boundary_mutations.run(L, rec, rb)

    failures = [r for r in results if r['result'] == 'FAIL']
    report = {'checker': 'check-security-unit.v7.py', 'checkerSha256': L.sha256_hex(open(os.path.abspath(__file__), 'rb').read()), 'ranAt': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
