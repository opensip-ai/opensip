...
def _verify_envelope_checked(stored_bytes, env, root, expected_kind, publisher_namespace=None):
    ...
    except (SchemaError, Reject) as e:
        return 'RJ-4 UNSIGNED', 'malformed envelope: canonical profile'
    s = env['subject']
...
_SCHEMA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'security-schemas.v8')
