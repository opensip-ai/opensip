...
import jsonschema
_SCHEMA=json.loads(Path(__file__).with_name('security-behavior-doctor-schema.v2.json').read_text())
...
def schema_ok(value,schema):
 try:return bounded(value) and not list(jsonschema.Draft202012Validator(schema).iter_errors(value))
...
def host_record_valid(x):
 try:
  ...
  permitted=permitted and action!='CA-2' and not(action=='CA-1' and sub=='IN_PROCESS') and not(action=='CA-3' and sub!='OUT_OF_ROOT_READ') and not(action=='CA-4' and sub!='PATH-TRUST-STATE-REFRESH')
...
def doctor_reader(x):
 try:
  ...
  for cr in r['consentRecords']:
   if cr['authorization']=='granted' and (cr['actionClass']=='CA-2' or cr['actionClass']=='CA-1' and cr['scope']['actionSubtype']=='IN_PROCESS'):errors.append('preview-excluded-grant')
