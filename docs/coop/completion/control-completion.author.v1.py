...
schema=json.loads((HERE/'control-message-schema.v1.json').read_text());schema['$id']='urn:opensip:design:control-schema:2';schema['title']='PROPOSED completed closed control major1 body schemas; independently reviewed application required'
...
(HERE/'control-completion.schema.v2.json').write_text(json.dumps(schema,indent=2)+'\n')
...
add('EFFECT/semantic-request','effectRequest','RF-5',{'direction':'component-to-host','semanticActionRequested':True})
add('EFFECT/semantic-wrong-direction','effectRequest','RF-7',{'semanticActionRequested':True})
...
(HERE/'control-completion.cases.v1.json').write_text(json.dumps(corpus,indent=2,ensure_ascii=False)+'\n');print('Authored',len(cases),'control cases')
