...
from the candidate result. `g13-result-schema.v4.json` adds `runnerClass` and
allows observed positive CPU/RAM values. `check_g13_result_design_v4.py` checks
...
cannot yield a valid PASS. Even an accurately observed runner must pass the
D-102 class admission: RAM is 7/14/16 GiB respectively within ±0.75 GiB
(inclusive, binary GiB); macOS product-version major is 15 and Linux
VERSION_ID is exactly 24.04. A truthful out-of-class inventory never authorizes
a PASS. OS builds and CPU brands are retained strata; image changes require
the paired baseline rule below. Synthetic inventory values in the reference tests
...
## Exact machine-field supersession

In `language-quality-matrix.completed.v2.json`,
`/performanceProfiles/whole-analyze-1000modules.v1/workerVcpu` and
`/performanceProfiles/whole-analyze-1000modules.v1/workerRamBytes` are replaced for
current qualification by the class relation and observed RAM admission above.
The matrix remains frozen as the unchanged quality-cell/corpus identity; its
old universal 4-vCPU/8-GiB fields are historical and cannot override this
explicit successor. No quality row, threshold, fixture or capability changes.
The result's matrixDigest still pins those exact matrix bytes; this separately
pinned successor supplies the current fleet semantics.
