...
def parse(raw,context):
 ...
 utf8_bounds(validation_value,SCHEMAS[t])
 if sequence_fault:fail('RF-7')
...
def main():
 ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--report',default=str(HERE/'control-completion.report.v4.json'));args=ap.parse_args()
 cases_path=HERE/'control-completion.cases.v4.json';corpus=json.loads(cases_path.read_text());results=[]
