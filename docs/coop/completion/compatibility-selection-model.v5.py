...
import copy,datetime,hashlib,importlib.util,json,re
...
def utc_second(value,reason):
 """Strict calendar-valid UTC second; malformed input is a controlled refusal."""
 require(isinstance(value,str) and re.fullmatch(r'[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z',value) is not None,reason)
 try:return datetime.datetime.strptime(value,'%Y-%m-%dT%H:%M:%SZ').replace(tzinfo=datetime.timezone.utc)
 except ValueError:raise Failure(reason)
...
def solve(bundle,x,host):
 ...
 try:
  ...
  now=host['now'];evaluation_time=utc_second(now,'HOST-EVALUATION-TIMESTAMP');require(root['issuedAt']<=now<root['expiresAt'],'ROOT-EXPIRED')
  envelope(bundle,'root',root,'root')
  _,rev,_=envelope(bundle,'revocation',root,'revocation');schema(rev,'revocation')
  issued_time=utc_second(rev['issuedAt'],'REVOCATION-TIMESTAMP')
  ...
  # Security v2 §4.2 and the retained trust machine mark age >90 days stale.
  # host.now is the owner's trusted evaluation high-water, not a sampled wall clock.
  require(evaluation_time-issued_time<=datetime.timedelta(days=90),'REVOCATION-STALE')
