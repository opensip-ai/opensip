...
Status: PROPOSED v4, successor to the frozen v3 package. Author: Codex protocol fixture author. No readiness register
...
`control-completion.protocol-join.v4.json` names exact old/new corpus hashes and
...
selection contains 9,777 cases. Corpus size is finite evidence, not a universal
proof of all streams or all scheduler behavior.

## Replay and limits

`control-completion.check.v4.py` exercises the actual JSON decoder, closed schema,
UTF-8 byte bounds and reference state/correlation checks. With Python3.12 and
jsonschema4.25.1:

```
python docs/coop/completion/control-completion.check.v4.py --report /tmp/control-review.json
```

The retained report is 457/457 PASS over 453 cases, two accepted source pins,
the complete matrix check and the RF5 reachability check. It captures exact checker/schema/corpus hashes.
All v1/v2/v3 files remain frozen history. Platform aliases remain macOS arm64/x86_64 and Linux arm64/x86_64. No shipping
