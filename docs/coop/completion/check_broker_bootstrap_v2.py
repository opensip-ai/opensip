...
M=load('broker_bootstrap','broker-bootstrap.model.v2.py')
CONTROL=load('broker_control','control-completion.check.v5.py')
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--report',default=str(HERE/'broker-bootstrap.report.v2.json'));ap.add_argument('--node',default=shutil.which('node'));args=ap.parse_args()
 results=[]
 ...
 base={'spawn':'spawn-1','component':'component-1','installGeneration':1,'manifest':'a'*64,'platform':'macos-arm64','policy':'b'*64,'projectKey':'PRIVATE-PROJECT-KEY','grantGeneration':1,'pid':1234,'bootUUID':'boot-1','declared':['PT-HOST-EFFECT-BROKERED','PT-FS-WRITE-HOST-STATE','PT-FS-READ-PROJECT'],'denied':[],'scopes':{t:[{'operationRef':'op-'+'2'*32,'target':'host-owned-target','parameters':{'host-approved':'parameter'}}] for t in ['PT-HOST-EFFECT-BROKERED','PT-FS-WRITE-HOST-STATE','PT-FS-READ-PROJECT']}}
 def broker():
 ...
 # BROKER-M1: independently authored required-token/scope regressions.
 repair_cases=[]
 for effect,underlying in [('HE-1','PT-FS-WRITE-HOST-STATE'),('HE-2','PT-FS-READ-PROJECT')]:
  for variant,expected in [('full','GRANTED'),('underlying-missing','PR-1'),('underlying-denied','PR-2'),('underlying-scope','PR-4'),('broker-missing','PR-1'),('broker-denied','PR-2'),('broker-scope','PR-4'),('missing-before-other-deny','PR-1'),('deny-before-other-scope','PR-2'),('revoke-before-scope','PR-5')]:
   b=M.HostBroker(base,entropy=lambda:'a'*32);request=b.register(effect,'op-'+'2'*32,'gj:internal:1:1','host-owned-target',{'host-approved':'parameter'});b.bootstrap()
   if variant=='underlying-missing':b.current['declared'].remove(underlying)
   if variant=='underlying-denied':b.current['denied']=[underlying]
   if variant=='underlying-scope':b.current['scopes'][underlying][0]['target']='wrong-exact-target'
   if variant=='broker-missing':b.current['declared'].remove('PT-HOST-EFFECT-BROKERED')
   if variant=='broker-denied':b.current['denied']=['PT-HOST-EFFECT-BROKERED']
   if variant=='broker-scope':b.current['scopes']['PT-HOST-EFFECT-BROKERED'][0]['parameters']={}
   if variant=='missing-before-other-deny':b.current['declared'].remove(underlying);b.current['denied']=['PT-HOST-EFFECT-BROKERED']
   if variant=='deny-before-other-scope':b.current['denied']=[underlying];b.current['scopes']['PT-HOST-EFFECT-BROKERED']=[]
   if variant=='revoke-before-scope':b.revoke();b.current['scopes'][underlying]=[]
   actual=b.dispatch(request,base);check('repair/required-token/'+effect+'/'+variant,actual['decision']==expected and actual['initiated']==[] and actual['wire']==(None if expected=='GRANTED' else 'RF-6'))
   repair_cases.append({'id':effect+'/'+variant,'body':request,'current':b.current,'transportContext':base,'schedule':b.schedule,'expectedDecision':expected,'actual':actual})
 draws=[]
 def constant_entropy():draws.append(1);return 'c'*32
 b=M.HostBroker(base,entropy=constant_entropy);b.register('HE-1','op-'+'1'*32,'gj:internal:1:1','one',{})
 check('repair/collision-eight-draw-refusal',raises(lambda:b.register('HE-1','op-'+'2'*32,'gj:internal:1:2','two',{}),M.StartupFailure) and len(draws)==9)
 check('repair/collision-no-partial-bootstrap',raises(b.bootstrap,M.StartupFailure) and b.failed and not b.started and not b._entries)
 check('repair/collision-no-effects-existing-grants-revoked',M.SEC.journal({'schedule':b.schedule})['revoked'] and not M.SEC.journal({'schedule':b.schedule})['initiated'])
