...
def parse(raw,context):
 ...
 sequence=v['seq']
 if type(v['type']) is not str or type(v['controlMajor']) is not int or not 1<=v['controlMajor']<=9007199254740991:fail('RF-2')
 if isinstance(sequence,LargeInteger):
  if sequence.startswith('-'):fail('RF-2')
  sequence_fault=True
 elif type(sequence) is int:
  if sequence<0:fail('RF-2')
  sequence_fault=sequence==0 or sequence>9007199254740991
 else:fail('RF-2')
 if not isinstance(v['body'],dict):fail('RF-2')
 if v['type']=='hello' and v['controlMajor']!=1:
  if sequence_fault:fail('RF-7')
 ...
 validation_value={**v,'seq':1 if sequence_fault else sequence,'body':dict(b)}
 numbers(validation_value)
 ...
 # Identity substitutions exist only for the specific RF3 schema exception.
 # The byte/encoding walk must see every original received identity string.
 utf8_bounds({**v,'seq':1 if sequence_fault else sequence},SCHEMAS[t])
 if sequence_fault:fail('RF-7')
...
def run_session(case):
 """Reference supervisor driver: closed receive directions never reopen."""
 state=case['context']['state'];stopped=set();seqs={'host-to-component':1,'component-to-host':1};steps=[]
 for step in case['sessionFrames']:
  direction=step['direction'];raw=bytes.fromhex(step['frameHex'])
  if direction in stopped:
   result={'verdict':'RF-7','bodyBuffered':False}
  else:
   context={**case['context'],'direction':direction,'state':state,'nextSeq':seqs[direction]}
   try:
    result=parse(raw,context);seqs[direction]+=1;state=result['nextState']
    if result['type'] in ['refusal','shutdownAck']:stopped.add(direction)
   except Refusal as e:
    result={'verdict':e.family,'bodyBuffered':e.body_read};stopped.add(direction);state='FAULTED'
  if len(stopped)==2:state='CLOSED'
  steps.append({'verdict':result['verdict'],'bodyBuffered':result['bodyBuffered'],'state':state,'stoppedDirections':sorted(stopped),'nextSeq':dict(seqs)})
 return {'verdict':'TRACE','steps':steps}

def run_case(case):
 if 'sessionFrames' in case:return run_session(case)
...
def main():
 ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--report',default=str(HERE/'control-completion.report.v5.json'));args=ap.parse_args()
 cases_path=HERE/'control-completion.cases.v5.json';corpus=json.loads(cases_path.read_text());results=[]
