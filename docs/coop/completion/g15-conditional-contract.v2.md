# Conditional G15 metadata and lock joins, version 2
...
`compatibility-selection-model.v5.py` first validates the complete new input
...
```
/tmp/opensip-architecture-review-env/bin/python docs/coop/completion/check_g15_conditional_v2.py --report /tmp/g15-conditional-review.v2.json
```
...
Retained result: **3,251/3,251 PASS**, including an unchanged byte-identical replay
...
The original fixture clock is 2026-12-21; the freshness repair also evaluates
2027-03-01. Neither is a claim about current release validity or a real ceremony.

## G15-M1: signed revocation freshness

This narrow successor repairs the independent review's missing age predicate.
The selected security rule is stale only when the trusted evaluation time minus
revocation issuedAt is **greater than 90 days**. Equality remains fresh, matching
security v2 §4.2 and the retained security behavior model's strict greater-than
predicate. The evaluator uses the externally supplied trusted evaluation
high-water; it does not substitute a fresh wall-clock sample or lower a floor.
Existing signature, counter-floor and future-time checks remain in force.

The reference parser now validates the UTC second's calendar before calculating
revocation age. Invalid calendar dates or malformed timestamp spellings produce
controlled refusal. No signed document can turn malformed time into permission
to skip freshness. No new grant, clock-persistence, or trust-state transition
mechanism is introduced here.

Seven retained cases use valid authorized TEST signatures and cover 90 days
minus one second, exact equality, 90 days plus one second, the 150-day failure,
an invalid calendar date, invalid timestamp spelling and an invalid trusted
evaluation timestamp. The original independent review's complete 150-day input
is retained byte-for-byte as an additional regression. Every refused case has an
empty resolved array and no lock. Accepted boundary cases retain the exact same
canonical lock custody.

The original sixty conditional inputs, all 960 matrix assignments, and the
unchanged 149-check resolver-core replay remain. The matrix points to the v2
case corpus without changing its original sixty case values. Frozen v1 subjects
remain unchanged; the new resolver is version 5.
