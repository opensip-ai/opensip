...
S=json.loads((B/'g13-result-schema.v2.json').read_text())
...
def main(args):
    ...
    report={'kind':'G13-result-validator-design-cases','productQualification':False,'environment':{'python':platform.python_version(),'jsonschema':importlib.metadata.version('jsonschema')},'count':len(results),'passed':len(results),'results':results,'pins':{p.name:SHA(p) for p in [Path(__file__),B/'g13-result-schema.v2.json',B/'language-quality-matrix.completed.v2.json']}}
    Path(args.report).write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'count':len(results),'passed':len(results)}))
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--report',default=str(B/'g13-result-design-report.v2.json'));main(p.parse_args())
