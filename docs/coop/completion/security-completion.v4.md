# Security, trust and platform-TCB completion — turn 4 (adjudicated repair)

Author: Claude (Herdr `wH:p2`). Status: PROPOSED, turn 4 under D-368: the bounded repair of the six
findings the fresh adjudicator UPHELD (`security-adjudication.v1.json` `7030a258…`, `.md` `b585b4e4…`),
plus the lead's separately recorded SEC-POLICY-N1 and the lead's broker-cap interpretation, awaiting the
adjudicator's one bounded confirmation. Scope: DR-112, DR-105, DR-114, DR-124, DR-126, DR-101 OD-101-2, the metadata
...
Turn 3 (`security-freeze.v3.json` `9486f6c4f3d8196a1489541dd5bff7d3b7869e15d0b9d88b5a2011a46c761f81`,
65 files, replay 230/230) drew OBJECT at 6 MUST-FIX / 0 SHOULD-FIX (`security-codex-review.v3.json`
`21b2fdee…`), CONTESTED, and every finding was independently UPHELD. The frozen turn-2 and turn-3 bytes
are retained unchanged; turn 4 lives entirely under `.v4` names and `security-freeze.v4.json` carries the
exact per-file diff against turn 3. It binds NOTHING, edits no register cell, authorizes no
implementation, claims no platform qualification, and adopts no successor by itself.
...
Retained with this document, all under `docs/coop/completion/`: `security-schemas.v4/` (eleven closed
JSON Schemas plus `grant-journal.sql`), `security-fixtures.v4/` (53 files), `security-vectors.v4.json`,
`security_unit_lib_v4.py`, `check-security-unit.v4.py`, and the frozen report
`security-unit.v4.report.json` (283 checks, 0 failures; cross-validated through the review
...
**One admission boundary (SEC3-M1).** `admit_root_document(bytes)` = parse → closed shape
(`root.schema.json`) → the semantic rules above, and returns an `AdmittedRoot` value or the refusal
list. `verify_envelope` accepts only an `AdmittedRoot`; a raw dictionary, however shaped, is
`ROOT-NOT-ADMITTED` before any verification step, so no caller can label an unvalidated root admitted.
...
bodySha256: hex64 when seq >= 1 and null only at COMMITTED 0}`; PENDING requires `seq >= 1`; no other
member. Any violation, including a string where an object is expected, quarantines `witnessMalformed`;
only a valid witness can authorize REVERT or ADVANCE.

**Witness reconciliation on open** (executable as `reconcile_witness`, twenty-one retained cases
including six malformed-shape negatives, and a simulated writer crashed after each protocol step
...
journal state and the sealed snapshot manifest must all be present; absence never admits); handle
grammar and operationRef grammar (full match; a trailing byte refuses); unknown handle; operation
...
- **HE-1 HOST-STAGED-STATE-WRITE (ordered algorithm, SEC3-M5).** The SDK writes
  ...
  `IRREVERSIBLE`, carrying the digest, length and the host-minted commit id; (4) commits **from the
  buffer** by temp-then-rename into the staged host-state class; (5) appends **RCO** or **ICO** durably
  with the actual outcome (`COMPLETED | FAILED | INDETERMINATE`, or `REVERTED` for a reverted reversible
  commit); (6) unlinks the stage file (the handle is single-use for HE-1); (7) sends `effectResult` with
  the receipt tied to that outcome record's commit id. No effect precedes its durable authorization and
  intent. **Recovery** (`he1_recover`, twelve retained cases): RA without intent closes as FAILED (the
  effect never began); reversible intent without outcome reverts the staged commit and records
  REVERTED; irreversible intent without outcome is decided by the durable footprint of the committed
  object (present with the recorded digest → COMPLETED, absent → FAILED, undeterminable →
  INDETERMINATE); a missing outcome record never means not-performed; a recorded outcome is never
  re-derived. **The HE-1 `rr:` is a receipt only:** it authenticates the
...
- **SDK resolution** (`resolve_result(ref, reader, ctx)`, nineteen retained cases; SEC3-M4): `ctx` is
  ...
  (`RESULT.CONTEXT_ABSENT | EFFECT_NOT_COMPLETED | REQUEST_SEQ_MISMATCH | REF_NOT_FROM_RESULT |
  SCRATCH_NOT_OWNED | REF_GRAMMAR | OVER_CAP | SPAWN_CAP_EXCEEDED | FILE_NOT_ADMITTED | ABSENT |
  INTEGRITY`) and nothing is sent on the wire. No new provider frame, no
...
`check-security-unit.v4.py --report security-unit.v4.report.json` (283 checks) reproduces every vector,
...
negatives, the raw-dictionary refusal and the weak-threshold probe, twenty-one witness cases plus
simulated crash points, the journal chain, binding,
digest-form locator with bounds and ten refused spellings, and the SQLite guards on a fresh database,
the thirty-one broker-handle cases (including absent context, empty and narrowed broker scopes,
missing snapshot binding, traversal, dot-segment, sealed-membership and trailing-newline targets), the
nineteen result-courier cases with required context, the seven stage-admission cases, the HE-1 order
and twelve recovery cases, the stated caps, the descriptor's validation against
...
As v2 §12 and as the turn-3 text; additionally it does not build, sign or require a custom kernel or root privilege; does not
