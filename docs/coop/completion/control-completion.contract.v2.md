...
Status: PROPOSED v2, successor to the frozen v1 package. Author: Codex protocol fixture author. No readiness register
...
`control-completion.protocol-join.v2.json` names exact old/new corpus hashes and
...
selection contains 9,759 cases. Corpus size is finite evidence, not a universal
proof of all streams or all scheduler behavior.

## Replay and limits

`control-completion.check.v2.py` exercises the actual JSON decoder, closed schema,
UTF-8 byte bounds and reference state/correlation checks. With Python3.12 and
jsonschema4.25.1:

```
python docs/coop/completion/control-completion.check.v2.py --report /tmp/control-review.json
```

The retained report is 439/439 PASS over 435 cases, two accepted source pins,
the complete matrix check and the RF5 reachability check. It captures exact checker/schema/corpus hashes.
All v1 files remain frozen history. Platform aliases remain macOS arm64/x86_64 and Linux arm64/x86_64. No shipping
