...
Status: PROPOSED v5, successor to the frozen v4 package. Author: Codex protocol fixture author. No readiness register
...
`control-completion.protocol-join.v5.json` names exact old/new corpus hashes and
...
selection contains 9,804 cases. Corpus size is finite evidence, not a universal
proof of all streams or all scheduler behavior.

## Replay and limits

`control-completion.check.v5.py` exercises the actual JSON decoder, closed schema,
UTF-8 byte bounds and reference state/correlation checks. With Python3.12 and
jsonschema4.25.1:

```
python docs/coop/completion/control-completion.check.v5.py --report /tmp/control-review.json
```

The retained report is 484/484 PASS over 480 cases, two accepted source pins,
the complete matrix check and the RF5 reachability check. It captures exact checker/schema/corpus hashes.
All v1/v2/v3/v4 files remain frozen history. Platform aliases remain macOS arm64/x86_64 and Linux arm64/x86_64. No shipping
...
## Compound-fault precedence repair

CCR-1 is repaired by recording a sequence range fault without immediately
returning RF7. For a known message, a valid seq sentinel is used only in the
schema-validation copy while every other field is checked. Any other applicable
structural fault yields RF2; only then does the original zero/overflow/large
sequence yield RF7. Arbitrarily large sequence text is still never converted
to int. The raw frame is never modified, and the sentinel never becomes a
received sequence or accepted state. Unsupported-major hello checks only the
frozen envelope, then the pending sequence fault, before body-opaque RF1.

Eight exact compound-invalid cases cover unknown body members/types, array
bodies, floats, string majors, oversized lexemes, and future-major opaque bodies.
Standalone seq fault cases retain RF7. A five-step supervisor trace also shows
a violating direction stop, a discarded later frame on that direction, receipt
of the peer's terminal refusal on the remaining direction, and permanent closure
of both directions. The trace uses an actual stateful driver around the parser;
it is reference design evidence and does not execute an OS process.

## Original identity UTF-8 repair

CCR4-1 is repaired by separating the schema-only identity substitutions from
the UTF-8 byte/representability walk. That walk sees the original parsed frame,
with only a faulty seq replaced by its numeric validation sentinel. Original
stableId and admittedManifestDigest strings are never substituted during this
walk, so malformed encoding or byte length remains structural RF2, including
when simultaneous seq zero/overflow would otherwise select RF7. The specific
RF3 identity mismatch rule continues only after applicable structural checks.

The three independent v4 probe frames and contexts are retained byte-for-byte,
with their corrected expected RF2 outcomes; the review's copied ACCEPT baseline
expectations are not adopted. Twenty-four additional hello/helloAck cases cover
both identity fields, surrogate and 1,025-byte values, and seq1/zero/overflow.
No other schema, state, authority or teardown choice changes.
