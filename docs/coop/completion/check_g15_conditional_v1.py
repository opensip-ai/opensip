...
s=importlib.util.spec_from_file_location('g15_check_model',P/'compatibility-selection-model.v4.py');M=importlib.util.module_from_spec(s);s.loader.exec_module(M)
...
def run(args):
 bundle=load('g15-conditional-bundle.v1.json');host=load('g15-conditional-host.v1.json');corpus=load('g15-conditional-cases.v1.json');goldens=load('g15-conditional-goldens.v1.json');variants=load('g15-conditional-variants.v1.json');matrix=load('g15-conditional-matrix.v1.json');results=[]
...
if __name__=='__main__':
 import sys
 ap=argparse.ArgumentParser();ap.add_argument('--report',default=str(P/'g15-conditional-report.v1.json'));ap.add_argument('--python',default=sys.executable);raise SystemExit(run(ap.parse_args()))
