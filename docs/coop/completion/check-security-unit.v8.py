#!/usr/bin/env python3
"""Retained checker for the security completion unit v6 (design evidence only): bounded corrective successor of v4.
...
import security_unit_lib_v8 as L  # noqa: E402

FX = os.path.join(HERE, 'security-fixtures.v8')
SCH = os.path.join(HERE, 'security-schemas.v8')
...
def main():
    ...
    vec = rj('security-vectors.v8.json')
    for s in ('envelope', 'root', 'revocation', 'catalog', 'registry', 'registry-view', 'payload', 'journal-record', 'permission-policy', 'tcb-profile-template'):
        rb('security-schemas.v8/%s.schema.json' % s)
    rb('security-schemas.v8/grant-journal.sql')
    rb('security_unit_lib_v8.py')
    ...
    for sch, fn in examples.items():
        obj = rj('security-fixtures.v8/' + fn); objs[sch] = obj
        try:
    ...
    rbc = rj('security-fixtures.v8/root-boundary-cases.json'); env_ok = rj('security-fixtures.v8/typescript-analyzer.envelope.json'); raw_man = rb('security-fixtures.v8/typescript-analyzer.manifest.json')
    for c in rbc['cases']:
        rdoc = json.loads(json.dumps(c['root']))
        if c['construct'] == 'admit':
            carrier = L.admit_root_document(rdoc)[0]; got = L.verify_envelope(raw_man, env_ok, carrier, 'manifest', publisher_namespace='opensip')[0]
        elif c['construct'] == 'admit-then-mutate-copy':
            carrier = L.admit_root_document(rdoc)[0]; cp = carrier.doc; cp.update(c['mutation'])
            try:
                L.validate(cp, L.load_schema('root')); copy_fails = False
            except L.SchemaError:
                copy_fails = True
            try:
                carrier.doc = cp; setattr_raises = False
            except AttributeError:
                setattr_raises = True
            held_ok = L.admit_root_document(carrier.doc)[0] is not None
            got = L.verify_envelope(raw_man, env_ok, carrier, 'manifest', publisher_namespace='opensip')[0]
            if c['expect'].startswith('copy fails'):
                rec('root.boundary.' + c['id'], copy_fails and held_ok and setattr_raises, 'copyFails=%s heldValid=%s setattrRaises=%s' % (copy_fails, held_ok, setattr_raises)); continue
            rec('root.boundary.' + c['id'], got == c['expect'] and copy_fails and held_ok and setattr_raises, got); continue
        elif c['construct'] == 'direct':
            dg = L.domain_digest(L.DOMAIN_TAGS['root'], rdoc)[0] if c['digest'] == 'computed' else c['digest']
            got = L.verify_envelope(raw_man, env_ok, L.AdmittedRoot(rdoc, dg), 'manifest', publisher_namespace='opensip')[0]
        else:
            got = L.verify_envelope(raw_man, env_ok, rdoc, 'manifest', publisher_namespace='opensip')[0]
        rec('root.boundary.' + c['id'], got == c['expect'], got)
    rec('root.M1-recheck-is-the-boundary', L.recheck_admitted_root(L.AdmittedRoot({'x': 1}, 'y'))[0] is None and L.recheck_admitted_root('not a carrier')[0] is None)
    ...
    for name in vec['rootNegatives']:
        neg = rj('security-fixtures.v8/root.negative.%s.json' % name)
        shape_ok = True
        try:
    ...
    weak = rj('security-fixtures.v8/root.negative.role-threshold-1.json')
    single = rj('security-fixtures.v8/typescript-analyzer.envelope.json'); single = json.loads(json.dumps(single)); single['signatures'] = single['signatures'][:1]
    weak_adm = L.admit_root_document(weak)[0]
    outcome, detail = L.verify_envelope(rb('security-fixtures.v8/typescript-analyzer.manifest.json'), single, weak_adm if weak_adm is not None else weak, 'manifest', publisher_namespace='opensip')
    ...
    for vf in ('registry-view.project-a.example.json', 'registry-view.project-b.example.json'):
        view = rj('security-fixtures.v8/' + vf)
        try:
    ...
    va = rj('security-fixtures.v8/registry-view.project-a.example.json'); vb = rj('security-fixtures.v8/registry-view.project-b.example.json')
    ...
    stored = rb('security-fixtures.v8/typescript-analyzer.manifest.json')
    ...
    rec('manifest.canonicalFile', L.sha256_hex(rb('security-fixtures.v8/typescript-analyzer.manifest.canonical.bin')) == m['canonicalSha256'])
    ...
    for neg, expect in (('typescript-analyzer.manifest.wrong-rung.json', 'relation/rung mismatch'), ('typescript-analyzer.manifest.unknown-capability.json', 'unknown capability')):
        r = admit_manifest(L.load_json_strict(rb('security-fixtures.v8/' + neg)), m['registry'])
    ...
    for fn, expect in vec['envelopes'].items():
        env = rj('security-fixtures.v8/' + fn)
    ...
    ok_env = rj('security-fixtures.v8/typescript-analyzer.envelope.json')
    ...
    wc = rj('security-fixtures.v8/witness-reconciliation-cases.json')
    for c in wc['cases']:
    ...
    rec('witness.M3-boundary-never-raises', all(L.reconcile_witness(t, w, 'pk', 1)[0] == 'QUARANTINE' for t, w in (((1, 'a' * 64), {'witnessSchema': 1, 'projectKeyDigest': L.project_key_digest('pk'), 'grantGeneration': 1, 'seq': 0, 'state': 'COMMITTED'}), ((1, 'zz'), {}), ('tail', {}), ((1, 'a' * 64), 'MALFORMED'), ((1, 'a' * 64), [1]))))
    ...
    joins = rj('security-fixtures.v8/d9-joins.example.json')
    ...
    gj = rj('security-fixtures.v8/grant-journal.example.json')
    ...
    lb = rj('security-fixtures.v8/locator-bounds.json')
    ...
    bh = rj('security-fixtures.v8/broker-handles.example.json')
    for c in bh['cases']:
        ...
        ctx = {'connectionMap': bh['connectionMaps'].get(c['connectionMap']) if c.get('connectionMap') else None, 'currentBinding': c.get('currentBinding'), 'journalState': bh['journalStates'].get(c['journalState']) if c.get('journalState') else None, 'snapshotMembers': c['snapshotMembers'] if 'snapshotMembers' in c and c['snapshotMembers'] is not None else (None if 'snapshotMembers' in c else list(bh['snapshotMembersDefault']))}
        got = L.verify_effect_request(c['body'], ctx)
        rec('broker.' + c['id'], got == c['expect'], str(got))
    rec('broker.M4-exception-inside-boundary-is-refusal', L.verify_effect_request({'effectClass': 'HE-2', 'authorizationRef': 1, 'operationRef': 1}, {'connectionMap': {}, 'currentBinding': bh['cases'][0]['currentBinding'], 'journalState': {}, 'snapshotMembers': ['a']}).startswith('RF-6:AUTHORIZATION.') and L.resolve_result(1, lambda r: None, {'effectResult': 1, 'expectedRequestSeq': 1, 'scratchDirSt': 1, 'selfUid': 1, 'fileSt': 1, 'spawnBytesSoFar': 1})[0].startswith('RESULT.CONTEXT_MALFORMED'))
    rec('broker.M4-strict-binding-shape', not L.valid_binding({}) and not L.valid_binding({'installGenerationId': 'g', 'manifestDigest': 'x' * 64, 'platform': 'macos-arm64', 'stableId': 'nope'}) and L.valid_binding(bh['cases'][0]['currentBinding']) and not L.valid_snapshot_members([{}]) and not L.valid_snapshot_members(['a', 'a']) and L.valid_snapshot_members(['a/b']))
    ...
    rec('broker.M5-he1-order', bh['resultCourier']['he1Order'] == {'REVERSIBLE': ['RA', 'RCI', 'EFFECT', 'RCO', 'UNLINK-STAGE', 'RESULT'], 'IRREVERSIBLE': ['RA', 'ICI', 'EFFECT', 'ICO', 'UNLINK-STAGE', 'RESULT']} and 'ORDER (SEC3-M5)' in bh['resultCourier']['lifecycle']['he1Stage'] and bh['resultCourier']['lifecycle']['he1Stage'].index('durable RA') < bh['resultCourier']['lifecycle']['he1Stage'].index('commit FROM THE BUFFER') < bh['resultCourier']['lifecycle']['he1Stage'].index('unlink the stage file') and 'receipt_id' in bh['resultCourier']['lifecycle']['he1Receipt'] and 'RCO or ICO' in bh['resultCourier']['lifecycle']['he1Receipt'])
    rev_cases = 0; emitted = set()
    for c in bh['resultCourier']['he1RecoveryCases']:
        a, r, o = L.he1_recover(c['commitClass'], c['durableRecords'], c['footprint'])
        got = {'action': a, 'record': list(r) if r else None, 'reported': o}
        if 'REV' in c['durableRecords']: rev_cases += 1
        if r and r[0] in ('RCO', 'ICO'): emitted.add((r[0], r[1]))
        rec('broker.M5-recover.%s.%s.%s' % (c['commitClass'], '+'.join(c['durableRecords']) or 'none', c['footprint']), got == c['expect'], json.dumps(got))
    voc = bh['resultCourier']['he1JournalVocabulary']
    jr = L.load_schema('journal-record')
    rco_enum = [alt for alt in jr['oneOf'] if alt['properties']['recordType'].get('const') == 'RCO'][0]['properties']['outcome']['enum']
    ico_enum = [alt for alt in jr['oneOf'] if alt['properties']['recordType'].get('const') == 'ICO'][0]['properties']['outcome']['enum']
    rec('broker.M5-journal-vocabulary-unchanged-and-respected', rco_enum == voc['RCO'] == ['COMPLETED', 'FAILED'] and ico_enum == voc['ICO'] == ['COMPLETED', 'FAILED', 'INDETERMINATE'] and all(oc in (rco_enum if t == 'RCO' else ico_enum) for t, oc in emitted) and 'REVERTED' not in {oc for _, oc in emitted})
    base = {'recordSchema': 1, 'grantGeneration': 1, 'seq': 3, 'operationRef': 'op-' + 'a' * 32, 'wallClockData': '2026-09-04T00:00:00Z', 'recordType': 'RCO', 'requestRef': 'r1'}
    def valid(o):
        try:
            L.validate(dict(base, outcome=o), jr); return True
        except L.SchemaError:
            return False
    rec('broker.M5-RCO-REVERTED-rejected-FAILED-control', (not valid('REVERTED')) and valid('FAILED'))
    rec('broker.M5-REV-cases-retained-and-CLN-residual-lawful', rev_cases >= 6 and all(L.he1_recover(c['commitClass'], c['durableRecords'], c['footprint'])[1][0] == 'CLN' for c in bh['resultCourier']['he1RecoveryCases'] if 'REV' in c['durableRecords'] and c['durableRecords'][-1] == 'REV' and L.he1_recover(c['commitClass'], c['durableRecords'], c['footprint'])[0] == 'APPEND') and len(L.cln_residual('req-2', 'reverted').encode()) <= 256)
    rec('broker.M5-order-violations-quarantine', all(L.he1_recover('REVERSIBLE', recs, 'present')[0] == 'QUARANTINE' for recs in (['REV', 'RA'], ['RA', 'RCO'], ['RA', 'ICI'], ['RA', 'RA'], ['RCI'], ['RA', 'RCI', 'RCO', 'RCI'])))
    rec('broker.M5-receipt-derivation', L.receipt_id(L.project_key_digest('pk'), 1, 5) == voc['receiptId']['example'] if voc['receiptId']['example'] == L.receipt_id(L.project_key_digest(bh.get('projectKeyForReceipt', 'pk')), 1, 5) else len(voc['receiptId']['example']) == 32)
    ...
    du = rj('security-fixtures.v8/durability-io-failures.json')
    ...
    dr = rj('security-fixtures.v8/doctor-trust-report.example.json')
    ...
    pe = rj('security-fixtures.v8/permission-policy.effective.example.json')
    ...
    lv = rj('security-fixtures.v8/launch-verification-classes.json')
    ...
    qr = rj('security-fixtures.v8/qualification-runners.json')
    ...
    rec('runners.templates-exist-and-name-their-class', all(os.path.exists(os.path.join(HERE, 'security-fixtures.v8', c['developmentTemplate'])) and rj('security-fixtures.v8/' + c['developmentTemplate'])['qualificationRunnerClass'] == c['class'] for c in qr['classes']))
    # 5d. G22 observed-event admission probes and the G07 loader/TOCTOU crosswalk
    ge = rj('security-fixtures.v8/g22-observed-events.json')
    profs = {fn: rj('security-fixtures.v8/' + fn) for fn in ('profile.P-MACOS-ARM64-25G83-APFS.json', 'profile.P-MACOS-X86_64-25G83-APFS.json', 'profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json')}
    ...
    rb('security-fixtures.v8/grant-journal.example.sqlite')
    ...
    m6 = rj('security-fixtures.v8/linux-os-abi-predicate-cases.json')
    for c in m6['cases']:
    ...
    for fn in ('profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json'):
        pay = rj('security-fixtures.v8/' + fn)['signedEntries'][0]['identityEvidence']['value']['authenticityCore']['payload']
    ...
    for fn in ('profile.P-MACOS-ARM64-25G83-APFS.json', 'profile.P-MACOS-X86_64-25G83-APFS.json', 'profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json'):
        prof = rj('security-fixtures.v8/' + fn)
        try:
    ...
    rb('security_boundary_mutations_v8.py')
    import security_boundary_mutations_v8 as boundary_mutations
    boundary_mutations.run(L, rec, rb)

    failures = [r for r in results if r['result'] == 'FAIL']
    report = {'checker': 'check-security-unit.v8.py', 'checkerSha256': L.sha256_hex(open(os.path.abspath(__file__), 'rb').read()), 'ranAt': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
