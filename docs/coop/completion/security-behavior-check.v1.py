...
spec=importlib.util.spec_from_file_location('model',P/'security-behavior-model.v1.py');m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
...
obj=json.loads((P/'security-behavior-cases.v1.json').read_text());results=[]
...
for c in obj['cases']:
 ...
 if c['model']=='doctor-reader':
  import jsonschema
  schema=json.loads((P/'security-behavior-doctor-schema.v1.json').read_text())
...
for r in local:
 ...
 results.append(r)
report={'schemaVersion':1,'standing':'DESIGN-EVIDENCE-ONLY','status':'PASS' if all(r['status']=='PASS' for r in results) else 'FAIL','caseCount':len(obj['cases']),'localProbeCount':len(local),'passCount':sum(r['status']=='PASS' for r in results),'sources':obj['sources'],'artifactPins':{n:hashlib.sha256((P/n).read_bytes()).hexdigest() for n in ['security-behavior-model.v1.py','security-behavior-author.v1.py','security-behavior-cases.v1.json','security-behavior-check.v1.py','security-behavior-doctor-schema.v1.json']},'results':results}
a=argparse.ArgumentParser();a.add_argument('--report',default=str(P/'security-behavior-report.v1.json'));args=a.parse_args();Path(args.report).write_text(json.dumps(report,indent=2)+'\n');print(report['status'],report['passCount'],'/',len(results))
for r in results:
