...
def journal(x):
 ...
 durable=[]; volatile=None; initiated=[]; inverse=[]; outcomes={}; refusals=[]; revoked=False; revoked_at=None; cln=False; duplicate=0
  ...
  if op=='WRITE':
   ...
   for r in list(durable):
    ...
    if r['type']=='ICI' and req not in outcomes:
     ...
     inverse.append(req);append('RCO',req,'CLEANED');outcomes[req]='CLEANED'
    elif r['type']=='RA' and not any(z.get('request')==req and z['type'] in ('RCI','ICI') for z in durable):outcomes[req]='CANCELED-BY-RECOVERY'
   if revoked and not cln:append('CLN',value='BOUND-EXHAUSTED' if x.get('cleanupFails') else 'CLOSED');apply(durable[-1])
  elif op=='CLEANUP':
   if revoked and not cln:
    for r in durable:
     if r['type']=='RCI' and r.get('request') not in inverse:inverse.append(r['request'])
    append('CLN',value='BOUND-EXHAUSTED' if x.get('cleanupFails') else 'CLOSED');apply(durable[-1])
  else: raise ValueError(op)
 return {'journal':durable,'initiated':initiated,'inverse':inverse,'outcomes':outcomes,'refusals':refusals,'revoked':revoked,'revokedAt':revoked_at,'cleanupClosed':cln,'duplicateRevocations':duplicate,'retryCount':0}
def wording(x):
...
def doctor_reader(x):
 ...
 def need(test,label):
  if not test:errors.append(label)
 required={'schemaVersion','mode','outcome','environment','checks','consentRecords','residualLimitations'}
 need(required<=r.keys(),'envelope-members');need(r.get('schemaVersion')==1,'schema-major');need(not any(k in r for k in ('ready','healthy','ok')),'aggregate-forbidden')
 if errors:return {'accepted':False,'errors':errors}
 env=r['environment'];need(isinstance(env,dict) and {'mode','modeSelectionReason','installedGenerationRef','platform','configurationLayersResolved','offlinePosture','generatedAt','projectRootRef'}<=env.keys(),'environment-members')
 if isinstance(env,dict):need(env.get('mode')==r['mode'] and r['mode'] in ('core','project'),'mode-match')
 need(isinstance(r['checks'],list) and bool(r['checks']),'checks-array')
 def limitation(v):return isinstance(v,dict) and {'statement','cause','wouldBeResolvedBy'}<=v.keys() and isinstance(v['statement'],str) and isinstance(v['cause'],str) and (v['wouldBeResolvedBy'] is None or isinstance(v['wouldBeResolvedBy'],str))
 statuses=[]
 for c in r['checks']:
  if not isinstance(c,dict) or not {'id','class','status','title','evidence','residualLimitation'}<=c.keys():errors.append('check-members');continue
  statuses.append(c['status']);need(c['status'] in STATUSES,'status-closed');need(all(isinstance(c[k],str) for k in ('id','class','title')),'check-types');need(isinstance(c['evidence'],list),'evidence-array')
  if c['status'] in ('PASS','FAIL'):need(bool(c['evidence']),'verdict-evidence')
  for ev in c['evidence']:need(isinstance(ev,dict) and {'kind','observed','bound'}<=ev.keys() and isinstance(ev['kind'],str) and isinstance(ev['bound'],str),'evidence-shape')
  if c['status'] in ('UNDETERMINED','CONSENT-REQUIRED'):need(limitation(c['residualLimitation']),'limitation-required')
  elif c['residualLimitation'] is not None:need(limitation(c['residualLimitation']),'limitation-shape')
 need(isinstance(r['consentRecords'],list) and isinstance(r['residualLimitations'],list),'array-types')
 derived=outcome({'statuses':statuses,'fault':r['outcome']=='OC-5','refused':r['outcome']=='OC-4'})
 need(r['outcome']==derived,'outcome-derived')
 if 'FAIL' in statuses and any(s in ('UNDETERMINED','CONSENT-REQUIRED') for s in statuses):need(bool(r['residualLimitations']),'incomplete-fail-residual')
 return {'accepted':not errors,'outcome':derived if not errors else None,'errors':errors}
...
MODELS.update({'host-record':host_record_valid,'doctor-reader':doctor_reader,'doctor-observe':doctor_observe,'remediation':remediation})
