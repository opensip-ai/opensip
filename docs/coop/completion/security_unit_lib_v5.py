...
_SCHEMA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'security-schemas.v5')
...
def _is_str(v, mx): return isinstance(v, str) and 1 <= len(v.encode('utf-8')) <= mx
...
def valid_connection_entry(e):
    return (isinstance(e, dict) and {'operationRef', 'effectClass', 'brokerLocator', 'underlyingLocator', 'grantGeneration', 'seq', 'binding', 'target'} <= set(e)
            ...
            and valid_binding(e['binding']) and isinstance(e['target'], dict))


def valid_grant_state(g):
    return (isinstance(g, dict) and g.get('status') in ('GRANT', 'REVOKED', 'CLOSED') and g.get('token') in PERMISSION_TOKENS
            and valid_binding(g.get('binding')) and isinstance(g.get('scope'), dict))
...
def valid_effect_result(er):
    return (isinstance(er, dict) and er.get('effectOutcome') in ('COMPLETED', 'FAILED', 'INDETERMINATE') and _strict_int(er.get('requestSeq'), 1, SEQ_MAX)
            and ('resultRef' not in er or isinstance(er['resultRef'], str)))
...
def _verify_effect_request_checked(body, ctx):
    ...
    if not all(isinstance(k, str) and valid_connection_entry(v) for k, v in connection_map.items()):
        return 'RF-6:AUTHORIZATION.CONTEXT_MALFORMED:connectionMap'
    if not all(isinstance(k, str) and valid_grant_state(v) for k, v in journal_state.items()):
...
def he1_recover(commit_class, durable_records, footprint):
    ...
    if commit_class not in HE1_ORDER or footprint not in ('present', 'absent', 'unknown'):
        raise ValueError('recovery inputs')
    if not isinstance(durable_records, (list, tuple)) or not all(r in _RECOVERY_RECORDS for r in durable_records):
    ...
    if intent not in recs:                                   # accepted, never begun
        return ('APPEND', ('CLN', 'not-begun'), 'FAILED') if revoked else ('APPEND', (outcome, 'FAILED'), 'FAILED')
