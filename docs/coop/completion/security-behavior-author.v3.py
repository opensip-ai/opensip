...
S={'type':'string','minLength':1,'maxLength':4096,'maxUtf8Bytes':4096};I={'type':'string','minLength':1,'maxLength':128,'maxUtf8Bytes':128};H={'type':'string','minLength':64,'maxLength':64,'maxUtf8Bytes':64,'pattern':r'^[0-9a-f]{64}(?![\s\S])'};N={'type':'integer','minimum':0,'maximum':9007199254740991}
...
policy=nullable(ob({'authority':S|{'maxLength':512,'maxUtf8Bytes':512},'artifactPath':S|{'maxLength':512,'maxUtf8Bytes':512},'selector':S|{'maxLength':512,'maxUtf8Bytes':512},'digest':H},closed=True))
...
schema['$comment']+=' The composed reader MUST execute maxUtf8Bytes on each annotated typed value, including array items and nullable/conditional branches; a plain validator that ignores this keyword is only structural validation, not full acceptance.'
(P/'security-behavior-doctor-schema.v3.json').write_text(json.dumps(schema,indent=2)+'\n')
...
# V3 repairs SBR2-1/2/3; new author: Codex distribution_fixtures.
# Retain the exact four counterexample inputs supplied by the independent reviewer.
review=json.loads((P/'security-behavior-independent-review.v2.json').read_text())
for probe in review['probes']:
 if probe['name'] in ('full-report-digest-final-newline','full-report-check-label-256-utf8-bytes','full-report-policy-authority-1024-utf8-bytes','full-report-forged-keychain-grant'):
  add('SBR2/reviewer/'+probe['name'],probe['model'],copy.deepcopy(probe['input']),{'accepted':False},['FC-SCHEMA','FC-HOSTILE'],h,'independent review v2 exact counterexample')
base_full=copy.deepcopy(next(c for c in CASES if c['id']=='full-doctor-consent-CA-1-SPAWN')['input'])
def put_path(value,path,replacement):
 for key in path[:-1]:value=value[key]
 value[path[-1]]=replacement
scope_path=['report','consentRecords',0,'scope']
def full_policy():
 x=copy.deepcopy(base_full)
 x['report']['consentRecords'][0]['scope']['policyProvenance']={'authority':'fixture-authority','artifactPath':'policy.json','selector':'$.policy','digest':'c'*64}
 return x
# Every SHA-256 carrier in a complete report, not merely its standalone projection.
for name,path in [('manifest',scope_path+['targetIdentity','admittedManifestDigest']),('contract',scope_path+['designContractRef','sha256']),('policy',scope_path+['policyProvenance','digest']),('admission',scope_path+['productAdmissionRefs',0,'digest'])]:
 for suffix in ['\n','\r\n','\x00','x']:
  x=full_policy();put_path(x,path,'a'*64+suffix)
  add('SBR2/digest/'+name+'/'+suffix.encode().hex(),'doctor-reader',x,{'accepted':False},['FC-SCHEMA','FC-HOSTILE'],h,'SHA-256 exact ASCII encoding')
 x=full_policy();put_path(x,path,'a'*64)
 add('SBR2/digest/'+name+'/equal-64','doctor-reader',x,{'accepted':True},['FC-SCHEMA'],h,'SHA-256 exact ASCII encoding')
# Typed annotations follow array items, nullable/conditional branches, and nested objects.
# Explicit caps are independently enumerated here; no model/schema cap lookup occurs.
fields=[
 ('check-id',['report','checks',0,'id'],128),
 ('scope-check-label',scope_path+['checks',0],128),
 ('scope-owner-label',scope_path+['owners',0],128),
 ('configuration-layer-label',['report','environment','configurationLayersResolved',0],128),
 ('scope-invocation',scope_path+['invocation'],128),
 ('target-generation',scope_path+['targetIdentity','generation'],128),
 ('audit-identity',scope_path+['auditIdentity'],128),
 ('installed-generation',['report','environment','installedGenerationRef'],128),
 ('check-title',['report','checks',0,'title'],4096),
 ('evidence-bound',['report','checks',0,'evidence',0,'bound'],4096),
 ('scope-resource-item',scope_path+['resources',0],4096),
 ('contract-path',scope_path+['designContractRef','path'],4096),
 ('limitation-statement',['report','consentRecords',0,'residualLimitation','statement'],4096),
 ('diagnostic-extension',['report','checks',0,'diagnostic'],1024),
 ('policy-authority',scope_path+['policyProvenance','authority'],512),
 ('policy-artifact-path',scope_path+['policyProvenance','artifactPath'],512),
 ('policy-selector',scope_path+['policyProvenance','selector'],512)]
for name,path,cap in fields:
 for size in [cap,cap+1]:
  for multi in [False,True]:
   x=full_policy();x['report']['consentRecords'][0]['scope']['resources']=['/fixture/resource']
   value=('é'*(size//2)+'x'*(size%2)) if multi else 'x'*size
   put_path(x,path,value)
   add('SBR2/full-byte-bound/'+name+'/'+str(size)+'/'+str(multi),'doctor-reader',x,{'accepted':size<=cap},['FC-SCHEMA','FC-HOSTILE'],h,'typed report caps and source-identity UTF-8 maxStringBytes')
# The fourth source-identity member is a digest: its exact 64-byte grammar is
# stricter than the enclosing 512-byte maximum; digest equality/+1 is above.
# Every legal subtype remains reportable when denied, including excluded ones.
for action,subtypes_for_action in subtypes.items():
 baseline_id={'CA-1':'full-doctor-consent-CA-1-SPAWN','CA-2':'full-doctor-consent-CA-2-None','CA-3':'full-doctor-consent-CA-3-OUT_OF_ROOT_READ','CA-4':'full-doctor-consent-CA-4-PATH-TRUST-STATE-REFRESH'}[action]
 baseline=next(c for c in CASES if c['id']==baseline_id)['input']
 for subtype in subtypes_for_action:
  admitted=(action,subtype) in [('CA-1','SPAWN'),('CA-3','OUT_OF_ROOT_READ'),('CA-4','PATH-TRUST-STATE-REFRESH')]
  for granted in [False,True]:
   x=copy.deepcopy(baseline);cr=x['report']['consentRecords'][0];cr['scope']['actionSubtype']=subtype
   cr.update({'authorization':'granted' if granted else 'denied','execution':'disclosed-trusted-code' if granted else 'refused','effectOutcome':'COMPLETED' if granted else 'DEFINITELY_NOT_PERFORMED'})
   add('SBR2/preview-subtype/'+action+'/'+str(subtype)+'/'+str(granted),'doctor-reader',x,{'accepted':admitted or not granted},['FC-SCHEMA','FC-NOT-SUFFICIENT','FC-SUBTYPE-CLASS'],h,'immutable preview exclusion shared across full report and host pair')
...
obj['repairReviewPin']={'path':'security-behavior-independent-review.v2.json','sha256':hashlib.sha256((P/'security-behavior-independent-review.v2.json').read_bytes()).hexdigest()}
(P/'security-behavior-cases.v3.json').write_text(json.dumps(obj,indent=2,ensure_ascii=False)+'\n')
print(len(CASES))
