-- PROPOSED lifecycle carrier v1. Design DDL, not product implementation.
...
-- lifecycle-carrier.contract.v1.json; absence raises and fails closed.
...
CREATE TABLE generation (
  id TEXT PRIMARY KEY NOT NULL CHECK (length(id)=36 AND id=lower(id) AND substr(id,9,1)='-' AND substr(id,14,1)='-' AND substr(id,19,1)='-' AND substr(id,24,1)='-' AND length(replace(id,'-',''))=32 AND replace(id,'-','') NOT GLOB '*[^0-9a-f]*' AND substr(id,15,1)='4' AND substr(id,20,1) IN ('8','9','a','b')),
  manifestDigest TEXT NOT NULL CHECK (length(manifestDigest)=64 AND manifestDigest NOT GLOB '*[^0-9a-f]*'),
  lockDigest TEXT NOT NULL CHECK (length(lockDigest)=64 AND lockDigest NOT GLOB '*[^0-9a-f]*'),
  platform TEXT NOT NULL CHECK(platform IN ('macos-arm64','macos-x86_64','linux-arm64','linux-x86_64')),
  immutablePath TEXT NOT NULL UNIQUE CHECK(immutablePath='generations/' || id),
  state TEXT NOT NULL CHECK(state IN ('PREPARING','VERIFIED','READY','QUARANTINED')),
  ...
  permissionPolicyDigest TEXT CHECK (length(permissionPolicyDigest)=64 AND permissionPolicyDigest NOT GLOB '*[^0-9a-f]*'),
...
CREATE TABLE project_registry (
  projectKey TEXT PRIMARY KEY NOT NULL CHECK (length(CAST(projectKey AS BLOB)) BETWEEN 1 AND 1024 AND instr(projectKey,char(0))=0),
  namespaceId TEXT NOT NULL UNIQUE CHECK (length(namespaceId)=36 AND namespaceId=lower(namespaceId) AND substr(namespaceId,9,1)='-' AND substr(namespaceId,14,1)='-' AND substr(namespaceId,19,1)='-' AND substr(namespaceId,24,1)='-' AND length(replace(namespaceId,'-',''))=32 AND replace(namespaceId,'-','') NOT GLOB '*[^0-9a-f]*' AND substr(namespaceId,15,1)='4' AND substr(namespaceId,20,1) IN ('8','9','a','b')),
  platform TEXT NOT NULL CHECK(platform IN ('macos','linux')),
  rootPathBytesHex TEXT NOT NULL CHECK(length(rootPathBytesHex)>=2 AND length(rootPathBytesHex)%2=0 AND substr(rootPathBytesHex,1,2)='2f' AND rootPathBytesHex NOT GLOB '*[^0-9a-f]*'),
  deviceId TEXT NOT NULL CHECK(length(deviceId) BETWEEN 1 AND 20 AND deviceId NOT GLOB '*[^0-9]*' AND (deviceId='0' OR substr(deviceId,1,1)<>'0') AND (length(deviceId)<20 OR deviceId<='18446744073709551615')),
  inodeId TEXT NOT NULL CHECK(length(inodeId) BETWEEN 1 AND 20 AND inodeId NOT GLOB '*[^0-9]*' AND (inodeId='0' OR substr(inodeId,1,1)<>'0') AND (length(inodeId)<20 OR inodeId<='18446744073709551615')),
...
CREATE TABLE project_selection (
  projectKey TEXT PRIMARY KEY NOT NULL CHECK (length(CAST(projectKey AS BLOB)) BETWEEN 1 AND 1024 AND instr(projectKey,char(0))=0) REFERENCES project_registry(projectKey) ON DELETE RESTRICT ON UPDATE RESTRICT,
  generationId TEXT NOT NULL REFERENCES generation(id) ON DELETE RESTRICT ON UPDATE RESTRICT
) STRICT;

CREATE TABLE operation_lease (
  leaseId TEXT PRIMARY KEY NOT NULL CHECK (length(leaseId)=36 AND leaseId=lower(leaseId) AND substr(leaseId,9,1)='-' AND substr(leaseId,14,1)='-' AND substr(leaseId,19,1)='-' AND substr(leaseId,24,1)='-' AND length(replace(leaseId,'-',''))=32 AND replace(leaseId,'-','') NOT GLOB '*[^0-9a-f]*' AND substr(leaseId,15,1)='4' AND substr(leaseId,20,1) IN ('8','9','a','b')),
  projectKey TEXT NOT NULL CHECK (length(CAST(projectKey AS BLOB)) BETWEEN 1 AND 1024 AND instr(projectKey,char(0))=0) REFERENCES project_registry(projectKey) ON DELETE RESTRICT ON UPDATE RESTRICT,
  generationId TEXT NOT NULL REFERENCES generation(id) ON DELETE RESTRICT ON UPDATE RESTRICT,
  supervisorBootId TEXT NOT NULL CHECK (length(supervisorBootId)=36 AND supervisorBootId=lower(supervisorBootId) AND substr(supervisorBootId,9,1)='-' AND substr(supervisorBootId,14,1)='-' AND substr(supervisorBootId,19,1)='-' AND substr(supervisorBootId,24,1)='-' AND length(replace(supervisorBootId,'-',''))=32 AND replace(supervisorBootId,'-','') NOT GLOB '*[^0-9a-f]*' AND substr(supervisorBootId,15,1)='4' AND substr(supervisorBootId,20,1) IN ('8','9','a','b')),
  processStartToken TEXT NOT NULL CHECK(length(processStartToken) BETWEEN 1 AND 256 AND instr(processStartToken,char(0))=0)
) STRICT;

CREATE TABLE transition (
  txId TEXT PRIMARY KEY NOT NULL CHECK (length(txId)=36 AND txId=lower(txId) AND substr(txId,9,1)='-' AND substr(txId,14,1)='-' AND substr(txId,19,1)='-' AND substr(txId,24,1)='-' AND length(replace(txId,'-',''))=32 AND replace(txId,'-','') NOT GLOB '*[^0-9a-f]*' AND substr(txId,15,1)='4' AND substr(txId,20,1) IN ('8','9','a','b')),
  projectKey TEXT NOT NULL CHECK (length(CAST(projectKey AS BLOB)) BETWEEN 1 AND 1024 AND instr(projectKey,char(0))=0) REFERENCES project_registry(projectKey) ON DELETE RESTRICT ON UPDATE RESTRICT,
  oldGeneration TEXT REFERENCES generation(id) ON DELETE RESTRICT ON UPDATE RESTRICT,
  newGeneration TEXT NOT NULL REFERENCES generation(id) ON DELETE RESTRICT ON UPDATE RESTRICT,
  phase TEXT NOT NULL CHECK(phase IN ('PREPARING','VERIFIED','READY','COMMITTED','ABORTED')),
...
CREATE TABLE quarantine (
  generationId TEXT PRIMARY KEY NOT NULL REFERENCES generation(id) ON DELETE RESTRICT ON UPDATE RESTRICT,
  reason TEXT NOT NULL CHECK(reason IN ('INCOMPLETE-PUBLICATION','CONTENT-MISMATCH','CURRENT-TRUST-REFUSAL','AMBIGUOUS-STATE')),
  observedDigest TEXT CHECK (length(observedDigest)=64 AND observedDigest NOT GLOB '*[^0-9a-f]*')
) STRICT;
