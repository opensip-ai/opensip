#!/usr/bin/env python3
"""Retained checker for the security completion unit v3 (design evidence only).
...
import security_unit_lib_v3 as L  # noqa: E402

FX = os.path.join(HERE, 'security-fixtures.v3')
SCH = os.path.join(HERE, 'security-schemas.v3')
...
def main():
    ...
    vec = rj('security-vectors.v3.json')
    for s in ('envelope', 'root', 'revocation', 'catalog', 'registry', 'registry-view', 'payload', 'journal-record', 'permission-policy', 'tcb-profile-template'):
        rb('security-schemas.v3/%s.schema.json' % s)
    rb('security-schemas.v3/grant-journal.sql')
    rb('security_unit_lib_v3.py')
    ...
    for sch, fn in examples.items():
        obj = rj('security-fixtures.v3/' + fn); objs[sch] = obj
        try:
            L.validate(obj, L.load_schema(sch)); rec('schema:' + sch, True)
        except L.SchemaError as e:
            rec('schema:' + sch, False, str(e))
    root = objs['root']
    rec('root.semantic-admission.positive', L.admit_root(root) == [], str(L.admit_root(root))[:200])
    for name in vec['rootNegatives']:
        neg = rj('security-fixtures.v3/root.negative.%s.json' % name)
        shape_ok = True
        try:
        ...
        rec('root.semantic-admission.negative:' + name, bool(refusals), 'shape-valid=%s refusals=%s' % (shape_ok, refusals[:3]))
    # a shape-valid root with a weak role threshold must not let one signature verify
    weak = rj('security-fixtures.v3/root.negative.role-threshold-1.json')
    single = rj('security-fixtures.v3/typescript-analyzer.envelope.json'); single = json.loads(json.dumps(single)); single['signatures'] = single['signatures'][:1]
    outcome, detail = L.verify_envelope(rb('security-fixtures.v3/typescript-analyzer.manifest.json'), single, weak, 'manifest', publisher_namespace='opensip')
    ...
    for vf in ('registry-view.project-a.example.json', 'registry-view.project-b.example.json'):
        view = rj('security-fixtures.v3/' + vf)
        try:
    ...
    va = rj('security-fixtures.v3/registry-view.project-a.example.json'); vb = rj('security-fixtures.v3/registry-view.project-b.example.json')
    ...
    stored = rb('security-fixtures.v3/typescript-analyzer.manifest.json')
    ...
    rec('manifest.canonicalFile', L.sha256_hex(rb('security-fixtures.v3/typescript-analyzer.manifest.canonical.bin')) == m['canonicalSha256'])
    ...
    for neg, expect in (('typescript-analyzer.manifest.wrong-rung.json', 'relation/rung mismatch'), ('typescript-analyzer.manifest.unknown-capability.json', 'unknown capability')):
        r = admit_manifest(L.load_json_strict(rb('security-fixtures.v3/' + neg)), m['registry'])
    ...
    for fn, expect in vec['envelopes'].items():
        env = rj('security-fixtures.v3/' + fn)
    ...
    ok_env = rj('security-fixtures.v3/typescript-analyzer.envelope.json')
    ...
    wc = rj('security-fixtures.v3/witness-reconciliation-cases.json')
    for c in wc['cases']:
    ...
    def simulate(crash_after):
        journal = [(1, 'h1'), (2, 'h2')]; witness = {'projectKeyDigest': L.project_key_digest('pk'), 'grantGeneration': 1, 'seq': 2, 'bodySha256': 'h2', 'state': 'COMMITTED'}
        n_, h_ = 3, 'h3'
        if crash_after >= 1: witness = {'projectKeyDigest': L.project_key_digest('pk'), 'grantGeneration': 1, 'seq': n_, 'bodySha256': h_, 'state': 'PENDING'}   # step 2 durable
        if crash_after >= 2: journal.append((n_, h_))                                                                              # step 3 commit durable
        if crash_after >= 3: witness = {'projectKeyDigest': L.project_key_digest('pk'), 'grantGeneration': 1, 'seq': n_, 'bodySha256': h_, 'state': 'COMMITTED'}  # step 4 durable
    ...
    joins = rj('security-fixtures.v3/d9-joins.example.json')
    ...
    gj = rj('security-fixtures.v3/grant-journal.example.json')
    ...
    lb = rj('security-fixtures.v3/locator-bounds.json')
    ...
    bh = rj('security-fixtures.v3/broker-handles.example.json')
    for c in bh['cases']:
        ...
        got = L.verify_effect_request(c['body'], bh['connectionMaps'][c['connectionMap']], c['currentBinding'], bh['journalStates'][c['journalState']], set(c['snapshotMembers']) if c.get('snapshotMembers') is not None else None)
    ...
    rec('broker.target-order-normalization-before-prefix', not L.target_within_scope('HE-2', {'snapshotDigest': 'd', 'memberPath': 'src/../private'}, {'pathPrefixes': ['src'], 'snapshotDigest': 'd'}) and L.target_within_scope('HE-2', {'snapshotDigest': 'd', 'memberPath': 'src/a.ts'}, {'pathPrefixes': ['src'], 'snapshotDigest': 'd'}) and not L.target_within_scope('HE-2', {'snapshotDigest': 'd', 'memberPath': 'srcx/a.ts'}, {'pathPrefixes': ['src'], 'snapshotDigest': 'd'}) and not L.target_within_scope('HE-2', {'snapshotDigest': 'other', 'memberPath': 'src/a.ts'}, {'pathPrefixes': ['src'], 'snapshotDigest': 'd'}) and not L.target_within_scope('HE-2', {'snapshotDigest': 'd', 'memberPath': 'src/a.ts'}, {'pathPrefixes': ['src'], 'snapshotDigest': 'd'}, {'src/b.ts'}))
    ...
    for c in bh['resultCourier']['cases']:
        outcome, data = L.resolve_result(c['resultRef'], lambda rid, sc=c['scratch']: sc[rid].encode() if rid in sc else None, c.get('effectResult'), c.get('expectedRequestSeq'), c.get('scratchDirSt'), c.get('selfUid'), c.get('fileSt'), c.get('spawnBytesSoFar', 0))
    ...
    du = rj('security-fixtures.v3/durability-io-failures.json')
    ...
    dr = rj('security-fixtures.v3/doctor-trust-report.example.json')
    ...
    pe = rj('security-fixtures.v3/permission-policy.effective.example.json')
    ...
    lv = rj('security-fixtures.v3/launch-verification-classes.json')
    ...
    qr = rj('security-fixtures.v3/qualification-runners.json')
    ...
    rec('runners.templates-exist-and-name-their-class', all(os.path.exists(os.path.join(HERE, 'security-fixtures.v3', c['developmentTemplate'])) and rj('security-fixtures.v3/' + c['developmentTemplate'])['qualificationRunnerClass'] == c['class'] for c in qr['classes']))
    # 5d. G22 observed-event admission probes and the G07 loader/TOCTOU crosswalk
    ge = rj('security-fixtures.v3/g22-observed-events.json')
    profs = {fn: rj('security-fixtures.v3/' + fn) for fn in ('profile.P-MACOS-ARM64-25G83-APFS.json', 'profile.P-MACOS-X86_64-25G83-APFS.json', 'profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json')}
    ...
    rb('security-fixtures.v3/grant-journal.example.sqlite')
    ...
    for fn in ('profile.P-MACOS-ARM64-25G83-APFS.json', 'profile.P-MACOS-X86_64-25G83-APFS.json', 'profile.P-LINUX-X86_64-UBUNTU2404-EXT4.json', 'profile.P-LINUX-ARM64-UBUNTU2404-EXT4.json'):
        prof = rj('security-fixtures.v3/' + fn)
        try:
    ...
    report = {'checker': 'check-security-unit.v3.py', 'checkerSha256': L.sha256_hex(open(os.path.abspath(__file__), 'rb').read()), 'ranAt': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
