"""Shared library for the security completion unit v3 (design evidence only).
...
def admit_root(root):
    ...
    refs(root.get('kernelAttestationKeys', []), 'kernelAttestationKeys')
...
def reconcile_witness(journal_tail, witness, project_key, grant_generation, terminal=False):
    ...
    if witness == 'MALFORMED':
        return 'QUARANTINE', 'witness malformed: witnesslessRestore'
    if witness is None:
        if tail_seq == 0:
    ...
    if witness.get('projectKeyDigest') != project_key_digest(project_key) or witness.get('grantGeneration') != grant_generation:
        return 'QUARANTINE', 'witness names another carrier'
    w_seq, w_sha, w_state = witness.get('seq'), witness.get('bodySha256'), witness.get('state')
    if w_state not in ('PENDING', 'COMMITTED') or not isinstance(w_seq, int) or w_seq < 0:
        return 'QUARANTINE', 'witness malformed state'
...
def verify_envelope(stored_bytes, env, root, expected_kind, publisher_namespace=None):
    ...
    if admit_root(root):
        return 'ROOT-NOT-ADMITTED', ';'.join(admit_root(root))[:200]
...
_SCHEMA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'security-schemas.v3')
...
def locator(project_key, grant_generation, seq):
    ...
    assert LOCATOR_RE.match(loc) and LOCATOR_MIN_BYTES <= len(loc.encode('ascii')) <= LOCATOR_MAX_BYTES
    return loc


def parse_locator(loc):
    """Host-side parse. Returns (projectKeyDigest, grantGeneration, seq) or None when not in the grammar."""
    if not isinstance(loc, str) or not LOCATOR_RE.match(loc):
        return None
    _, d, g, s = loc.split(':')
    g, s = int(g), int(s)
...
def verify_effect_request(body, connection_map, current_binding, journal_state, snapshot_members=None):
    """Section 7.4 host-side verification of an effectRequest body after RF-2..RF-5 passed.
    connection_map: the handles minted for THIS connection (one handle per GRANT record; several handles may
    share one operationRef): {handle: {operationRef, effectClass, brokerLocator, underlyingLocator,
    grantGeneration, seq, binding{installGenerationId, manifestDigest, platform, stableId}, target}}.
    current_binding: the connection's admitted identity (PR-4: the caller is the spawn the map was minted for).
    journal_state: {locator: {'status': 'GRANT'|'REVOKED'|'CLOSED', 'token': ..., 'binding': {...}, 'scope': {...}}}.
    Returns None (proceed to RA) or an RF-6 refusal reason from the closed list."""
    ar, opr, ec = body.get('authorizationRef'), body.get('operationRef'), body.get('effectClass')
    if not isinstance(ar, str) or not HANDLE_RE.match(ar):
        return 'RF-6:AUTHORIZATION.HANDLE_GRAMMAR'
    if not isinstance(opr, str) or not OPERATION_REF_RE.match(opr):
    ...
    under = journal_state[entry['underlyingLocator']]
    if not target_within_scope(ec, entry['target'], under.get('scope', {}), snapshot_members):
        return 'RF-6:AUTHORIZATION.TARGET_OUTSIDE_SCOPE'
    return None
...
def normalized_member_path(path):
    """A sealed-snapshot member path: relative, NFC, '/'-separated, no empty, '.' or '..' segment, no
    backslash or NUL, at most 1024 UTF-8 bytes. Returns True only for the exact normalized spelling."""
    ...
    if unicodedata.normalize('NFC', path) != path or '\\' in path or '\x00' in path or path.startswith('/'):
        return False
    segs = path.split('/')
    return all(seg not in ('', '.', '..') for seg in segs)


def target_within_scope(effect_class, target, scope, snapshot_members=None):
    """The exact prebound target of the handle must lie inside the underlying grant's scope at request
    time. Order for HE-2 (each step refuses before the next): (1) the member path is a normalized
    snapshot member path (no dot or traversal segment), (2) the target names the grant's sealed snapshot,
    (3) the path is a member of that sealed snapshot's manifest (exact lookup, when the manifest is
    supplied), (4) the path lies under a granted prefix by whole path segments. Only then is the
    textual prefix relation consulted, so 'src/../private' never reaches it."""
    if effect_class == 'HE-2':
        path = target.get('memberPath')
        if not normalized_member_path(path):
            return False
        if target.get('snapshotDigest') != scope.get('snapshotDigest', target.get('snapshotDigest')):
            return False
        if snapshot_members is not None and path not in snapshot_members:
            return False
        return any(path == p or path.startswith(p.rstrip('/') + '/') for p in scope.get('pathPrefixes', []) if normalized_member_path(p))
    if effect_class == 'HE-1':
        return target.get('stateClass') == scope.get('stateClass') and isinstance(target.get('byteCap'), int) and 0 < target['byteCap'] <= RESULT_BYTES_MAX_PER_RESULT
    return False
...
def stage_file_admission(st, host_uid, byte_cap):
    ...
    openat(heldScratchFd, ..., O_RDONLY|O_NOFOLLOW|O_CLOEXEC). st = {'type': 'regular'|'symlink'|'directory'|
    'fifo'|..., 'nlink': int, 'uid': int, 'size': int}. Returns None (admit) or the FAILED reason. The bytes
    are then read ONCE from that fd into a buffer of exactly st.size (bounded before allocation), digested
    and committed from the buffer, so the receipt digest is the digest of the committed bytes by
    construction; the stage file is consumed (unlinked) and the handle is single-use for HE-1."""
    if st.get('type') != 'regular':
        return 'STAGE.NOT_REGULAR'          # O_NOFOLLOW already refuses a symlink with ELOOP; recorded here as the same class
    if st.get('nlink', 0) != 1:
        return 'STAGE.HARD_LINKED'
    if st.get('uid') != host_uid:
        return 'STAGE.WRONG_OWNER'
    if not (0 <= st.get('size', -1) <= byte_cap):
...
def resolve_result(result_ref, read_scratch, effect_result=None, expected_request_seq=None, scratch_dir_st=None, self_uid=None, file_st=None, spawn_bytes_so_far=0):
    """SDK-side resolution, in this order, each step an SDK-local typed error (nothing on the wire):
    (1) the effectResult that carried the ref is COMPLETED and its requestSeq is the SDK's own request;
    (2) the held scratch directory (opened once at startup with O_DIRECTORY|O_NOFOLLOW) is owned by this
    uid, mode 0700, not a symlink; (3) the ref grammar and per-result cap; (4) the aggregate per-spawn cap;
    (5) the file opened by openat(held, '<rid>.bin', O_RDONLY|O_NOFOLLOW|O_CLOEXEC) is regular, nlink 1,
    owned by this uid, and its size equals the ref length (the buffer is allocated only now, at that
    size); (6) the bytes read equal the length and the digest. Returns ('OK', bytes) or ('RESULT.<reason>', None)."""
    if effect_result is not None:
        if effect_result.get('effectOutcome') != 'COMPLETED':
            return 'RESULT.EFFECT_NOT_COMPLETED', None
        if expected_request_seq is not None and effect_result.get('requestSeq') != expected_request_seq:
            return 'RESULT.REQUEST_SEQ_MISMATCH', None
        if effect_result.get('resultRef') != result_ref:
            return 'RESULT.REF_NOT_FROM_RESULT', None
    if scratch_dir_st is not None and (scratch_dir_st.get('type') != 'directory' or scratch_dir_st.get('uid') != self_uid or scratch_dir_st.get('mode', 0) & 0o077):
        return 'RESULT.SCRATCH_NOT_OWNED', None
    if not isinstance(result_ref, str) or not RESULT_REF_RE.match(result_ref):
    ...
    if spawn_bytes_so_far + length > RESULT_BYTES_MAX_PER_SPAWN:
        return 'RESULT.SPAWN_CAP_EXCEEDED', None
    if file_st is not None and (file_st.get('type') != 'regular' or file_st.get('nlink') != 1 or file_st.get('uid') != self_uid or file_st.get('size') != length):
        return 'RESULT.FILE_NOT_ADMITTED', None
    data = read_scratch(rid)
    if data is None:
        return 'RESULT.ABSENT', None
    if len(data) != length or sha256_hex(data) != digest:
...
def _prefix_within(child, parents):
    return any(child == p or child.startswith(p.rstrip('/') + '/') for p in parents)
