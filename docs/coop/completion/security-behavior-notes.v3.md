...
```
/tmp/opensip-architecture-review-env/bin/python docs/coop/completion/security-behavior-check.v3.py --report /tmp/security-behavior-independent-report.json
```
...
## Version 3 repair ownership and bound

Codex `distribution_fixtures` authored this successor after the previous author's
session ended. All eight version 2 evidence files remain unchanged. The new
corpus preserves its first 1298 cases exactly and adds 112 cases, including the
four complete counterexample inputs from the pinned independent review v2.
The author script never imports the model or obtains expected values from it.

SBR2-1: all four complete-report digest carriers now use a 64-character/byte
ASCII grammar and an absolute end assertion. Trailing newline, CRLF, NUL and
ordinary suffix mutations refuse. Each carrier also has an exact-64 positive.

SBR2-2: `maxUtf8Bytes` schema annotations carry each type's byte limit through
arrays, nullable values and conditional branches. The composed reader registers
and executes this keyword using the jsonschema extension interface. A generic
validator that ignores the keyword is structural validation only. The existing
general extension-field, container, document and depth limits remain active.
Complete-report ASCII and multibyte equality/+1 cases cover label-array items,
identities, general strings, diagnostics and policy authority/path/selector.
Policy digest is the fourth source-identity member; its exact 64-byte grammar is
stricter than the enclosing 512-byte limit and has its own equality/+1 cases.

SBR2-3: full host pairs and embedded doctor consent records call one shared
immutable preview-exclusion predicate. The complete-report corpus exercises
all ten legal class/subtype pairs both granted and denied. Excluded subtypes
remain valid denied observations, and none can claim a preview grant.

The retained result is 1415/1415 PASS: 1410 model cases and five local ambient
probes. Replay writes to an external `--report` path and reproduces the retained
report exactly. Security normative integration and OS/product qualification
