# Security, trust and platform-TCB completion — lead-owned corrective integration v8
...
bounded confirmations (v4 and v5); LEAD-CORRECTION-REVIEW 1 OBJECT (2 MUST/1 SHOULD),
retained as security-lead-review.v1.json; LEAD-CORRECTION-REVIEW 2 is now required.
This is not a reset or another confirmation under the exhausted first exception.
All earlier frozen units and failure probes remain unchanged. Before v7 authoring,
the lead notified the user in commentary: “The completed security review rejects
v6 with two required fixes and one documentation fix. It confirms that the earlier
v5 defects are repaired. I’m addressing the envelope typing, policy restriction
and context-name issues on new v7 paths, then sending the frozen repair back for
independent review.” M1/M3/M4/M5 and
inseparable regressions are repaired here; M2/M6, N1 and caps remain governed by
their accepted dispositions. The exact per-file diff against v6 accompanies the
...
Retained with this document, all under `docs/coop/completion/`: `security-schemas.v8/` (eleven closed
JSON Schemas plus `grant-journal.sql`, byte-identical to v4), `security-fixtures.v8/` (54 files),
`security-vectors.v8.json`, `security_unit_lib_v8.py`, `check-security-unit.v8.py`, and the frozen report
`security-unit.v8.report.json` (864 checks, 0 failures; cross-validated through the review
...
- **HE-1 HOST-STAGED-STATE-WRITE (ordered algorithm, SEC3-M5).** The SDK writes
  ...
  It closes the dead operation’s grant generation; subsequent work requires a fresh
  generation and grants. This is the fail-closed availability cost of recovery.
...
- **SDK resolution** (`resolve_result(ref, reader, ctx)`, nineteen retained cases; SEC3-M4): `ctx` is
  one required record `{effectResult, expectedRequestSeq, scratchDirSt, selfUid, fileSt,
...
`check-security-unit.v8.py --report security-unit.v8.report.json` (864 checks) reproduces every vector,
...
`security_boundary_mutations_v8.py` applies type-changing mutations at every
...
## 11. Lead review 1 repair (v7)

The full signature envelope is canonical-profile validated before its schema or
signature use. A float `envelopeSchema: 2.0` is a typed verification refusal even
though JSON Schema numeric const equality alone considers it equal to 2. NFD,
wrong types, unknown members and missing required envelope members likewise
cannot verify. Positive valid envelope verification and all prior refusal
precedence cases remain tested.

`merge_policy` is explicitly the raw-policy admission and narrowing boundary.
Both supplied raw documents undergo the closed permission-policy schema and
canonical validation inside it, before scope/duplicate logic. Every exception
is `POLICY.SHAPE`, with no effective grants. Other policy refusals also return
an empty result, never a partially widened grant. A project `stateClass` must
equal an explicitly present global value; an absent global state class cannot
be created by a project layer. Omission in the project layer inherits the global
field as before. List fields retain normalized whole-segment/subset narrowing.
The operator source schemas and their scopes are unchanged.

The higher-level file-absence adapter `effective_policy_for_operation` maps a
missing global file to EMPTY_GLOBAL_POLICY and a missing selected-project file
to EMPTY_PROJECT_POLICY, matching foundation §4. Direct `merge_policy(None, ...)`
is malformed; a missing document is not silently an authority grant. The adapter
requires a strict Boolean project-selection flag. These absence controls are
separate from malformed source input.

The v7 mutation suite additionally covers every full envelope and policy member,
the exact schema-valid state-class widening and downstream no-grant composition,
all Claude policy-shape probes, and the normative courier-context spelling.
Root canonical failures have the fixed `ROOT.CANONICAL` token; exception class
names no longer vary this boundary’s reason. Pure `_target_within_scope` also
rejects malformed scope inputs defensively; enforcing authorization stays in
`verify_effect_request`. No prior frozen file, wire schema, permission schema,
journal vocabulary, numeric bound, platform requirement or scope ride changes.


## 12. Lead review 2 repair

Lead review 2 found no MUST-FIX and one SHOULD-FIX: canonical-profile rejection
at the envelope boundary was exposed as `RJ-4 ENVELOPE_MISMATCH` with an
exception detail. V8 maps both schema and canonical-profile malformed envelopes
to the fixed malformed-envelope refusal `RJ-4 UNSIGNED`, before routing, digest or
signature use. Valid envelopes and all routing/digest/signature precedence cases
remain unchanged. This bounded correction preserves the closed refusal vocabulary
and does not alter trust decisions.
