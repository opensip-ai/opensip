...
> **Date:** 2026-08-14
> **Protocol:** D-000 turn 3 of 3. Same cycle as turns 1
> and 2. Frozen turn-1 and turn-2 subjects are not
> edited. Frozen D-096 / D-097 / D-099 / D-101 / D-102
> subjects are not edited. Not a SATISFIED re-record.
...
Turn-1 subject `coordinator-decisions.D-100.draft.md`
`aaf3dea0ce4736c09eb63e45b95ba6556417632caa79f8594f93b00bf5ede7d0`
held frozen. Turn-2 subject
`coordinator-decisions.D-100.turn2.draft.md`
`13d7e9082c42244a533a960d78cd910b4f7fb068d5e843c91b087fc53d8bffba`
held frozen. Claude 2 OBJECT, 1 MUST-FIX D100-T2-MF-1.
Codex CONSENT.

| ID | Sev | Disposition |
|---|---|---|
| D100-T2-MF-1 | MUST-FIX | ACCEPTED. Candidate-keying is the base grant-coverage test. The row rule is only the 106/109/113 special case. Windows is not a file-08 row. |
| ADV-D100-01 | MUST-FIX | Remains accepted: 106/109/113 still one selected row per recording. |
...
2. **Preconditions for one later owner-recording entry.**
   ...
   - an applicable user-made grant (D-096 (A)) is ADOPTED,
     ...
     role for the **candidate** being recorded, and
     where that candidate maps to a file-08 row, for
     the row being recorded (for the 106/109/113
     candidate, the selected row's roles; the Windows
     platform set is not a file-08 row);
...
3. **The only eligible candidates (digests as recorded
   ...
   | `deferral.DR-106-109-113.preview.v2.json` | `194b01fc6bd9201ebdeee71b83846cff09417d7521464bd81a0fbe2aabf7502c` | Shared draft only. See special rule below. |
   ...
   **Special rule — `deferral.DR-106-109-113.preview.v2.json`.**
   Each later owner-recording entry must name **exactly
   one** of DR-106, DR-109, or DR-113. The applicable
   D-096 (A) grant must cover every named role for that
   selected row (DR-106: Product + evidence + release;
   DR-109: Evidence + storage architecture; DR-113:
   Evidence + product). Each selected row gets its own
   D-000 cycle and its own commit. Recording, superseding,
   or overturning one of those three rows does not
   record, supersede, or overturn either other row.
...
4. **Required pins on each later owner-recording entry.**
   ...
   - candidate path and sha256; both verdict paths and
     sha256s; the file-08 row being recorded, if the
     candidate has one (for 106/109/113, exactly one of
     those three; for the Windows platform set, none —
     it is not a file-08 row per D-096); the owner
     role(s) being recorded; preview scope; Route A
     remainder if any;
...
5. **What a later owner-recording may do.** Record the
   ...
   explicit deferral for the roles of the named row, or
   for the named platform set where the candidate is
   not a file-08 row (Windows). For 106/109/113, the
   selected row only. It may not mark any row
   `SATISFIED`. It may not edit file 08. SATISFIED, if
   ever, is a **separate** later D-000-reviewed MF-6.
   Owner-recording precedes that MF-6 and is not it.
   It may not authorize `docs/v2/implementation/`. It
   may not apply a V1 successor, move the freeze, or
   move the claim register. It may not bundle DR-106,
   DR-109, and DR-113 into one owner-recording.
...
- Bundle DR-106, DR-109, and DR-113 in one
  owner-recording. Rejected: ADV-D100-01.
- Drop the Windows candidate to avoid the row-key.
  Rejected: D100-T2-MF-1; D-096 recorded that set.
...
| D-100 turn-1 subject | `aaf3dea0ce4736c09eb63e45b95ba6556417632caa79f8594f93b00bf5ede7d0` |
| D-100 turn-2 subject | `13d7e9082c42244a533a960d78cd910b4f7fb068d5e843c91b087fc53d8bffba` |
| D-100 Claude 2 turn 2 | `b9106f5918a10196cf03c9fbc6de29adde83ad48bc7a0f4d6e0346801e0ac092` |
| D-100 Codex turn 2 | `da4c5136d2a194478708f7a4f55b79b10bceab7b0ca2bd92d0dd65803648e834` |
