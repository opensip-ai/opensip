#!/usr/bin/env python3
"""Exact architecture conformance checker for retention-tiers.v19.json.
...
STARTUP_REFUSAL = (
    "RT19-CHECKER-UNSUPPORTED-INVOCATION: use python3 -I -B "
    "check-retention-custody-v19.py"
)
...
BINDING = "retention-tiers.v19.json"
RT19_STATUS = "CANDIDATE-NOT-APPLIED/AWAITING-INDEPENDENT-REVIEW"
...
RT18 = "retention-tiers.v18.json"
RT18_CHECKER = "check-retention-custody-v18.py"
RT18_ADJUDICATION = "retention-tiers.v18.adjudication-v17-review-response.json"
RT18_REVIEW = "retention-tiers.v18.review-independent-prefreeze.json"
ARCHITECTURE_PLAN = "ARCHITECTURE-TO-IMPLEMENTATION-PLAN.md"

PINS: dict[str, str] = {
    RT18: "c6fdb07bba36f67050362883109a25c1d9ac590d7c31b5384f9bf0c7889ffb14",
    RT18_CHECKER: "c59c58d4bf3ca21b851d09f97dc499886e5c207a50d7f7a632dc2795ed180b62",
    RT18_ADJUDICATION: "9f37c0909d3013a3916375db5cf519241f7bf69bcc11848a8504fa2c9ce96279",
    RT18_REVIEW: "54c1cc03b265653fdb582ed03a685fedf0dabd7f561bbf03b9f5e27af546f4a6",
...
ATOMIC_RUNTIME_TYPES = (
    ...
    range, slice, types.CodeType, type(Ellipsis), type(NotImplemented), object,
    type(REF_RE),
)
...
class FrozenAuthority:
    ...
    def freeze(self) -> None:
        ...
        for name in (RT18, RT18_ADJUDICATION, RT18_REVIEW,
    ...
    def _validate_reviews(self) -> None:
        ...
        rt17_review = self.parsed[RT18_REVIEW]
        ...
        if rt17_review.get("decision") != "REJECT" or \
                ...
                verdict17.get("blockingFindingCount") != 2 or \
                finding_ids17 != [
                    "RT18-PF-01-CLOSED-GRAPH-DENOMINATOR-INCOMPLETE",
                    "RT18-PF-02-IDENTITY-DEDUP-UNSAFE-UNDER-ID-REUSE",
                ]:
            raise AuthorityError("RT18 exact two-blocker review binding drift")
        ...
        for row in subject_rows17:
            if not isinstance(row, dict) or \
                ...
                raise AuthorityError("RT18 review subject window drift")
            subjects17[row.get("path")] = row.get("sha256AtStart")
        if subjects17 != {
                RT18: PINS[RT18], RT18_CHECKER: PINS[RT18_CHECKER],
                RT18_ADJUDICATION: PINS[RT18_ADJUDICATION]}:
            raise AuthorityError("RT18 review exact subjects drift")
        if rt17_review.get("exactByteReviewBasis", {}).get(
            ...
            raise AuthorityError("RT18 review input window drift")
        transitive18 = rt17_review.get("exactByteReviewBasis", {}).get(
            "transitiveWindow", {})
        if transitive18.get("inputCount") != 55 or \
                transitive18.get("retainedRT17PinCount") != 51 or \
                transitive18.get("individualPinCorruptionsRejectedBeforeCallback") != "55/55" or \
                transitive18.get("authorityCallbacksAfterCorruptPin") != 0 or \
                transitive18.get("physicalAliasRejected") is not True or \
                transitive18.get("allEndHashesRecomputed") is not True or \
                transitive18.get("endHashErrors") != [] or \
                transitive18.get("endIdentityErrors") != []:
            raise AuthorityError("RT18 retained 55-pin pass binding drift")
...
def expected_candidate(authority: FrozenAuthority) -> dict[str, Any]:
    candidate = copy.deepcopy(authority.parsed[RT18])
    candidate["version"] = 19
    candidate["status"] = RT19_STATUS
    candidate["supersedesAsArchitectureCandidate"] = RT18
    candidate["authority"] = {
        "candidateState": "NOT-APPLIED",
        "authorityClaim": "NONE",
        "semanticSourceRule": "The exact RT18 semantic object is inherited only for its independently retained passing evidence: exact 55-pin authority checks, EP8 API/receipt commitments, real D9 v1.10 dataclass semantics, current concrete return graph, semantic data/reducers, unchanged 7/2 and 5/3 raw-byte fixtures, nine leases, 13/7 root delta, and executed D9 consumers. Its two rejected graph-quarantine claims are replaced exactly by this successor.",
    ...
    ownership["rootDelta"] = {
        "predecessor": RT18, "predecessorRootCount": 20,
        ...
        "equation": "13 independently reviewed unchanged roots + 7 metadata/verifier-only roots = 20; status, semanticLeaseProtocolV3, and every semantic/data/fixture root are exact RT18 bytes as parsed data",
    }

    candidate["invariants"] = copy.deepcopy(authority.parsed[RT18]["invariants"]) + [
        "RT19-25: the retained RT18 55-input window is replaced lineage-for-lineage with exact RT18 artifact/checker/adjudication/review while remaining exactly 55 unique alias-collapsed, read-once, hash-bound, end-stat-checked inputs; all 55 single-pin corruptions and physical aliases refuse before callback",
        f"RT19-26: a closed exact ordered {len(GRAPH_EDGE_REGISTRY)}-edge registry plus exact ordered {len(GRAPH_RUNTIME_KIND_REGISTRY)}-kind disposition registry and exact scan-local class-identity declaration pass cover the retained 35 cases and seven missing class/frame/suspended-execution edges; every undeclared non-atomic runtime kind emits RT19-GRAPH-UNKNOWN-RUNTIME-TYPE",
        "RT19-27: the exact 43-case independent denominator passes both as synthetic roots and after each attack is injected into a returned operation function custom-attribute map beneath the actual VerifiedSemanticRTSnapshotV2 scan root; every new declaration has an independent deletion refusal",
        "RT19-28: cycle deduplication retains a strong id-to-object reference for every expanded identity until the complete scan ends; the nested-class mappingproxy id-reuse probe reaches its sentinel 20/20 as a helper root and 20/20 beneath the actual snapshot root",
        "RT19-29: graph traversal remains depth/node bounded and contains mapping/iterator/property/attribute errors, registry drift, unknown runtime kinds, forbidden identities and impossible live-id collisions as named findings without traceback escape",
    ]
    candidate["retainedResiduals"] = [
        "RT18 is independently REJECTED for exactly RT18-PF-01-CLOSED-GRAPH-DENOMINATOR-INCOMPLETE and RT18-PF-02-IDENTITY-DEDUP-UNSAFE-UNDER-ID-REUSE. Its retained passes for exact 55-pin corruption/alias refusal, EP8 nine-API commitments, real D9 v1.10 dataclass, current concrete snapshot, 7/2 and 5/3 byte fixtures, nine leases, D9 6/16/10 and the 13/7 root delta are inherited without semantic change; RT19 itself remains unreviewed and NOT-APPLIED.",
    ...
    api["constructor"]["signature"] = \
        "validate_semantic_rt_snapshot_v4(FrozenDependencySnapshotV5, VerifiedD9SnapshotV1) -> VerifiedSemanticRTSnapshotV2"
    ...
    api["returnedSnapshot"] = {
        ...
        "retainsExactly": [
            "snapshotId",
            "deep-immutable verified RT19 candidate data projection",
        ...
        "objectGraphRule": "Before return, a depth/node-bounded traversal validates the exact ordered edge and runtime-kind registries; retains every expanded object strongly in an id-to-object custody table until scan completion; covers the retained mapping/container/memoryview/callable/module/cell/object/class/descriptor edges plus class metaclass, frame globals/locals/back, and generator/coroutine/async-generator frames; and rejects every unknown non-atomic runtime type. Every one of the exact 43 cases must pass directly and after injection beneath an actual returned operation function with VerifiedSemanticRTSnapshotV2 as root. Forbidden identity, registry drift, traversal exception, impossible live-id collision, or budget exhaustion is a named refusal without traceback.",
    ...
    api["predecessorRejectionDisposition"] = {
        "reviewArtifact": RT18_REVIEW,
        "reviewSha256": PINS[RT18_REVIEW],
        "decision": "REJECT", "blockingFindingCount": 2,
        "repairedExactly": [
            "RT18-PF-01-CLOSED-GRAPH-DENOMINATOR-INCOMPLETE",
            "RT18-PF-02-IDENTITY-DEDUP-UNSAFE-UNDER-ID-REUSE",
        ],
    ...
    api["authenticatedLoaderBindingV5"] = {
        ...
        "graphQuarantine": {
            ...
            "registryMode": "CLOSED/EXACT-ORDERED/NO-UNKNOWN-EDGE-OR-RUNTIME-KIND",
            ...
            "namedRefusalCodes": [
                "RT19-GRAPH-FORBIDDEN-IDENTITY",
                "RT19-GRAPH-REGISTRY-MISSING",
                "RT19-GRAPH-REGISTRY-DUPLICATE",
                "RT19-GRAPH-REGISTRY-ORDER",
                "RT19-GRAPH-UNKNOWN-EDGE-TYPE",
                "RT19-GRAPH-RUNTIME-REGISTRY-MISSING",
                "RT19-GRAPH-RUNTIME-REGISTRY-DUPLICATE",
                "RT19-GRAPH-RUNTIME-REGISTRY-ORDER",
                "RT19-GRAPH-RUNTIME-REGISTRY-UNKNOWN",
                "RT19-GRAPH-UNKNOWN-RUNTIME-TYPE",
                "RT19-GRAPH-IDENTITY-CUSTODY-COLLISION",
                "RT19-GRAPH-TRAVERSAL-ERROR",
                "RT19-GRAPH-NODE-BUDGET",
                "RT19-GRAPH-DEPTH-BUDGET",
            ],
            "activeAttackFixtureIds": [
                "RT19-GRAPH-CLASS-DICTIONARY",
                "RT19-GRAPH-NAME-MANGLED-SLOT",
                "RT19-GRAPH-INHERITED-NAME-MANGLED-SLOT",
                "RT19-GRAPH-MEMORYVIEW-BACKING-BUFFER",
                "RT19-GRAPH-RAISING-CUSTOM-MAPPING",
                "RT19-GRAPH-RAISING-ITERATOR",
                "RT19-GRAPH-RAISING-PROPERTY",
                "RT19-GRAPH-CYCLE",
                "RT19-GRAPH-NODE-BUDGET",
                "RT19-GRAPH-DEPTH-BUDGET",
                "RT19-GRAPH-CLASS-METACLASS",
                "RT19-GRAPH-FRAME-GLOBALS",
                "RT19-GRAPH-FRAME-LOCALS",
                "RT19-GRAPH-FRAME-BACK",
                "RT19-GRAPH-GENERATOR-FRAME",
                "RT19-GRAPH-COROUTINE-FRAME",
                "RT19-GRAPH-ASYNC-GENERATOR-FRAME",
                "RT19-GRAPH-UNKNOWN-RUNTIME-TYPE",
                "RT19-GRAPH-MAPPINGPROXY-ID-REUSE",
            ],
            "independentDenominator": {
                "caseCount": 43,
                "syntheticRootExpected": "43/43",
                "actualSnapshotRootExpected": "43/43",
            ...
            "failureRule": "any reachable forbidden identity, edge/runtime registry drift, unknown non-atomic runtime type, traversal exception, impossible live-id collision, or node/depth exhaustion produces a named controlled finding and refuses before return; no traceback or exception object is returned",
        },
...
def _semantic_findings(candidate: dict[str, Any], expected: dict[str, Any],
    ...
    findings: list[str] = []
    rt13 = authority.parsed[RT13]
    rt17 = authority.parsed[RT18]
    if list(candidate) != ROOT_ORDER:
        findings.append("RT19-ROOT-ORDER: exact 20-root order drifted")
    unchanged = [
    ...
    for key in unchanged:
        ...
        if difference:
            findings.append(f"RT19-RT18-NARROW-PASS-PRESERVATION: {difference}")
    ...
    if candidate.get("status") != RT19_STATUS:
        findings.append("RT19-STATUS: candidate/review boundary drifted")
...
def _graph_registry_findings(
    ...
    try:
        ...
        return ["RT19-GRAPH-TRAVERSAL-ERROR: path=$ edge=edge-registry "
    ...
    for edge_id in ids:
        if edge_id in seen_ids:
            findings.append(
                f"RT19-GRAPH-REGISTRY-DUPLICATE: edge={edge_id}")
        seen_ids.add(edge_id)
    for edge_id in GRAPH_EDGE_IDS:
        if edge_id not in seen_ids:
            findings.append(f"RT19-GRAPH-REGISTRY-MISSING: edge={edge_id}")
    for edge_id in ids:
        if edge_id not in GRAPH_EDGE_IDS:
            findings.append(
                f"RT19-GRAPH-UNKNOWN-EDGE-TYPE: edge={edge_id}")
    if not findings and tuple(ids) != GRAPH_EDGE_IDS:
        findings.append("RT19-GRAPH-REGISTRY-ORDER: exact order drifted")
    return findings
...
def _runtime_registry_findings(
    ...
    try:
        ...
        return ["RT19-GRAPH-TRAVERSAL-ERROR: path=$ "
    ...
    for kind_id in ids:
        if kind_id in seen:
            findings.append(
                f"RT19-GRAPH-RUNTIME-REGISTRY-DUPLICATE: kind={kind_id}")
        seen.add(kind_id)
    for kind_id in GRAPH_RUNTIME_KIND_IDS:
        if kind_id not in seen:
            findings.append(
                f"RT19-GRAPH-RUNTIME-REGISTRY-MISSING: kind={kind_id}")
    for kind_id in ids:
        if kind_id not in GRAPH_RUNTIME_KIND_IDS:
            findings.append(
                f"RT19-GRAPH-RUNTIME-REGISTRY-UNKNOWN: kind={kind_id}")
    if not findings and tuple(ids) != GRAPH_RUNTIME_KIND_IDS:
        findings.append(
            "RT19-GRAPH-RUNTIME-REGISTRY-ORDER: exact order drifted")
    return findings


def _graph_error(path: str, edge: str, exc: BaseException) -> str:
    return (f"RT19-GRAPH-TRAVERSAL-ERROR: path={path} edge={edge} "
            f"exception={type(exc).__name__}")
...
def _predeclared_runtime_types(
    ...
    while queue and len(expanded) <= max_nodes:
        ...
        if kind is None or kind in ("atomic-exact", "opaque-runtime-exact") or \
                depth >= max_depth:
            continue
        try:
...
def _forbidden_identity_paths(
    ...
    if type(max_nodes) is not int or max_nodes < 1:
        return {"forbiddenHits": [], "visited": 0,
                "findings": [f"RT19-GRAPH-NODE-BUDGET: invalid={max_nodes}"],
                "clean": False}
    if type(max_depth) is not int or max_depth < 1:
        return {"forbiddenHits": [], "visited": 0,
                "findings": [f"RT19-GRAPH-DEPTH-BUDGET: invalid={max_depth}"],
                "clean": False}
    queue: deque[tuple[str, Any, int]] = deque([("$", root, 0)])
    ...
    while queue:
        ...
        if len(expanded) > max_nodes:
            findings.append(
                f"RT19-GRAPH-NODE-BUDGET: path={path} limit={max_nodes}")
            break
        try:
        ...
        if runtime_kind is None:
            continue
        if runtime_kind == "atomic-exact":
            continue
        if depth >= max_depth:
            findings.append(
                f"RT19-GRAPH-DEPTH-BUDGET: path={path} limit={max_depth}")
            continue
        try:
...
def _quarantine_edge_selftest(
    ...
    sentinel = object()
    buffer_sentinel = b"rt19-memoryview-backing-sentinel"
    ...
    selftest_admitted_types = (
        ...
        RaisingProperty, WrappedDescriptors,
    )
    ...
    exec(compile(
        ...
        "<rt19-frame-globals>", "exec"), frame_globals_namespace)
    ...
    generator_attack = (_item for _item in ())
    ...
    attack_attribute = "__rt19_actual_root_attack__"
    ...
    def actual_root_scan(
        ...
        if snapshot is None or not isinstance(operation, types.FunctionType) or \
            ...
            return {"forbiddenHits": [], "visited": 0,
                    "findings": ["RT19-GRAPH-ACTUAL-ROOT-INJECTION-FAILED"],
                    "clean": False}
    ...
    record("unknown-type", any(
        row.startswith("RT19-GRAPH-UNKNOWN-RUNTIME-TYPE:")
    ...
    try:
        async_close = async_generator_attack.aclose()
    ...
    record("raising-custom-mapping-controlled", any(
        row.startswith("RT19-GRAPH-TRAVERSAL-ERROR:") and
    ...
    for index, row in enumerate(GRAPH_EDGE_REGISTRY):
        ...
        wanted = f"RT19-GRAPH-REGISTRY-MISSING: edge={row['id']}"
    ...
    record("registry-unknown-controlled", any(
        row == "RT19-GRAPH-UNKNOWN-EDGE-TYPE: edge=hostile-unknown-edge"
    ...
    record("registry-duplicate-controlled", any(
        row.startswith("RT19-GRAPH-REGISTRY-DUPLICATE:")
    ...
    record("registry-order-controlled", any(
        row.startswith("RT19-GRAPH-REGISTRY-ORDER:")
...
def _snapshot_quarantine_findings(
    ...
    count = scan["visited"]
    findings = [
        f"RT19-GRAPH-FORBIDDEN-IDENTITY: path={row['path']} reaches="
    ...
    if tuple(receipt) != (
            ...
            "fixtureProjectionCommitment", "results", "resultsCommitment",
            "receiptCommitment"):
        findings.append("RT19-EP8-RECEIPT: closed ordered fields drifted")
    if list(receipt.get("requiredCheckerApi", ())) != \
            list(EXPECTED_EP8_REQUIRED_API):
        findings.append("RT19-EP8-RECEIPT: exact nine-name authority drifted")
    expected_counts = {name: (2 if name == "encode_semantic_object_binding" else 1)
                       for name in EXPECTED_EP8_REQUIRED_API}
    if dict(receipt.get("exactCallCounts", {})) != expected_counts:
        findings.append("RT19-EP8-RECEIPT: exact invocation counts drifted")
    try:
        goldens = snapshot.candidate["verifiedSemanticRtApiContract"][
            "authenticatedLoaderBindingV5"]["ep8Closure"]["resultGoldens"]
        if receipt["resultsCommitment"] != goldens["resultsCommitment"] or \
                ...
                snapshot.fixture_projection["projectionCommitment"]:
            findings.append("RT19-EP8-RECEIPT: executable commitment join drifted")
    except (KeyError, TypeError):
        findings.append("RT19-EP8-RECEIPT: executable commitment join absent")
    ...
    if any(callable(value) for _path, value in receipt_children):
        findings.append("RT19-EP8-RECEIPT: direct callable retained")
    if set(snapshot.operations) != {
            "derive_operational_custody_projection_v1",
            "validate_raw_object_resolution_v1", "reduce_semantic_lease_v3",
            "is_retention_local_unavailable_v1"}:
        findings.append("RT19-SNAPSHOT-SURFACE: operation allowlist drifted")
    if count < 200:
        findings.append("RT19-SNAPSHOT-GRAPH-QUARANTINE: traversal was incomplete")
    return findings
...
def _candidate_mutations() -> list[Mutation]:
    ...
    return [
        ...
        ("graph edge omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["graphQuarantine"]["edgeRegistry"].pop()),
        ("runtime kind omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["graphQuarantine"]["runtimeKindRegistry"].pop()),
        ("identity custody mode", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["graphQuarantine"]["identityCustody"].__setitem__("mode", "BARE-ID-SET")),
        ("EP8 required API omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["ep8Closure"]["requiredCheckerApi"].pop()),
        ("EP8 forbidden API extra", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["ep8Closure"]["requiredCheckerApi"].append("check")),
        ("EP8 call count", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["ep8Closure"]["exactCallCounts"].__setitem__("validate_bundle", 0)),
        ("EP8 receipt commitment", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["ep8Closure"]["resultGoldens"].__setitem__("receiptCommitment", "sha256:" + "0" * 64)),
        ("fixture projection commitment", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["ep8Closure"]["resultGoldens"].__setitem__("fixtureProjectionCommitment", "sha256:" + "0" * 64)),
        ("v1.10 real dataclass", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["temporaryVerifiedModuleExecutions"][0].__setitem__("isDataclass", False)),
        ("v1.10 temporary cleanup", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["temporaryVerifiedModuleExecutions"][0].__setitem__("restoredToAbsent", False)),
        ("v1.10 decorator interposition", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV5"]["d9Interpositions"].append({"source": "check-d9-v1.10.py", "kind": "dataclass-decorator"})),
...
def selftest(candidate: dict[str, Any], candidate_source: bytes,
    ...
    if candidate_source != pretty(expected) or candidate_source != pretty(candidate):
        print("REFUSING to self-test: base is not exact canonical RT19 candidate")
        return 1
    ...
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
          "43/43 synthetic + 43/43 actual-snapshot-root denominator cases")
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
          "20/20 helper + 20/20 actual-root mappingproxy id-reuse probes")
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
          "7/7 successor edge deletions + "
          f"{len(GRAPH_RUNTIME_KIND_REGISTRY)}/"
          f"{len(GRAPH_RUNTIME_KIND_REGISTRY)} runtime-kind deletions controlled")
    ...
    print(
        ...
        "attack, budget, exception, and deletion checks passed; exact 43/43 "
        "synthetic and actual-root denominator plus 20/20 helper and actual-root "
        "identity-reuse probes passed"
    )
    return 0
...
def main(argv: list[str]) -> int:
    try:
        ...
        print(f"cannot freeze RT19 authority: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"cannot load RT19 candidate: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        findings.append(f"RT19-AUTHORITY-END: {exc}")
    if findings:
    ...
    raw = candidate["verifiedSemanticRtApiContract"][
        "rawObjectResolutionConformance"]["fixtureGoldens"]
    print(
        f"RT19 contract OK - {path.name}; {len(PINS)} frozen inputs read once; "
