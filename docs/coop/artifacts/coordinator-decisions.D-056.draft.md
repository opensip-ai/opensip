...
> of D-084.
> **Decision type:** RULE-GOVERNED register-content evaluation
> rule. Not a rider on any application. Not D-013 alternative
> (b): that alternative was coining `DESIGN-ACCEPTED`. This
> entry does not coin a token.
...
a harness at DR-G21. The DR-102 cell says SATISFIED waits on
that execution.
...
D-013 alternative (b) was coining `DESIGN-ACCEPTED`. D-006
turn-2 NOTE-03 said the closed status vocabulary stays closed
unless a dedicated decision coins a new token. This entry is
not that decision. SATISFIED remains the only
architecture-satisfaction token. This entry states how
SATISFIED is *evidenced* for a narrow eligible class so
condition 2 and condition 5 are not a deadlock.
...
row may use this rule only when **all** of the following are
true of that row at the moment of that later cycle:
...
2. Every remaining acceptance-evidence member is **only**
   ...
   **not** a remainder this rule may split.
...
4. A dedicated later D-000 cycle plus independent
   SATISFIED-GRADE review of *that row* accepts the split.
5. An MF-6 file-08 cell edit records SATISFIED. This entry is
   not that edit.
...
are **not** eligible for a later split under this rule until
the named design leftover is closed:

| Row | Why ineligible today |
|---|---|
| DR-103 | Fixture *authoring* plus independent review remain. Authoring is design work (clause 5 below), not a C4 remainder. |
| DR-104 | Negative-test corpus authoring remains. This rule is not that authoring. |
...
| DR-106 / 108 / 109 / 110 / 113 / 116 / 128 / 129 / 130 | Deferral limb of condition 2 (D-002 / D-010), not this rule. |

## Decision

1. **Evaluation rule for condition 2, architecture-preview
   scope.** For a row that meets Eligibility above:
   ...
   - File 08's SATISFIED legend is read, for those eligible
     rows, as: the independently reviewed design contract or
     the already-recorded D-000 decision, plus the named
...
2. **This entry marks no row SATISFIED.** Each later SATISFIED
   ...
   unlawful (DR-204).
3. **Does not amend D-001's five conditions.** Condition 2
   still requires SATISFIED or explicit deferral. This entry
   states how SATISFIED is evidenced for eligible rows so
   condition 2 and condition 5 are not a deadlock. After this
   entry, condition 2 remains NOT MET. Zero SATISFIED is
   added.
4. **Does not rewrite live cells.** Those rewrites are later
   MF-6 acts, one row at a time, after Eligibility (4)–(5).
   Today's cells stay authoritative.
...
6. **Deferred D-002 / D-010 items stay on the deferral limb.**
   This rule is not a deferral and does not replace those
...
Total before any dependent SATISFIED re-record. After one
lands, overturn also requires that re-record's supersession.
Overturn: C-D056.
