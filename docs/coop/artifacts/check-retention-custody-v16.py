#!/usr/bin/env python3
"""Exact architecture conformance checker for retention-tiers.v16.json.
...
STARTUP_REFUSAL = (
    "RT16-CHECKER-UNSUPPORTED-INVOCATION: use python3 -I -B "
    "check-retention-custody-v16.py"
)
...
BINDING = "retention-tiers.v16.json"
RT16_STATUS = "CANDIDATE-NOT-APPLIED/AWAITING-INDEPENDENT-REVIEW"
...
D9_INTERPOSITION_SPECS: Mapping[str, Mapping[str, str]] = MappingProxyType({
    ...
    "check-d9-v1.10.py": MappingProxyType({
        "kind": "dataclass-decorator", "target": "Authority",
        "seam": "dataclass",
        "astSha256": "15c1f09a630af41694efa00494d87fae422572c7c7c7a2a4dcf09e7d4e5bb59b",
    }),
...
class VerifiedEP8Adapter:
    """Immutable allowlist over the authenticated EP8 source closure."""
    __slots__ = ("_api", "source_sha256", "closure_sha256", "_sealed")

    def __init__(self, module: types.ModuleType) -> None:
        names = (
            "_open_test_project_store", "resolve_stored_evaluation",
            "assert_store_continuity", "validate_bundle",
            "derive_raw_proof_requirements", "derive_dependency_edges",
            "derive_semantic_requirements", "check",
        )
        api = {name: getattr(module, name) for name in names}
        if any(not callable(value) for value in api.values()):
            raise AuthorityError("EP8 allowlisted adapter export is noncallable")
        object.__setattr__(self, "_api", MappingProxyType(api))
        object.__setattr__(self, "source_sha256", PINS[EP8_CHECKER])
        object.__setattr__(self, "closure_sha256", sha256(compact({
    ...
    def __setattr__(self, _name: str, _value: Any) -> None:
        raise AttributeError("VerifiedEP8Adapter is immutable")
...
class FrozenAuthority:
    def __init__(self) -> None:
        ...
        self.ep8_adapter: VerifiedEP8Adapter | None = None
    ...
    def freeze(self) -> None:
        ...
        for name in (RT15, RT15_ADJUDICATION, RT15_REVIEW, EP8, EP8_REVIEW,
    ...
    def _interposition_node(source: bytes, name: str,
        ...
        for node in ast.walk(tree):
            if kind in ("eager-function", "eager-loader-method") and \
                ...
                matches.append(node)
            elif kind == "dataclass-decorator" and \
                    isinstance(node, ast.ClassDef) and node.name == target and \
        ...
        line = node.decorator_list[0].lineno \
            if isinstance(node, ast.ClassDef) else node.lineno
        return node, line
    ...
    def _execute_interposed(self, name: str, replacement: Any) -> types.ModuleType:
        ...
        def passthrough_dataclass(*_args: Any, **_kwargs: Any) -> Callable[[Any], Any]:
        ...
        def tracer(frame: Any, event: str, _arg: Any) -> Any:
            ...
            elif kind == "dataclass-decorator":
                original = frame.f_globals.get("dataclass")
                if not callable(original):
                    raise AuthorityError(f"{name}: dataclass seam absent")
                frame.f_globals["dataclass"] = passthrough_dataclass
                restore = lambda: frame.f_globals.__setitem__("dataclass", original)
            else:
    ...
    def _build_ep8_closure(self) -> None:
        ...
        if self.read_counts != before:
            raise AuthorityError("EP8 delayed adapter changed outer read counters")
        self.modules.update(source_modules)
        self.ep8_adapter = VerifiedEP8Adapter(ep8)
    ...
    def _build_d9_closure(self) -> None:
        ...
        v110 = self._execute_interposed("check-d9-v1.10.py", None)
...
def expected_candidate(authority: FrozenAuthority) -> dict[str, Any]:
    candidate = copy.deepcopy(authority.parsed[RT15])
    candidate["version"] = 16
    candidate["status"] = RT16_STATUS
    candidate["supersedesAsArchitectureCandidate"] = RT15
    candidate["authority"] = {
        "candidateState": "NOT-APPLIED",
        "authorityClaim": "NONE",
        "semanticSourceRule": "The exact RT15 semantic object is inherited only under its independent narrow PASS for roots, RT13 projection, raw fixtures, and nine lease scenarios; its two rejected verifier claims are replaced by this successor's authenticated loaders.",
    ...
    ownership["rootDelta"] = {
        "predecessor": RT15, "predecessorRootCount": 20,
        ...
        "metadataOrVerifierOnlyChangedRootKeys": [
            "version", "supersedesAsArchitectureCandidate", "authority",
        ...
        "equation": "13 independently reviewed unchanged roots + 7 metadata/verifier-only roots = 20; no semantic, fixture, lease, storage, policy, assurance, seal, or identity root added or removed",
    }

    candidate["invariants"] = copy.deepcopy(authority.parsed[RT15]["invariants"]) + [
        "RT16-15: all 47 unique predecessor, EP8, RT13, RT14, and D9 closure inputs are alias-collapsed, read once, hash-bound before local source execution, and stat-checked at end",
        "RT16-16: exact EP8 delayed dependency seams resolve only authenticated buffers/modules, and all six frozen RT14 D9 consumer paths plus all sixteen rows execute through the exact D9 v1.13 API before D9 is quarantined",
    ]
    candidate["retainedResiduals"] = [
        "RT15 is independently REJECTED only for RT15-PF-01 and RT15-PF-02. Its narrow PASS for the 20-root semantic object, exact RT13 projection, byte-bearing fixture universes, and nine LeaseStateV3 scenarios is inherited without semantic change; RT16 itself remains unreviewed and NOT-APPLIED.",
        ...
        "Any downstream successor must pin exact independently reviewed RT16 artifact/checker bytes and an exact fixture snapshot/result, rebuild its enclosing values, and receive its own fresh independent review.",
    ...
    api["constructor"]["signature"] = \
        "validate_semantic_rt_snapshot_v1(FrozenDependencySnapshotV2, VerifiedD9SnapshotV1) -> VerifiedSemanticRTSnapshotV1"
    api["constructor"]["compatibilityCapabilityUse"] = \
        "VerifiedD9SnapshotV1 is the exact executed D9 v1.13 API. The verifier resolves and invokes all six frozen RT14 consumer paths and all sixteen rows through it, then proves it absent from the returned snapshot."
    api["frozenDependencySnapshotContract"] = {
        ...
        "requiredProperties": [
            ...
            "every retained local source compiles from its exact authenticated buffer under a synthetic filename and is never registered as a local sys.modules entry",
            ...
            "D9 eager bootstrap interposition is exact-AST-selected, declared, single-fire, restored exactly once, and refuses zero, duplicate, broadened, or drifted targets",
    ...
    api["returnedSnapshot"]["retainsExactly"][1] = \
        "verified RT16 candidate buffer/object"
    api["verifierTcb"] = {
        "soleInvocationPrefix": ["python3", "-I", "-B"],
        "requiredFlags": {"isolated": 1, "ignoreEnvironment": 1,
                          "noUserSite": 1, "dontWriteBytecode": True},
        ...
        "nestedProcesses": "FORBIDDEN", "localPathImports": "FORBIDDEN",
        "nestedContentRereads": "FORBIDDEN", "pyc": "FORBIDDEN",
        "sysModulesLocalRegistration": "FORBIDDEN",
        "sourceExecutionOrder": "all 47 local data/checker/review/plan buffers hash and review-bind before any verified local source buffer executes",
    }
    api["predecessorRejectionDisposition"] = {
        "reviewArtifact": RT15_REVIEW,
        "reviewSha256": PINS[RT15_REVIEW],
        ...
        "preservedNarrowPass": [
            "20-root semantic object", "exact RT13 semantic projection",
            "7/2 and 5/3 byte-bearing raw fixtures",
            "nine LeaseStateV3 scenarios",
        ],
    }
    api["authenticatedLoaderBindingV2"] = {
        "ep8Closure": {
            ...
            "delayedResolution": "immutable filename-to-buffer/module closures remain active for the lifetime of the allowlisted EP8 adapter",
        },
        "d9Interpositions": [
        ...
        "selectorRule": "Select one exact assignment or decorated class by target identity and exact attribute-free AST SHA-256; line numbers are derived evidence only and never selector authority.",
        "failureRule": "Refuse source hash drift, zero/multiple AST matches, AST digest drift, broadened seam, missing seam, non-single fire, skipped restoration, local sys.modules registration, filesystem fallback, or semantic-source substitution.",
    }
    return candidate
...
def _semantic_findings(candidate: dict[str, Any], expected: dict[str, Any],
    ...
    findings: list[str] = []
    rt13 = authority.parsed[RT13]
    rt15 = authority.parsed[RT15]
    if list(candidate) != ROOT_ORDER:
        findings.append("RT16-ROOT-ORDER: exact 20-root order drifted")
    unchanged = [
        "artifact", "status", "claimId", "dependencies",
        "semanticBasisProjection", "operationalCustodyProjectionContract",
        "semanticLeaseProtocolV3", "storageAndLineage", "custodyPolicy",
    ...
    for key in unchanged:
        difference = exact_recursive_equal(candidate.get(key), rt15[key], f"$.{key}")
        if difference:
            findings.append(f"RT16-RT16-NARROW-PASS-PRESERVATION: {difference}")
    ...
    if candidate.get("status") != RT16_STATUS:
        findings.append("RT16-STATUS: candidate/review boundary drifted")
...
class VerifiedSemanticRTSnapshotV1:
    """Opaque immutable capability returned only after verifier-only D9 use."""
    __slots__ = ("snapshot_id", "candidate", "rt13", "ep8", "operations",
                 "fixtures", "_sealed")

    def __init__(self, candidate: dict[str, Any], authority: FrozenAuthority) -> None:
        if authority.ep8_adapter is None:
            raise AuthorityError("EP8 adapter was not constructed")
        object.__setattr__(self, "snapshot_id", domain_digest(
            "opensip.verified-semantic-rt-snapshot.v1", pretty(candidate)))
        object.__setattr__(self, "candidate", copy.deepcopy(candidate))
        object.__setattr__(self, "rt13", copy.deepcopy(authority.parsed[RT13]))
        object.__setattr__(self, "ep8", authority.ep8_adapter)
        ...
        fixtures = candidate["verifiedSemanticRtApiContract"][
            "rawObjectResolutionConformance"]["fixtureGoldens"]
        object.__setattr__(self, "fixtures", copy.deepcopy(fixtures))
        object.__setattr__(self, "_sealed", True)

    def __setattr__(self, _name: str, _value: Any) -> None:
        raise AttributeError("VerifiedSemanticRTSnapshotV1 is immutable")


def _snapshot_quarantine_findings(
        snapshot: VerifiedSemanticRTSnapshotV1, authority: FrozenAuthority,
) -> list[str]:
    findings: list[str] = []
    forbidden = {
        id(authority.modules[name]) for name in (
            D9_CHECKER, "check-d9-v1.12.py", "check-d9-v1.11.py",
            "check-d9-v1.10.py", "check-d9-v1.8.py", "check-d9-v1.7.py",
            "check-d9.py",
        )
    }
    forbidden |= {
        id(authority.buffers[name]) for name in PINS
        if name.startswith("d9-") or name.startswith("check-d9") or
        name == D9_REVIEW
    }
    direct = [snapshot.candidate, snapshot.rt13, snapshot.ep8,
              snapshot.operations, snapshot.fixtures]
    if any(id(value) in forbidden for value in direct):
        findings.append("RT16-SNAPSHOT-D9-QUARANTINE: direct D9 object retained")
    for function in snapshot.ep8._api.values():
        globals_map = getattr(function, "__globals__", {})
        if any(id(value) in forbidden for value in globals_map.values()):
            findings.append("RT16-SNAPSHOT-D9-QUARANTINE: EP8 adapter reaches D9")
            break
    if set(snapshot.operations) != {
        ...
        findings.append("RT16-SNAPSHOT-SURFACE: operation allowlist drifted")
    return findings
...
def check_contract(candidate: Any, authority: FrozenAuthority,
    ...
    try:
        ...
        if not findings:
            snapshot = VerifiedSemanticRTSnapshotV1(candidate, authority)
...
def selftest(candidate: dict[str, Any], candidate_source: bytes,
    ...
    if candidate_source != pretty(expected) or candidate_source != pretty(candidate):
        print("REFUSING to self-test: base is not exact canonical RT16 candidate")
        return 1
...
def main(argv: list[str]) -> int:
    try:
        ...
        print(f"cannot freeze RT16 authority: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"cannot load RT16 candidate: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        findings.append(f"RT16-AUTHORITY-END: {exc}")
    if findings:
    ...
    raw = candidate["verifiedSemanticRtApiContract"][
        "rawObjectResolutionConformance"]["fixtureGoldens"]
    print(
        f"RT16 contract OK - {path.name}; {len(PINS)} frozen inputs read once; "
