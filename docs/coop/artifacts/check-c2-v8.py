#!/usr/bin/env python3
"""Retained executable checker for the C-2 plan/stage contract, v8.

WHY v8 EXISTS
-------------
`1.0 == 1` has now defeated five consecutive artifacts written to defeat it,
...
c2-plan-stage-schema.v7.review-independent.json, at
eaeb43b4a912da253518b88500757c852391e2f6846927defd5048f2bc5d0622, is a REJECT on
ONE blocking finding and ten non-blocking ones, and it is the narrowest
rejection in the chain.  The reviewer attacked the duplicate-key repair in both
directions and could not break it, hunted the whole bytes-versus-parse class and
found nothing outside the two declared lists, ran the 139-row matrix to
termination with 0 escapes and 0 COUNTER-ONLY rows, and swept 164 candidate
mutations with 0 admitted.  NONE OF THAT IS REBUILT HERE.  What is repaired is
the one thing none of it touched:

    c2-plan-stage-schema.v7.json
      change `theParseDefect/minimalReproduction/reproducedByThisLane` from
      `true` to `false` -- ONE TOKEN, ONE BYTE, no source modification
    python3 -I -B check-c2-v7.py <that file>   ->   exit 0, full green banner,
      including `148 integer leaves of THIS document bound ... with no unbound
      bucket`, over a document that now says this lane did NOT reproduce the one
      finding v7 exists to discharge.

THE PATTERN, WHICH MATTERS MORE THAN THE BYTE
---------------------------------------------
The v7 reviewer's sharpest sentence is not about booleans.  It is this:

    RES-C2V7-07 names STRINGS -- the third consecutive version whose residual
    points one type away from the live hazard.
    ...
    v7   RES-C2V7-07 names strings;              the live hazard is booleans

v6 was authored to escape exactly this at the COMPARISON layer: it stopped
patching operators one at a time and built a primitive, because a spelling
enumeration cannot close a class.  The same failure reappeared one level up, at
the ENUMERATION layer, where the totality that decides WHICH LEAVES ARE UNDER
THE PRIMITIVE was itself written as a list of types -- `_integer_leaf_steps` and
`_number_leaf_steps`, each naming the type it covered.  A totality that
enumerates which types it covers is a list of places, and the defect moves to a
place not on the list.

So v8 does not add a boolean walker.  `_number_leaf_steps` is DELETED, not
joined by a sibling.  The candidate totality is `jx_leaf_census`, which names
the two CONTAINER cases and lets EVERY other JSON type fall through to a leaf;
`jx_type_space` MEASURES, over a witness corpus required to realise every member
of JX_TYPES, which types the census reports as leaves and which it descends
through; JX_TYPES is required to be exactly the RFC 8259 value space under one
declared refinement of `number`; and LEAF_TYPE_RULES is required to equal that
MEASUREMENT rather than to assert its own coverage.  A leaf whose JSON type has
no disposition rule is a NAMED FINDING, not a silent pass.  There is no list
left for a future residual to point one type away from.

Three obligations cover every leaf, and the third is what closes the types whose
VALUE is genuinely narrative:

    VALUE       every leaf ruled BIND -- null, boolean, integer, number -- is
                bound to a value this run measured or to a byte of a
                hash-verified pinned input.  C2V8-UNBOUND otherwise.
    SHAPE       the PATH and the JSON TYPE of every NODE of this document,
                container nodes included, are hashed into one digest published
                in the document and bound to what this run measures.  Adding,
                removing, renaming or retyping any node is C2V8-SKELETON --
                including a string leaf, including an EMPTY array or object,
                which holds no leaf for any census to reach.
    POPULATION  the per-type leaf counts are published and bound, so this
                document's own account of what it contains cannot drift.

What is NOT closed is the TEXT of a narrative string leaf, and its boundary is
published as a measured number rather than described.  RES-C2V8-07.

WHAT WENT WRONG FIVE TIMES, AND WHERE THE LIST KEEPS MOVING TO
--------------------------------------------------------------
Every repair before v6 guarded CALL SITES.  v4 guarded the wire validator and
...
v6 fixed that and built the primitive -- and the defect moved BELOW it, to the
parser, which is not a place on any list either and is the one boundary every
layer inherits without choosing to.  v7 fixed THAT: the parse became a gate,
`jx_loads` is that gate, it lives INSIDE the portable primitive so an adopter
cannot take the comparison and leave the parse behind, and a STRUCTURAL scan of
the file's own tree requires every json.load/json.loads call to pass through it.

And the defect moved ABOVE both, to the ENUMERATION -- the thing that decides
WHICH LEAVES ARE PUT TO THE PRIMITIVE AT ALL.  v7 decided it with two
hand-written walkers, one per JSON type it had thought of, and a boolean leaf
was put to nothing.  That is the same list-of-places failure at a third level,
and it is why v8's answer is not a third walker:

    the comparison   v6   ONE primitive, not a list of operators
    the parse        v7   ONE gate, not a list of inputs
    the enumeration  v8   ONE walk that names no scalar type, whose coverage is
                          MEASURED against the JSON data model, not a list of
                          the types an author remembered

v8 keeps v6's and v7's design entirely -- the primitive, the parse gate, the
scan, the sweeps, the differentials and the mutation suite are all preserved --
and changes exactly one thing: the totality.
...
truthiness is emptiness and not numeric value.  That scope is RES-C2V8-09.
...
instead of a defect a reader has to discover by trying it.
  ...
  2. PARSE EVERY JSON INPUT WITH `jx_loads`, NOT ONLY THE ONE YOU CALL THE
     ...
     STATED IT LOOSELY (OBS-C2V7-03): copying the delimited region gives you the
     parse gate's AVAILABILITY and nothing more.  `json_load_sites`,
...
as RES-C2V8-04, and is not repaired here.
...
6. v8 does not re-run check-c2-v4.py --selftest, the predecessor's ~24k-case
   contract-root execution matrix.  Retained as RES-C2V8-03.
7. The type lock closes JSON TYPE.  A type-correct integer with a wrong VALUE at
   ...
   counter register.  Retained as RES-C2V8-05.
7b. A NARRATIVE STRING LEAF'S TEXT IS NOT BOUND.  Its path and its type are, and
   ...
   The size of that gap is a measured number in RES-C2V8-07 and in the banner,
...
10. L8 covers ONE class -- "the bytes on disk and the object every guard reads
   ...
   are their own reviewed bytes and are not v8's to change.  RES-C2V8-08.
11. L9 closes "a declared string that names something that exists NOWHERE".  It
   ...
   visible rather than described.  RES-C2V8-10.
12. Everything this run does NOT recompute is in RES-C2V8-11, in the artifact,
...
Usage: python3 -I -B artifacts/check-c2-v8.py [contract]  ·  --selftest
...
BINDING = "c2-plan-stage-schema.v8.json"
DECLARED_FLAGS = ("--selftest",)
DECLARED_EXIT_CODES = frozenset({"0", "1", "2", "3"})
HERE = pathlib.Path(__file__).resolve().parent
ARTIFACT_ID = "opensip.c2-plan-stage-schema.v8"
...
def jx_type_space() -> dict:
    ...
    for witness in JX_TYPE_WITNESSES:
        ...
        here = [row for row in jx_leaf_census({"probe": witness})
                if jx_equal(row[0], ["probe"])]
        if jx_int_in_range(len(here), 1, 1) and jx_equal(here[0][1], kind):
            scalars.append(kind)
        elif jx_int_in_range(len(here), 0, 0):
            containers.append(kind)
        else:
            escapes.append("the leaf census reports " + str(len(here)) + " leaf/leaves "
                           "at the position holding the JSON " + kind + " witness " +
                           repr(witness) + ", so that type is neither a leaf nor a "
                           "container and the census is not total over it")
    ...
    for kind in jx_sorted([item for item in containers if jx_in(item, scalars)]):
        escapes.append("the leaf census treats the JSON type " + repr(kind) + " as a "
                       "container for one witness and as a leaf for another, so its "
                       "totality is not well defined over that type")
    refined = []
...
def load_authority(directory: pathlib.Path = HERE) -> Authority:
    ...
    with redirect_stdout(sink):
        v4 = _execute_snapshot("opensip_c2v8_pinned_v4_checker", V4_CHECKER,
                               snapshots[V4_CHECKER], directory)
        v4._OWN_TREE_CACHE = ast.parse(snapshots[V4_CHECKER])
        v3_module = v4._execute_snapshot("opensip_c2v8_pinned_v3_checker", V3_CHECKER,
...
#
# What L8 does NOT cover is in the contract's L8 blindSpots and in RES-C2V8-08,
# RES-C2V8-12 and RES-C2V8-13: string-escape spellings; the parses performed
...
_PARSE_FINDING_IDS = {"duplicate-key": "C2V8-DUPKEY",
                      "non-rfc-constant": "C2V8-NONRFC",
                      "number-text": "C2V8-NUMBER-TEXT"}
...
def parse_problem_findings(problems, label) -> list:
    ...
    for problem in problems:
        ...
        prefix = jx_get(_PARSE_FINDING_IDS, kind, "C2V8-PARSE")
        findings.append(
...
# is that the scan is still syntactic: RES-C2V8-12.
...
def parse_depth_probe(depth=PARSE_DEPTH_PROBE_DEPTH) -> dict:
    ...
    if [item for item in findings
            if item.startswith("C2V8-DUPKEY:") and expected in item]:
        named += 1
    elif not escapes:
...
def parse_probe(label="probe") -> dict:
    ...
    for name, text, kind, position in JX_PARSE_CORPUS:
        ...
        if not kind:
            ...
            continue
        expected = jx_get(_PARSE_FINDING_IDS, kind, "C2V8-PARSE")
...
def parse_differential(authority) -> dict:
    ...
    for vector, document, needle, insert, position, note in PARSE_DIFFERENTIAL_VECTORS:
        ...
        hit = [item for item in findings
               if item.startswith("C2V8-DUPKEY:") and position in item]
        if hit:
            named += 1
        else:
...
def parse_integrity_findings(authority, tree) -> list:
    ...
    for site in scan["ungated"]:
        findings.append(
            "C2V8-PARSE-SCAN: line " + str(site["line"]) + " parses JSON without an "
    ...
    if not jx_int_in_range(scan["sites"], 1, 10 ** 6):
        findings.append("C2V8-PARSE-SCAN: this file appears to parse no JSON at all, so "
    ...
    if not jx_int_in_range(scan["declaredHazardSites"], 1, 10 ** 6):
        findings.append("C2V8-PARSE-SCAN: the operator space no longer demonstrates the "
                        "bare-parse hazard live, so the row that covers it proves "
                        "nothing")
    if not jx_int_in_range(scan["declaredPredecessorParses"], 1, 10 ** 6):
        findings.append("C2V8-PARSE-SCAN: this file no longer reproduces the "
    ...
    if jx_bind(decomposed, scan["sites"]) is not None:
        findings.append(
            "C2V8-PARSE-SCAN: the parse call sites decompose into " +
    ...
    if not jx_int_in_range(
            ...
            2, 10 ** 6):
        findings.append(
            "C2V8-PARSE-SCAN: the decoder-evasion detector reports no site in a probe "
    ...
    for site in scan["evasions"]:
        findings.append(
            "C2V8-PARSE-SCAN: line " + str(site["line"]) + " reaches the JSON decoder "
    ...
    findings.extend("C2V8-PARSE-DEPTH: " + item for item in depth["escapes"])
    if jx_bind(depth["named"], 1) is not None:
        findings.append("C2V8-PARSE-DEPTH: the duplicate key nested " +
    ...
    if jx_bind(depth["controls"], 1) is not None:
        findings.append("C2V8-PARSE-DEPTH: the control at depth " +
    ...
    if not jx_int_in_range(authority.pinned_number_tokens["tokens"], 1, 10 ** 9):
        findings.append("C2V8-PARSE-SCAN: the number-token scanner finds no token at "
    ...
    for name in jx_sorted(list(scan["predecessors"])):
        ...
        if not jx_int_in_range(entry["sites"], 1, 10 ** 6):
            findings.append("C2V8-PARSE-SCAN: the pinned " + name + " reports no JSON "
        ...
        if jx_in(name, list(PARSE_UNHOOKED_PREDECESSORS)):
            if jx_bind(entry["hooked"], 0) is not None:
                findings.append("C2V8-PARSE-SCAN: the pinned " + name + " now hooks " +
            ...
            findings.append("C2V8-PARSE-SCAN: the pinned " + name + " no longer hooks "
    ...
    findings.extend("C2V8-PARSE-PROBE: " + item for item in probe["escapes"])
    if not jx_int_in_range(probe["executedCases"], 1, 10 ** 6):
        findings.append("C2V8-PARSE-PROBE: the parse layer probed no case, so it is a "
                        "claim over an unobserved region")
    if jx_bind(probe["admissions"], 0) is not None:
        findings.append("C2V8-PARSE-PROBE: " + str(probe["admissions"]) + " byte/parse "
    ...
    findings.extend("C2V8-PARSE-DIFFERENTIAL: " + item for item in differential["escapes"])
    if jx_bind(differential["predecessorParseBlind"], differential["vectors"]) is not None:
        findings.append(
            "C2V8-PARSE-DIFFERENTIAL: " + str(differential["predecessorParseBlind"]) +
    ...
    if jx_bind(differential["successorNamedByPosition"],
        ...
        findings.append(
            "C2V8-PARSE-DIFFERENTIAL: " + str(differential["successorNamedByPosition"]) +
        ...
        try:
            if jx_equal(kind, "set"):
                ...
                if not jx_equal(current, op.get("from")):
                    findings.append(
                        "C2V8-DERIVATION: operation " + str(index) + " at " + path +
                ...
                if jx_has_at(parent, steps[-1]):
                    findings.append("C2V8-DERIVATION: operation " + str(index) + " adds " +
...
def project_to_v4_identity(effective, projection):
    ...
    if jx_type(fields) != "object" or not fields:
        return projected, ["C2V8-PROJECTION: v4InheritanceProjection.fields must be a "
                           "non-empty JSON object"]
    if not jx_equal(fields, dict(DECLARED_PROJECTION_FIELDS)):
        findings.append(
            "C2V8-PROJECTION: the declared projection is " + repr(fields) + "; this "
    ...
    for path in jx_sorted(list(fields)):
        try:
            ...
            findings.append("C2V8-PROJECTION: " + path + " does not resolve (" +
                            type(exc).__name__ + ")")
    drift = jx_sorted(_wire_diff_paths(effective, projected))
    declared = jx_sorted(list(fields))
    if not jx_equal(drift, declared):
        findings.append("C2V8-PROJECTION: the projection handed to the inherited oracle "
...
def measure(effective, authority):
    ...
    a trust relationship in RES-C2V8-04.
    """
    v4 = authority.v4
...
def census_comparison_findings(effective, measurement) -> list:
    ...
    for position in jx_sorted(list(positions)):
        ...
        if not present:
            findings.append("C2V8-CENSUS: " + position + " is measured by this run at " +
        ...
        if not jx_int(measured_value):
            findings.append("C2V8-INSTRUMENT: " + position + ": " + reason)
        elif not jx_int(published):
            findings.append("C2V8-TYPE: " + position + ": " + reason)
        else:
            findings.append("C2V8-CENSUS: " + position + ": " + reason)
    return findings
...
def register_findings(effective, measurement, authority) -> list:
    ...
    if jx_type(spec) != "object":
        findings.append("C2V8-REGISTER: the effective contract declares no "
    ...
    if not jx_equal(jx_sorted(jx_unique(declared_inherited)),
        ...
        findings.append(
            "C2V8-REGISTER: the inherited half of the register is " +
    ...
    if missing:
        findings.append("C2V8-REGISTER: " + str(len(missing)) + " published counter "
                        "position(s) are measured and compared but are not registered, "
                        "so a site could be added silently: " + repr(missing[:6]))
    if extra:
        findings.append("C2V8-REGISTER: " + str(len(extra)) + " registered position(s) "
    ...
    if count_reason is not None:
        findings.append("C2V8-REGISTER: censusCounterPositionCount: " + count_reason)
    ...
    if uncovered or not jx_int_in_range(len(covered), 1, 10 ** 9):
        findings.append(
            "C2V8-REGISTER: the per-comparison-site partition covers " +
    ...
    if "type" not in rule or "before" not in rule:
        findings.append("C2V8-REGISTER: the register does not state plainly that the "
...
# `_number_leaf_steps` is GONE.  It was v7's second type-named walker: v6's
...
# JSON data model.  `_integer_leaf_steps` survives BELOW, unchanged, because L2b
# and L6b genuinely are about integers: the type lock locks integer leaves
# against the predecessor and the sweep injects a float at each of them.  Those
# are scoped claims about a scoped surface, and they say so.
...
def document_type_findings(effective, locked, prefix="C2V8-TYPE") -> list:
    findings = []
    for steps in locked:
...
def document_type_probe(effective, locked, prefix="C2V8-TYPE") -> dict:
...
def document_type_lock_findings(base, effective, authority) -> list:
    ...
    for label, document in (("verified predecessor", base),
        ...
        if missing:
            findings.append(
                "C2V8-DOCLOCK: " + str(len(missing)) + " integer leaf/leaves of the " +
    ...
    findings.extend("C2V8-DOCLOCK: " + item for item in probe["escapes"])
    if not jx_int_in_range(probe["executedCases"], 1, 10 ** 9):
        findings.append("C2V8-DOCLOCK: the document-wide type lock probed no position, "
                        "so it is a claim over an unobserved region")
    if jx_bind(probe["admissions"], 0) is not None:
        findings.append("C2V8-DOCLOCK: " + str(probe["admissions"]) + " float "
    ...
    if jx_bind(probe["namedTypeRejections"], probe["executedCases"]) is not None:
        findings.append("C2V8-DOCLOCK: " + str(probe["namedTypeRejections"]) + " of " +
...
#               hash-verified pinned input.  An unbound one is C2V8-UNBOUND.
#   SHAPE       the PATH and the JSON TYPE of every NODE of the document,
#               container nodes included, are hashed into one digest published
#               in the document and bound to what this run measures.  Adding,
#               removing, renaming or retyping any node anywhere is
#               C2V8-SKELETON, whatever JSON type is involved and whether or not
...
# RES-C2V8-07 and its boundary is a measured number, not a description.
...
CANDIDATE_COUNTER_ALIASES = (
    ...
    (["theSuccessorDefect", "measuredThisRun", "v8RejectedByName"],
     "successorV8RejectedByName"),
    ...
    (["theParseDefect", "measuredThisRun", "v8NamedByPosition"],
    ...
    (["theEnumerationDefect", "measuredThisRun", "v8EnumeratedTheLeaf"],
...
def leaf_rule_findings(space) -> list:
    ...
    for kind in jx_sorted(jx_difference(list(JX_TYPES), ruled)):
        findings.append("C2V8-LEAFRULE: the JSON type " + repr(kind) + " has no leaf "
    ...
    for kind in jx_sorted(jx_difference(ruled, list(JX_TYPES))):
        findings.append("C2V8-LEAFRULE: the leaf disposition table rules on " +
    ...
    for kind, rule in LEAF_TYPE_RULES:
        if not jx_in(rule, list(LEAF_RULE_KINDS)):
            findings.append("C2V8-LEAFRULE: the JSON type " + repr(kind) + " is ruled " +
    ...
    if not jx_equal(declared_containers, jx_sorted(list(space["containerTypes"]))):
        findings.append("C2V8-LEAFRULE: the table calls " + repr(declared_containers) +
    ...
    if not jx_equal(declared_leaves, jx_sorted(list(space["scalarTypes"]))):
        findings.append("C2V8-LEAFRULE: the table gives leaf dispositions to " +
    ...
    for item in space["escapes"]:
        findings.append("C2V8-LEAFRULE: " + item)
    if not jx_int_in_range(space["types"], len(JX_TYPES), len(JX_TYPES)):
        findings.append("C2V8-LEAFRULE: the witness corpus realises " +
...
def document_skeleton(c) -> list:
    """`[path text, JSON type]` for EVERY node of the document, containers too.
    ...
    it describes.  A narrative string's TEXT is outside this (RES-C2V8-07 and
    ...
    so an added, removed, renamed or retyped node anywhere in the document is a
    named finding regardless of which JSON type it happens to be.  An empty
    array or an empty object holds no leaf at all and is invisible to any leaf
    census; this is the layer that sees it.
    """
...
def candidate_leaf_ledger(c, bound_paths) -> dict:
    ...
    for steps, kind in census:
        ...
        if rule is None:
            unruled += 1
            findings.append(
                "C2V8-LEAFRULE: the candidate publishes a leaf of JSON type " +
        ...
        unbound_count += 1
        findings.append(
            "C2V8-UNBOUND: the candidate publishes a JSON " + kind + " at " +
...
def candidate_bindings(c, live, authority, base, measurement):
    ...
    bind(["version"], "identity", 8, "this checker's declared successor identity")
    bind(["supersedes"], "identity", 7,
    ...
    for index, op in enumerate(operations):
        ...
        for sub in _integer_leaf_steps(op.get("from")):
            try:
                ...
                findings.append(
                    "C2V8-UNBOUND: derivedFrom/operations/" + str(index) + "/from/" +
        ...
        for sub in _integer_leaf_steps(op.get("value")):
            ...
            if expected is None:
                findings.append(
                    "C2V8-UNBOUND: the derivation writes a JSON integer into the "
    ...
    for key in jx_sorted(list(live)):
        bind(["v8MeasuredCounters", key], "measured", jx_get(live, key),
             "the value this run measured for " + key)
...
def _declared_text_bindings():
    ...
    return (
        ...
        (["checkerModeContract", "checker"], "check-c2-v8.py",
...
def _effective_expected(steps, live, positions):
    ...
    if jx_equal(list(steps[:1]), ["supersedes"]):
        return 7, "this checker's declared successor identity"
...
def candidate_lock_findings(c, live, authority, base, measurement) -> list:
    ...
    if not jx_int_in_range(len(leaf_rule_findings(LEAF_RULE_DETECTOR_PROBE)),
                           1, 10 ** 6):
        findings.append(
            "C2V8-LEAFRULE: the leaf disposition check does not fire on a measured type "
    ...
    for kind in jx_sorted(jx_difference(list(JX_TYPES), produced)):
        findings.append(
            "C2V8-CANDLOCK: the hostile spelling table declares no witness of the JSON "
    ...
    for steps in jx_sorted(jx_unique(duplicates)):
        findings.append("C2V8-UNBOUND: " + _steps_text(steps) + " is bound twice, so "
    ...
    for steps in jx_sorted(jx_difference(bound_paths, census_paths)):
        findings.append(
            "C2V8-UNBOUND: this run binds " + _steps_text(steps) + " but the candidate "
    ...
    if jx_bind(len(skeleton), ledger["nodes"]) is not None:
        findings.append(
            "C2V8-SKELETON: the skeleton describes " + str(len(skeleton)) + " node(s) "
    ...
    if not jx_int_in_range(len(containers), 1, 10 ** 9):
        findings.append(
            "C2V8-SKELETON: the skeleton describes no container node at all, so an "
    ...
    try:
        ...
        findings.append("C2V8-SKELETON: the candidate publishes no "
    ...
    if published_digest is not None:
        ...
        if reason is not None:
            findings.append(
                "C2V8-SKELETON: documentSkeleton/sha256: " + reason + "; the skeleton "
    ...
    for record in jx_sorted_by(bindings, "steps"):
        ...
        if reason is None:
            continue
        prefix = "C2V8-CANDIDATE" if jx_same_type(published, expected) else "C2V8-TYPE"
    ...
    findings.extend("C2V8-TOTALITY: " + item for item in totality["escapes"])
    if not jx_int_in_range(totality["executedCases"], 1, 10 ** 9):
        findings.append("C2V8-TOTALITY: the candidate totality probed no position, so "
    ...
    if jx_bind(totality["typesCovered"], len(JX_TYPES)) is not None:
        findings.append("C2V8-TOTALITY: the totality probe covered " +
    ...
    if jx_bind(totality["admissions"], 0) is not None:
        findings.append("C2V8-TOTALITY: " + str(totality["admissions"]) + " injected "
                        "leaf/leaves that no layer binds were ADMITTED")
    probe = candidate_type_probe(c, bindings)
    authority.candidate_lock = probe
    findings.extend("C2V8-CANDLOCK: " + item for item in probe["escapes"])
    if not jx_int_in_range(probe["executedCases"], 1, 10 ** 9):
        findings.append("C2V8-CANDLOCK: the candidate lock probed no position, so it is "
                        "a claim over an unobserved region")
    if jx_bind(probe["admissions"], 0) is not None:
        findings.append("C2V8-CANDLOCK: " + str(probe["admissions"]) + " hostile "
...
CANDIDATE_TOTALITY_TARGETS = ([], ["theDefect"], ["v8MeasuredCounters"],
                              ["documentSkeleton"])
...
def candidate_totality_probe(c, bound_paths, clean_digest) -> dict:
    ...
    for kind, rule in LEAF_TYPE_RULES:
        jx_put(rules, kind, rule)
    cases = named = admitted = skeleton_only = 0
    escapes = []
    ...
    for target in CANDIDATE_TOTALITY_TARGETS:
        ...
        for label, _kind, value, sub in CANDIDATE_TOTALITY_WITNESSES:
            ...
            try:
                ...
                # the measured size of RES-C2V8-07.
                ledger = candidate_leaf_ledger(c, bound_paths)
    ...
    return {"executedCases": cases, "namedRejections": named, "admissions": admitted,
            "namedBySkeletonOnly": skeleton_only, "typesCovered": len(covered),
            "escapes": escapes}
...
def bare_truthiness_sites(tree) -> list:
    ...
    the honest scope is in RES-C2V8-09 rather than in a guard that would have to
...
def scan_findings(authority, self_tree=None) -> list:
    ...
    self_scan = wire_comparison_scan(tree, "check-c2-v8.py")
    ...
    for site in self_scan["sites"]:
        findings.append(
            "C2V8-SCAN: " + site["function"] + " line " + str(site["line"]) + " (" +
    ...
    if not jx_int_in_range(self_scan["gateCallSites"], 8, 10 ** 6):
        findings.append("C2V8-SCAN: only " + str(self_scan["gateCallSites"]) +
    ...
    for key, allowed, why in (
        ...
        if jx_bind(jx_get(primitives, key), allowed) is not None:
            findings.append("C2V8-SCAN: this file carries " +
    ...
    if jx_bind(self_scan["functionLikeNodes"], len(walked) - len(gated)) is not None:
        findings.append(
            "C2V8-SCAN-BLIND: the scan reports " + str(self_scan["functionLikeNodes"]) +
    ...
    if not jx_int_in_range(len(adjudicated), 1, 10 ** 6):
        findings.append(
            "C2V8-SCAN-BLIND: the pinned adjudication yields no comparison line at all, "
    ...
    if unseen:
        findings.append(
            "C2V8-SCAN-BLIND: run over the pinned predecessor this scan does NOT report "
    ...
    if not jx_in(V5_DEFECT_LINE, v5_found):
        findings.append(
            "C2V8-SCAN-BLIND: run over the pinned check-c2-v5.py this scan does NOT "
    ...
    if not jx_int_in_range(predecessor_scan["ungatedComputedOperandComparisons"],
                           1, 10 ** 6):
        findings.append("C2V8-SCAN-BLIND: the scan reports no computed-operand "
    ...
    if not jx_int_in_range(jx_get(truthiness, V6_CHECKER), 1, 10 ** 6):
        findings.append("C2V8-SCAN-BLIND: the bare-truthiness model reports nothing at "
...
# somewhere in the file.  That residual is RES-C2V8-10.
# =============================================================================
...
def declared_string_findings(c, authority, tree) -> list:
    ...
    findings = ["C2V8-DECLARED-STRING: " + item for item in result["escapes"]]
    if not jx_int_in_range(result["rows"], 1, 10 ** 6):
        findings.append("C2V8-DECLARED-STRING: there are no retained assertion rows to "
                        "check, so this layer is a statement about an empty set")
    if not jx_int_in_range(result["namesChecked"], 1, 10 ** 6):
        findings.append("C2V8-DECLARED-STRING: no primitive name is named by the "
                        "docstring or the candidate, so the adoption-instruction check "
                        "detects nothing")
    if not jx_int_in_range(result["atEmitSite"], 1, 10 ** 6):
        findings.append("C2V8-DECLARED-STRING: not one declared subject is proved at "
    ...
    if jx_bind(result["unresolved"], 0) is not None:
        findings.append("C2V8-DECLARED-STRING: " + str(result["unresolved"]) +
                        " declared subject(s) name a string that exists nowhere")
    if jx_bind(result["namesUndefined"], 0) is not None:
        findings.append("C2V8-DECLARED-STRING: " + str(result["namesUndefined"]) +
...
def behavioural_layer(effective, measurement) -> dict:
    ...
    for position in jx_sorted(list(positions)):
        ...
        for label in HOSTILE_SPELLINGS:
            ...
            hit = [item for item in findings
                   if item.startswith("C2V8-TYPE:") and position in item]
            if hit:
                named += 1
                continue
            if not findings:
            ...
            escapes.append(position + "/" + label + ": rejected, but by " +
                           findings[0].split(":")[0] + " rather than a C2V8-TYPE "
    ...
    for position in jx_sorted(list(positions)):
        ...
        if [item for item in findings
                if item.startswith("C2V8-INSTRUMENT:") and position in item]:
            probes_named += 1
        else:
            probes_escaped += 1
...
def behavioural_findings(effective, measurement, authority, tree) -> list:
    ...
    if not jx_int_in_range(result["executedCases"], 1, 10 ** 9):
        findings.append("C2V8-BEHAVIOUR: the behavioural layer executed no case at all")
    if jx_bind(result["admissions"], 0) is not None:
        findings.append("C2V8-BEHAVIOUR: " + str(result["admissions"]) +
    ...
    if jx_bind(result["rejectedWithoutNamingThePosition"], 0) is not None:
        findings.append(
            "C2V8-BEHAVIOUR: " + str(result["rejectedWithoutNamingThePosition"]) +
            " case(s) were rejected without a C2V8-TYPE finding naming the position; "
    ...
    for key, expected in (("tokenCollisions", 0), ("crossTypeAdmissions", 0),
        ...
        if jx_bind(jx_get(primitive, key), expected) is not None:
            findings.append("C2V8-PRIMITIVE: " + key + " is " +
    ...
    if jx_bind(primitive["roundTrips"], primitive["corpusValues"]) is not None or \
        ...
        findings.append("C2V8-PRIMITIVE: the canonical encoding is not invertible over "
    ...
    if jx_bind(primitive["operatorSpaceHazardsReproduced"],
        ...
        findings.append("C2V8-PRIMITIVE: the operator space is not fully demonstrated "
...
def predecessor_differential(authority) -> dict:
    ...
    for vector, position, spelling, note in DIFFERENTIAL_VECTORS:
        ...
        v8_findings = census_comparison_findings(mutant, measurement)
        v8_named = [item for item in v8_findings
                    if item.startswith("C2V8-TYPE:") and position in item]
        if not v4_named:
        ...
        if v8_named:
            ...
            escapes.append(vector + ": this checker did NOT reject " + position +
                           " spelled " + spelling + " with a C2V8-TYPE finding naming "
                           "the position")
        rows.append({"vector": vector, "position": position, "spelling": spelling,
                     ...
                     "successorNamedThePosition": bool(v8_named)})
...
def sweep_findings(authority) -> list:
    ...
    if jx_bind(sweep["integerLeavesInjected"], live_leaves) is not None:
        findings.append("C2V8-SWEEP: the sweep reports " +
    ...
    if jx_bind(total, sweep["integerLeavesInjected"]) is not None:
        findings.append("C2V8-SWEEP: admitted plus rejected is " + str(total) + " but " +
    ...
    if jx_bind(parts, sweep["admittedToAFullGreenRun"]) is not None:
        findings.append("C2V8-SWEEP: the census and non-census admissions sum to " +
    ...
    if not jx_int_in_range(sweep["admittedOutsideTheCensusBlock"], 1, 10 ** 6):
        findings.append("C2V8-SWEEP: no leaf OUTSIDE the census block drives the pinned "
    ...
    if missing:
        findings.append("C2V8-SWEEP: the sweep no longer finds " + repr(missing) +
...
def successor_differential(authority, candidate) -> dict:
    ...
    rows, escapes = [], []
    admitted = rejected = 0
    for vector, v5_path, v8_path, spelling, note in SUCCESSOR_DIFFERENTIAL_VECTORS:
        ...
        named = False
        try:
            current = _resolve_steps(mutant, v8_path)
        except MALFORMED_SHAPE_EXCEPTIONS:
            escapes.append(vector + ": " + _steps_text(v8_path) + " does not resolve in "
                           "this candidate, so the vector is not retained as an "
                           "executable case")
            rows.append({"vector": vector, "path": _steps_text(v8_path), "note": note,
                         "v5FullyGreen": v5_green, "v8NamedThePosition": False})
            continue
        value = float(current) if jx_equal(spelling, "float-equal") else \
            (True if jx_equal(spelling, "boolean-true") else 1)
        _assign_steps(mutant, v8_path, value)
        findings = candidate_probe_findings(mutant, authority)
        named = bool([item for item in findings if _steps_text(v8_path) in item])
        if named:
            rejected += 1
        else:
            escapes.append(vector + ": this checker did NOT name " +
                           _steps_text(v8_path) + " when it was spelled " + spelling)
        rows.append({"vector": vector, "path": _steps_text(v8_path), "note": note,
                     "v5FullyGreen": v5_green, "v8NamedThePosition": named})
    return {"vectors": len(SUCCESSOR_DIFFERENTIAL_VECTORS), "rows": rows,
            "v5AdmittedToAFullGreenRun": admitted, "v8RejectedByName": rejected,
            "escapes": escapes}
...
def _v5_admissions(authority) -> list:
    ...
    try:
        with redirect_stdout(sink):
            v5 = _execute_snapshot("opensip_c2v8_pinned_v5_checker", V5_CHECKER,
    ...
    for vector, v5_path, _v8_path, spelling, _note in SUCCESSOR_DIFFERENTIAL_VECTORS:
        mutant = copy.deepcopy(contract)
        try:
...
# RES-C2V8-11 records that split, in the artifact, as v7 recorded its own.
# =============================================================================
...
def _pinned_v7_module(authority):
    ...
    sink = io.StringIO()
    with redirect_stdout(sink):
        module = _execute_snapshot("opensip_c2v8_pinned_v7_checker", V7_CHECKER,
...
def enumeration_findings(authority) -> list:
    ...
    findings = ["C2V8-ENUMERATION: " + item for item in result["escapes"]]
    expected_blind = result["vectors"] - result["controls"]
    if jx_bind(result["predecessorBlind"], expected_blind) is not None:
        findings.append(
            "C2V8-ENUMERATION: " + str(result["predecessorBlind"]) + " of the " +
    ...
    if jx_bind(result["successorEnumerated"], result["vectors"]) is not None:
        findings.append(
            "C2V8-ENUMERATION: this checker's leaf census reached " +
    ...
    if jx_bind(result["predecessorReachedControls"], result["controls"]) is not None:
        findings.append(
            "C2V8-ENUMERATION: the pinned predecessor reached " +
    ...
    if not jx_int_in_range(result["bytesAdded"], 1, 1):
        findings.append(
            "C2V8-ENUMERATION: the headline reproduction is recorded as adding " +
...
def live_register(authority) -> dict:
    ...
    return {
        ...
        "successorV8RejectedByName": succ["v8RejectedByName"],
...
BANNER_TEMPLATES = (
    "C-2 v8 contract OK - {documentName}; IR-C2V4-01 and the four blocking findings "
    ...
    "derives its coverage from the JSON data model - and IR-C2V7-02 repaired by making "
    "the duplicate-path walk iterative; {pinnedInputs} inputs hash-verified before "
    ...
    "named {successorV8RejectedByName} at the analogous position of its own document",
    ...
    "silently, and the scope is RES-C2V8-09 rather than a guard nobody could keep green",
...
def banner_findings(live, runtime) -> list:
    ...
    if not jx_int_in_range(len(banner_digit_tokens(BANNER_DETECTOR_PROBE)), 1, 10 ** 6):
        findings.append(
            "C2V8-BANNER: the bare-digit detector does not fire on " +
    ...
    if "{" in rendered[0] or "}" in rendered[0]:
        findings.append(
            "C2V8-BANNER: the renderer left an uninterpolated field in " +
    ...
    for token in jx_sorted(jx_unique(banner_digit_tokens(BANNER_TEMPLATES))):
        findings.append(
            "C2V8-BANNER: the banner carries the literal figure " + repr(token) +
    ...
    for key in unknown:
        findings.append("C2V8-BANNER: the banner interpolates " + key + ", which this "
                        "run does not measure")
    if not jx_int_in_range(len(jx_unique(referenced)), 1, 10 ** 6):
        findings.append("C2V8-BANNER: the banner interpolates nothing, so every figure "
                        "in it would be source text")
    declared = jx_sorted(list(RUNTIME_ONLY_FIELDS))
    if not jx_equal(jx_sorted(list(runtime)), declared):
        findings.append("C2V8-BANNER: the run-only banner fields are " +
...
REQUIRED_RESIDUAL_IDS = ("RES-C2V8-01", "RES-C2V8-02", "RES-C2V8-03", "RES-C2V8-04",
                         "RES-C2V8-05", "RES-C2V8-06", "RES-C2V8-07", "RES-C2V8-08",
                         "RES-C2V8-09", "RES-C2V8-10", "RES-C2V8-11", "RES-C2V8-12",
                         "RES-C2V8-13", "RES-C2V8-14", "RES-C2V8-15", "RES-C2V8-16")
...
def _identity_findings(c) -> list:
    ...
    if jx_bind_text(c.get("artifact"), ARTIFACT_ID) is not None:
        findings.append("C2V8: candidate is not " + ARTIFACT_ID)
    if not jx_exact_int(c.get("version"), 8) or not jx_exact_int(c.get("supersedes"), 7):
        findings.append("C2V8: candidate must declare version 8 superseding 7 as JSON "
    ...
    if "CANDIDATE-NOT-APPLIED" not in status or "AWAITING-INDEPENDENT-REVIEW" not in status:
        findings.append("C2V8: candidate status must remain CANDIDATE-NOT-APPLIED / "
    ...
    for finding_id in DISCHARGED_FINDING_IDS:
        if not jx_has(resolves, finding_id):
            findings.append("C2V8: the candidate does not name " + finding_id +
...
def _inventory_findings(c) -> list:
    ...
    for layer in layers:
        ...
        if jx_type(spots) != "array" or not spots:
            findings.append("C2V8-INVENTORY: layer " + repr(layer.get("id")) +
    ...
    if not jx_equal(jx_sorted(jx_unique(seen)), jx_sorted(list(REQUIRED_LAYER_IDS))):
        findings.append("C2V8-INVENTORY: the guard inventory declares layers " +
    ...
    if not jx_int_in_range(len(behind), 80, 10 ** 6):
        findings.append("C2V8-INVENTORY: the inventory does not state what stands behind "
    ...
    if "nothing" not in terminal.lower():
        findings.append("C2V8-INVENTORY: the inventory does not state plainly that the "
...
def _residual_findings(c) -> list:
    ...
    if missing:
        findings.append("C2V8-RESIDUAL: the candidate does not retain " + repr(missing) +
    ...
    for item in residuals:
        ...
        if not jx_in(status, ["RETAINED", "RETAINED-OPEN"]):
            findings.append("C2V8-RESIDUAL: " + repr(item.get("id")) + " is not RETAINED")
        why = item.get("whyNotClosed")
        why = why if jx_type(why) == "string" else ""
        if not jx_int_in_range(len(why), 40, 10 ** 6):
            findings.append("C2V8-RESIDUAL: " + repr(item.get("id")) + " does not say "
    ...
    if jx_type(accounting) != "object":
        findings.append("C2V8-RESIDUAL: the candidate publishes no residualAccounting "
...
def _recording_findings(c) -> list:
    ...
    if missing:
        findings.append("C2V8-RECORD: " + str(len(missing)) + " pinned input(s) are "
    ...
    for item in recorded:
        ...
        if jx_has(PINS, name):
            ...
            if reason is not None:
                findings.append("C2V8-RECORD: " + str(name) + ": " + reason +
        ...
        if not jx_int_in_range(len(role), 12, 10 ** 6):
            findings.append("C2V8-RECORD: " + repr(name) + " is recorded without a role, "
...
def _mode_findings(c) -> list:
    ...
    if jx_bind_text(mode.get("checker"), "check-c2-v8.py") is not None or \
            not jx_equal(jx_sorted(list(codes)), jx_sorted(list(DECLARED_EXIT_CODES))) or \
            "REFUSED" not in refusal:
        findings.append("C2V8: the checker mode contract does not declare exactly the "
...
def _repro_findings(c, authority) -> list:
    ...
    if jx_bind_text(repro.get("sourceModification"), "NONE") is not None:
        findings.append("C2V8-REPRO: the minimal reproduction is not recorded as "
    ...
    if jx_bind_text(repro.get("predecessorSha256"), jx_get(PINS, V4_CHECKER)) is not None:
        findings.append("C2V8-REPRO: the reproduction is recorded against " +
    ...
    for index, site in enumerate(sites):
        ...
        if not jx_int(value):
            findings.append(
                "C2V8-TYPE: theDefect/repairedComparisonSites/" + str(index) +
    ...
    if not jx_subset(adjudicated, lines):
        findings.append("C2V8-REPRO: the candidate enumerates comparison sites " +
    ...
    if not jx_equal(jx_sorted(jx_unique(declared)), jx_sorted(jx_unique(expected))):
        findings.append("C2V8-REPRO: the candidate records successor false-accept "
...
def _parse_repro_findings(c, authority) -> list:
    ...
    if jx_bind_text(defect.get("findingId"), "IR-C2V6-01") is not None or \
        ...
        findings.append("C2V8-PARSE-REPRO: the candidate does not record IR-C2V6-01 as "
                        "the BLOCKING finding this successor exists to discharge")
    repro = defect.get("minimalReproduction")
    repro = repro if jx_type(repro) == "object" else {}
    if jx_bind_text(repro.get("sourceModification"), "NONE") is not None:
        findings.append("C2V8-PARSE-REPRO: the minimal reproduction is not recorded as "
                        "requiring no source modification, which is the whole reason "
    ...
    if jx_bind_text(repro.get("predecessorSha256"), jx_get(PINS, V6_CHECKER)) is not None:
        findings.append("C2V8-PARSE-REPRO: the reproduction is recorded against " +
                        repr(repro.get("predecessorSha256")) + ", not the verified "
    ...
    if jx_bind_text(perrun, "MECHANISM") is not None:
        findings.append("C2V8-PARSE-REPRO: the candidate must say plainly WHICH HALF of "
    ...
    if not jx_equal(jx_sorted(jx_unique(declared)), jx_sorted(jx_unique(expected))):
        findings.append("C2V8-PARSE-REPRO: the candidate records parse vectors " +
...
def _enumeration_repro_findings(c, authority) -> list:
    ...
    if jx_bind_text(defect.get("findingId"), "IR-C2V7-01") is not None or \
            jx_bind_text(defect.get("disposition"), "BLOCKING") is not None:
        findings.append("C2V8-ENUM-REPRO: the candidate does not record IR-C2V7-01 as "
                        "the BLOCKING finding this successor exists to discharge")
    repro = defect.get("minimalReproduction")
    repro = repro if jx_type(repro) == "object" else {}
    if jx_bind_text(repro.get("sourceModification"), "NONE") is not None:
        findings.append("C2V8-ENUM-REPRO: the minimal reproduction is not recorded as "
                        "requiring no source modification, which is the whole reason "
                        "IR-C2V7-01 is BLOCKING rather than advisory")
    if jx_bind_text(repro.get("predecessorSha256"),
                    jx_get(PINS, V7_CHECKER)) is not None:
        findings.append("C2V8-ENUM-REPRO: the reproduction is recorded against " +
                        repr(repro.get("predecessorSha256")) + ", not the verified "
    ...
    if jx_bind_text(repro.get("reproducedPerRun"), "MECHANISM") is not None:
        findings.append("C2V8-ENUM-REPRO: the candidate must say plainly WHICH HALF of "
    ...
    if jx_bind_text(repro.get("mutantSha256"),
                    jx_get(authority.enumeration_differential, "mutantDigest",
                           "")) is not None:
        findings.append("C2V8-ENUM-REPRO: the candidate records the one-byte mutant of "
    ...
    if not jx_equal(jx_sorted(jx_unique(declared)), jx_sorted(jx_unique(expected))):
        findings.append("C2V8-ENUM-REPRO: the candidate records enumeration vectors " +
                        repr(jx_sorted(declared)) + " but this run executes " +
                        repr(jx_sorted(expected)) + "; a retained vector that is not "
                        "executed is prose")
    return findings
...
def _disclaimer_findings(c) -> list:
    ...
    that is exactly the class RES-C2V8-10 concedes L9 does not close.  A guard
    ...
    if "check-c2-v8.py" not in text:
        findings.append(
            "C2V8-DISCLAIMER: knownLimitations/thisIsNotAVerdict does not name "
            "check-c2-v8.py, the checker whose green run it exists to discount; v7's "
            "named check-c2-v6.py, the REJECTED predecessor, and nothing in that "
            "artifact could see it")
    for predecessor in (V4_CHECKER, V5_CHECKER, V6_CHECKER, V7_CHECKER):
        if predecessor in text:
            findings.append(
                "C2V8-DISCLAIMER: knownLimitations/thisIsNotAVerdict names " +
...
def candidate_probe_findings(candidate, authority) -> list:
    ...
    for record in jx_sorted_by(bindings, "steps"):
        ...
        if reason is not None:
            prefix = "C2V8-TYPE" if not jx_int(published) else "C2V8-CANDIDATE"
...
def differential_findings(authority) -> list:
    ...
    findings = ["C2V8-DIFFERENTIAL: " + item for item in result["escapes"]]
    if jx_bind(result["successorRejectedByName"], result["vectors"]) is not None:
        findings.append("C2V8-DIFFERENTIAL: " + str(result["successorRejectedByName"]) +
    ...
    if not jx_int_in_range(result["predecessorAdmittedThePosition"], 1, 10 ** 6):
        findings.append("C2V8-DIFFERENTIAL: the pinned predecessor named every vector "
    ...
    if not jx_int_in_range(result["predecessorFullyGreenRuns"], 1, 10 ** 6):
        findings.append("C2V8-DIFFERENTIAL: no retained vector reproduces the "
    ...
    findings.extend("C2V8-SUCCESSOR: " + item for item in successor["escapes"])
    if jx_bind(successor["v8RejectedByName"], successor["vectors"]) is not None:
        findings.append("C2V8-SUCCESSOR: " + str(successor["v8RejectedByName"]) + " of " +
    ...
    if not jx_int_in_range(successor["v5AdmittedToAFullGreenRun"], 1, 10 ** 6):
        findings.append("C2V8-SUCCESSOR: the pinned check-c2-v5.py admits none of its "
...
_EMPTY_PRIMITIVE = {
    ...
    "typeSpaceCases": 0, "typeSpaceAdmissions": 0, "escapes": [],
}
...
def _check(c, authority, self_tree=None) -> list:
    ...
    if jx_bind_text(derived.get("artifact"), V4_CONTRACT) is not None or \
        ...
        return findings + [
            "C2V8-DERIVATION: the candidate must derive from " + V4_CONTRACT + " at " +
    ...
    findings.extend("C2V8-INHERITED: " + str(item) for item in inherited)
    ...
    for name in authority.portability["names"]:
        findings.append("C2V8-PRIMITIVE: the primitive block references " + name +
    ...
    if not jx_int_in_range(probe["freeNames"], 1, 10 ** 6):
        findings.append(
            "C2V8-PRIMITIVE: the portability detector reports a block that plainly "
...
def _execute_tree(tree):
    module = types.ModuleType("opensip_c2v8_mutated")
...
_GATE_MUTATIONS = (
    ...
    ("restore v7's type-named enumeration: report only integer and number "
     ...
     "C2V8-PRIMITIVE", "jx_leaf_census"),
    ("stop the node walk seeing containers", "jx_walk",
     "return jx_leaf_census(value)", "C2V8-PRIMITIVE", "jx_walk"),
    ("let the total binding compare across JSON types", "jx_bind_value",
     "return None if published == measured else 'drift'",
     "C2V8-PRIMITIVE", "jx_bind_value"),
    ...
    ("declare the type space instead of measuring it", "jx_type_space",
     ...
     "C2V8-LEAFRULE", "the walker's own measured behaviour"),
)
...
GATE_MUTATIONS = tuple(
    (name, function, body, expected,
     function if expected == "C2V8-PRIMITIVE" and not subject else subject)
...
LAYER_MUTATIONS = (
    ...
    ("disable the behavioural layer", "behavioural_layer",
     ...
     "C2V8-BEHAVIOUR", "executed no case", False),
    ("confine the document lock to the census block", "locked_integer_leaves",
     "return []", "C2V8-DOCLOCK", "outside the lock", False),
    ("lock only the base direction, so a derived leaf escapes",
     ...
     "C2V8-DOCLOCK", "outside the lock", False),
    ("probe only the first leaf of the document lock", "document_type_probe",
     ...
     "C2V8-DOCLOCK", "probed no position", False),
    ("stop binding the candidate's own integer leaves", "candidate_bindings",
     "return [], []", "C2V8-UNBOUND", "no layer of this run binds", False),
    ("stop probing the candidate's own integer leaves", "candidate_type_probe",
     ...
     "C2V8-CANDLOCK", "probed no position", False),
    ("let the candidate binding table be narrowed silently", "_effective_expected",
     "return None, 'unmeasured'", "C2V8-UNBOUND", "the derivation writes a JSON integer",
     False),
    ("hollow out the whole-document sweep", "predecessor_sweep",
     ...
     "'elapsedSeconds': 0}", "C2V8-SWEEP", "integer leaves injected", True),
    ...
    ("stop reporting the byte/parse divergences at all", "parse_problem_findings",
     "return []", "C2V8-PARSE-PROBE", "duplicate at the root", False),
    ("stop looking for unhooked JSON parses in this file's own tree",
     "json_load_sites", "return []", "C2V8-PARSE-SCAN", "detects nothing", False),
    ("hollow out the parse probe", "parse_probe",
     "return {'executedCases': 0, 'namedAtThePosition': 0, 'controls': 0, "
     "'admissions': 0, 'escapes': []}", "C2V8-PARSE-PROBE", "probed no case", False),
    ("collapse the differential against the REJECTED v6's own parse",
     ...
     "C2V8-PARSE-DIFFERENTIAL", "unable to distinguish", False),
    ...
    ("restore v7's type-named candidate totality verbatim (IR-C2V7-01's own shape)",
     ...
     "    findings.append('C2V8-UNBOUND: the candidate publishes a JSON leaf at ' +\n"
    ...
    ("hollow out the whole-type-space injection probe", "candidate_totality_probe",
     ...
     "'namedBySkeletonOnly': 0, 'typesCovered': len(JX_TYPES), 'escapes': []}",
     "C2V8-TOTALITY", "probed no position", False),
    ("narrow the injection probe back to the two types v7 covered",
     ...
     "'namedBySkeletonOnly': 0, 'typesCovered': 2, 'escapes': []}",
     "C2V8-TOTALITY", "narrower than the data model", False),
    ("stop hashing the document's own shape", "document_skeleton",
     "return []", "C2V8-SKELETON", "does not cover every node", False),
    ("declare the leaf disposition table without measuring the type space",
     "leaf_rule_findings", "return []", "C2V8-LEAFRULE", "detects nothing", False),
    ...
    ("stop reading the number tokens of the pinned inputs",
     ...
     "C2V8-PARSE-SCAN", "finds no token at all", False),
...
def _m_absorb_a_residual(c):
    c["residuals"] = [item for item in c["residuals"]
                      if not jx_equal(item.get("id"), "RES-C2V8-03")]
...
def _m_rename_a_key(c):
    """No leaf is added or removed and no value changes; only a PATH does."""
...
CONTRACT_MUTATIONS = (
    ...
    ("understate one of this checker's own published counters",
     _m_understate_a_v8_counter, "C2V8-CANDIDATE", "behaviouralExecutedCases"),
    ("spell one of this checker's own published counters as a float",
     _m_float_a_v8_counter, "C2V8-TYPE", "registeredCensusPositions"),
    ("publish a counter this run does not measure", _m_publish_an_unmeasured_counter,
     "C2V8-UNBOUND", "counterNobodyMeasures"),
    ("soften the recorded minimal reproduction", _m_soften_the_repro, "C2V8-REPRO",
    ...
    ("spell a boolean reproduction claim as the integer the host language calls "
     "equal to it", _m_integer_the_reproduction_claim, "C2V8-TYPE",
     "theParseDefect/minimalReproduction/reproducedByThisLane"),
    ("add a JSON BOOLEAN leaf nobody binds, at the root (IR-C2V7-01 verbatim)",
     _m_inject_a_boolean_leaf, "C2V8-UNBOUND", "reviewerInjectedBool"),
    ("add the same boolean leaf at depth", _m_inject_a_boolean_leaf_at_depth,
     "C2V8-UNBOUND", "reviewerInjectedBoolAtDepth"),
    ("add a JSON NULL leaf nobody binds (OBS-C2V7-09's null spelling)",
     _m_inject_a_null_leaf, "C2V8-UNBOUND", "reviewerInjectedNull"),
    ("add a JSON STRING leaf nobody binds (OBS-C2V7-09's string spelling)",
     _m_inject_a_string_leaf, "C2V8-SKELETON", "documentSkeleton/sha256"),
    ("add an EMPTY object, which holds no leaf for any census to reach",
     _m_inject_an_empty_object, "C2V8-SKELETON", "documentSkeleton/sha256"),
    ("rename a key without adding, removing or changing any value",
     _m_rename_a_key, "C2V8-SKELETON", "documentSkeleton/sha256"),
    ("republish the skeleton digest so it no longer describes this document",
     _m_corrupt_the_skeleton_digest, "C2V8-SKELETON", "documentSkeleton/sha256"),
    ("drift this document's own count of its boolean leaves",
     _m_drift_a_skeleton_count, "C2V8-CANDIDATE", "documentSkeleton/booleanLeaves"),
    ("soften the IR-C2V7-01 minimal reproduction", _m_soften_the_enumeration_repro,
     "C2V8-ENUM-REPRO", "no source modification"),
    ("overclaim how much of the IR-C2V7-01 reproduction an ordinary run recomputes",
     _m_overclaim_the_enumeration_repro, "C2V8-ENUM-REPRO", "WHICH HALF"),
    ("stop retaining one of the enumeration vectors", _m_drop_an_enumeration_vector,
     "C2V8-ENUM-REPRO", "IR-C2V7-01"),
    ("point the this-is-not-a-verdict disclaimer at the predecessor "
     "(OBS-C2V7-05 verbatim)", _m_disclaim_the_predecessor, "C2V8-DISCLAIMER",
     "check-c2-v7.py"),
)
...
def _s_inject_named_constant_comparison(tree):
    ...
    subject.body.extend(ast.parse(
        "_LAW18_PROBE = 1\n"
        "def _c2v8_named_constant_probe(row):\n"
...
SCAN_MUTATIONS = (
    ("inject a wire value compared against a COMPUTED integer",
     _s_inject_computed_operand_comparison, "C2V8-SCAN", "_c2v8_computed_probe"),
    ("inject IR-C2V5-01's comprehension-result subset test verbatim",
     _s_inject_comprehension_subset, "C2V8-SCAN", "_c2v8_subset_probe"),
    ("inject an operator.ne call, which is never an ast.Compare",
     _s_inject_operator_call, "C2V8-SCAN", "_c2v8_operator_probe"),
    ("inject a dict key lookup on a wire key", _s_inject_dict_key_lookup,
     "C2V8-SCAN", "_c2v8_keylookup_probe"),
    ("inject a set() dedup that collapses 1, 1.0 and True",
     _s_inject_dedup_cardinality, "C2V8-SCAN", "_c2v8_dedup_probe"),
    ("inject a wire value compared against a module-level named integer",
     _s_inject_named_constant_comparison, "C2V8-SCAN", "_c2v8_named_constant_probe"),
    ("inject membership in a numeric literal container",
     _s_inject_membership_comparison, "C2V8-SCAN", "_c2v8_membership_probe"),
    ("inject a max() over a wire sequence", _s_inject_sorted_launder,
     "C2V8-SCAN", "_c2v8_sorted_probe"),
)
...
PARSE_MUTATIONS = (
    ...
    ("shadow this document's own SKELETON node count, inside the block that binds "
     ...
     '\n    "nodes": ', '\n    "nodes": 1.0,\n    "nodes": ',
     "C2V8-DUPKEY", "documentSkeleton/nodes"),
...
def parse_mutation_findings(text, authority):
    ...
    try:
        ...
        return ["C2V8-PARSE: the mutant did not parse (" + type(exc).__name__ + ": " +
                str(exc) + ")"]
...
def pinned_v6_over_a_duplicate_key(authority):
    ...
    try:
        with redirect_stdout(sink):
            v6 = _execute_snapshot("opensip_c2v8_pinned_v6_checker", V6_CHECKER,
    ...
    result = {"vector": vector, "document": document, "position": position,
              ...
              "v8Findings": parse_mutation_findings(mutant, authority)}
...
def pinned_v7_over_a_boolean_flip(authority):
    ...
    try:
        with redirect_stdout(sink):
            v7 = _execute_snapshot("opensip_c2v8_pinned_v7_executed", V7_CHECKER,
                                   authority.snapshots[V7_CHECKER],
                                   authority.directory)
    ...
    result = {"document": V7_CONTRACT, "position": _steps_text(position),
              ...
              "v8CensusReportedIt": bool(census),
              "v8LeafType": census[0][1] if census else ""}
...
RETAINED_ROWS = (len(PARSED_ROOT_CASES) + len(CONTRACT_MUTATIONS) +
                 ...
                 2 + len(DIFFERENTIAL_VECTORS) + len(SUCCESSOR_DIFFERENTIAL_VECTORS) +
...
def selftest(candidate, authority, path):
    ...
    print("C-2 v8 mutation self-test over " + path.name + " - each row must be REJECTED "
    ...
    for name, root in PARSED_ROOT_CASES:
        ...
        named = hit_for(findings, "C2V8-TOTALITY-ROOT", "non-empty JSON object")
        if not named:
            escaped.append("parsed-JSON root " + name + ": no named C2V8-TOTALITY-ROOT "
                           "finding")
    ...
    for name, mutate, expected, subject in SCAN_MUTATIONS:
        try:
            ...
            findings = ["C2V8-SCAN: " + site["function"] + " line " +
        ...
        report(bool(hit), name, hit[0][:180] if hit else
               "NO C2V8-SCAN finding naming " + subject)
    ...
    v8_hit = hit_for(external["v8Findings"], "C2V8-DUPKEY", external["position"])
    ok = external["v6FullyGreen"] and bool(v8_hit)
    ...
    report(ok, "IR-C2V6-01 against the PINNED check-c2-v6.py [" + V6_CHECKER + "]",
           ...
           (v8_hit[0][:140] if v8_hit else "NO NAMED FINDING"))
    ...
    ok = flip["v7FullyGreen"] and flip["v8CensusReportedIt"] and \
        jx_equal(flip["v8LeafType"], "boolean")
    ...
    if not flip["v8CensusReportedIt"]:
    ...
    report(ok, "IR-C2V7-01 against the PINNED check-c2-v7.py [" + V7_CHECKER + "]",
           ...
           "; this checker's census reports a JSON " + repr(flip["v8LeafType"]) +
    ...
    for row in clean_successor["rows"]:
        ok = row["v8NamedThePosition"] and row["v5FullyGreen"]
        if not ok:
        ...
        report(ok, "retained v5 blocking vector " + row["vector"] + " at " + row["path"],
               ...
               "of its own document: " + str(row["v8NamedThePosition"]))
...
def main(argv):
    try:
        ...
        print("C2V8-UNSUPPORTED-INVOCATION: " + str(exc), file=sys.stderr)
        return 2
    try:
        authority = load_authority()
    except AuthorityLoadError as exc:
        print("C2V8-PINNED-INPUT-REFUSED: " + type(exc).__name__ + ": " + str(exc),
              file=sys.stderr)
        return 2
    ...
    try:
        ...
        print("cannot load C-2 v8 candidate " + str(path) + ": " + type(exc).__name__ +
