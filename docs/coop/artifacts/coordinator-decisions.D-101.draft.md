# D-101 — D-006 fleet-class successor plus G03/G04 named identifiers

> **Status:** DRAFT — under review.
> **Date:** 2026-08-14
> **Protocol:** D-000 new cycle, turn 1 of 3. Not a fourth
> turn of CONTESTED D-099. Frozen D-099 subjects are not
...
> Rosetta. **Does not** overturn D-094, D-098, or D-099
> CONTESTED, D-096, or D-097.

D-099 is CONTESTED at `90c80fc31b4c4a0dc376da62184d1fd6d82f21a4`.
This cycle restates the D-099 turn-3 contract in this
file's own bytes and replaces only the macOS process-arch
and start-identity steps, plus the Linux AT_PLATFORM
dereference. D099-T3-MF-1, D099-T3-SF-1, and
ADV-D099-T3-01 accepted. PROC_PIDARCHINFO is **not**
used: it is absent from MacOSX15.4.sdk (the fleet class).

| ID | Sev | Disposition |
|---|---|---|
| D099-T3-MF-1 | MUST-FIX | ACCEPTED. macOS process arch is `sysctl.proc_cputype` with PID T appended. Not a `kinfo_proc` CPU-type member. Not `PROC_PIDARCHINFO`. |
| ADV-D099-T3-01 | MUST-FIX | ACCEPTED. Same. Start identity is `PROC_PIDTBSDINFO` `pbi_start_tvsec`/`pbi_start_tvusec`, size-checked and re-read. |
| D099-T3-SF-1 | SHOULD-FIX | ACCEPTED. Start identity is compared: S vs S1 after the other observations. |
| D099-T3-N-1 | NOTE | ACCEPTED. The child sets `PTRACE_TRACEME`. The parent wait is the post-`execve` `SIGTRAP` that `PTRACE_TRACEME` produces. |
| ADV-D099-T3-N-01 | NOTE | ACCEPTED. AT_PLATFORM is dereferenced in T's memory while T is stopped. |
| D099-T3-N-2 | NOTE | ACCEPTED. macOS binds D by hashing `proc_pidpath(T)` while T is suspended; that is weaker than Linux `/proc/T/exe` fd hash and is stated. |
...
**macOS target-process protocol (item 10).** APIs are
those present on MacOSX15.4.sdk (the fleet class).
`PROC_PIDARCHINFO` / `struct proc_archinfo` are absent
...
3. `proc_pidpath(T)` must be P (same inode). sha256 of
   the bytes at that path must equal D. This path-hash,
   taken while T is suspended and holds the vnode, is
   weaker than Linux's `/proc/T/exe` fd hash. Darwin has
   no fd-to-running-image equivalent. A path swap after
   the hash is detectable because T remains suspended
   and unreaped until step 8.
4. Start identity. `n = proc_pidinfo(T, PROC_PIDTBSDINFO,
   ...
   and is not used.
5. Process arch. `sysctlnametomib("sysctl.proc_cputype",
   mib, &miblen)`; append T as the next mib word;
   `sysctl(mib, miblen+1, &cputype, &len, NULL, 0)`.
   `len` must be `sizeof(cpu_type_t)` and the call must
   succeed. That `cpu_type_t` is the process-arch field.
   Accepted: `macos-15` → `CPU_TYPE_ARM64` (`0x0100000c`);
   `macos-15-intel` → `CPU_TYPE_X86_64` (`0x01000007`).
   Other, short `len`, or error → reject. This is not
   `file`/`lipo` of the on-disk path. This is not a
   `kinfo_proc` CPU-type member (that member does not
   exist). This is not `PROC_PIDARCHINFO`.
6. Translation field of T: `sysctl(CTL_KERN, KERN_PROC,
   ...
   only if H, E, and `sysctl.proc_cputype` all match the
   class. Unreadable → reject. Parent
...
```
named: harness.DR-G03.core-startup (D-101; D-006 fleet-class successor; not authored; not QUALIFIED). cold-cache harness, p50/p95/p99, fleet class
```
...
```
named: harness.DR-G04.core-memory (D-101; D-006 fleet-class successor; not authored; not QUALIFIED). same fleet classes and lifecycle commands
```
...
```
**18 of 18 required gates name a recorded identifier** (D-086 / D-088 / D-101; not authored; not QUALIFIED); G03/G04 named under D-101 hosted-fleet-class contract
```
...
- Fourth turn of D-099. Rejected: CONTESTED; new cycle only.
...
C-D101 plus restore of the two reserved cells, 16-of-18
...
overturn D-099 CONTESTED.

## Measured inputs at turn-1 dispatch

| Path | sha256 |
|---|---|
| COORD (live) | `0f184eb32dceab330e8ec0b6e9fb9886881fcb0f228ebf346ce5be2e2dc2b4cf` |
...
| D-099 CONTESTED predecessor commit | `90c80fc31b4c4a0dc376da62184d1fd6d82f21a4` |
| D-099 turn-3 subject | `3249a26c738c35cbd20f31ebe442559fc027acc764a610cb4dc93eac3b4ee47a` |
| D-099 Claude 2 turn 3 | `83b0aefe6487882adff7a7317e10f4445161fc14c65d47aa339d052cafe6f2a7` |
| D-099 Codex turn 3 | `c53c261f71496b86e5c7bf43d6d87d5a606d5a2bc7e129b05db1a8e04a9c1d47` |
