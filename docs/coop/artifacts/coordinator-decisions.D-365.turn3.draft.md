...
> independent reviewers on the same MUST-FIX; turn 2 drew OBJECT from both at 0
> MUST-FIX and 1 SHOULD-FIX each, again the same finding. This turn lands every
> turn-1 identifier:
...
> `93ae1670135e8a45f03f53da8cada144a257f5b894c7c5b0b8dab9d6ef845a7a` and the
> turn-2 subject `coordinator-decisions.D-365.turn2.draft.md`
> `476392bf5a7686ae1e9889c03fd5bd4b77057a8b910f308689a292594ea53259` remain
> frozen and unrecorded. This turn also lands every turn-2 identifier:
> **CODEX-D365-T2-SF1**, **CLAUDE-D365-T2-SF1**, **CLAUDE-D365-T2-ADV-1**,
> **CLAUDE-D365-T2-ADV-2**, **CLAUDE-D365-T2-ADV-3** and
> **CLAUDE-D365-T2-ADV-4**. Codex turn 2 returned
> `currentFindingIdentifiers` `["CODEX-D365-T2-SF1"]`, an empty mustFix list, an
> empty advisories list, an empty observations list and an empty
> observationsNotFindings list. Claude 2 turn 2 returned
> `currentFindingIdentifiers` `["CLAUDE-D365-T2-SF1"]`, an empty mustFix list,
> four advisories and five unlabelled entries in its observationsNotFindings
> list; this entry invents no identifier for those five.
...
5. The snapshot heading date is not edited. It reads "Current position —
   measured snapshot, 2026-08-15" and was last set by D-135; D-236 and D-363
   each rewrote the condition-2 cell without touching it, and this entry keeps
   that practice rather than starting a different one inside a re-measurement.
   Moving the heading date is a hygiene act of its own.
6. Condition 2's "Required" cell already reads "Every slice-affecting V2 row
...
D-134 carries the convention-conforming commit pinned above,
`d3a3b744a7b90619d381aea1efec864e430def72` ("D-134: add DR-131 and DR-133 to
D-002 SATISFIED-requiring set"), an ancestor of HEAD. D-002 and D-010 predate
the `D-NNN: …` commit-subject convention and carry no such commit; each is
pinned by its COORD heading, and COORD is pinned above. Quotations from D-002,
D-134 and file 08 carry no emphasis of this entry's own; bold inside the D-134
quotation is D-134's, and D-002's own bold is not reproduced in the two D-002
quotations. The 21-row and nine-row listings are lists of row identifiers, not
quotations: D-002's parenthetical scoping text for DR-105 is abridged to
"(scoped)" and DR-117's is omitted, and both are read at D-002 itself.
