#!/usr/bin/env python3
"""Fail-closed checker for section31-supplier-coverage.v2.
...
TABLE = ROOT / "docs/coop/artifacts/section31-supplier-coverage.v2.json"
...
def main() -> int:
    ...
    rows = table["items"]
    expected_n = table["counts"]["items"]
    if len(rows) != expected_n:
        failures.append(f"item count {len(rows)} != counts.items {expected_n}")
