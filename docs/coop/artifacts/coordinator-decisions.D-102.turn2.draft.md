...
> Frozen turn-1 subject is not edited. Frozen D-101 and
> D-099 subjects are not edited. Not a fourth turn of
> D-101. Not a SATISFIED re-record.
...
Turn-1 subject `coordinator-decisions.D-102.draft.md`
`2bcb6c4aee1d0d8a7a9e456f1011c95febac81bf39caa89238d3f9a8367d4f9f`
held frozen. Claude 2 OBJECT, 0 MUST-FIX, 1 SHOULD-FIX
D102-T1-SF-1. Codex CONSENT, 0 MUST-FIX, 0 SHOULD-FIX.
D-101 remains CONTESTED at `c159c224c8766715b21a1522755aeb235c8335df`.
The contract lives in this file's bytes.

| ID | Sev | Disposition |
|---|---|---|
| D102-T1-SF-1 | SHOULD-FIX | ACCEPTED. `posix_spawnattr_setarchpref_np` passes subtype `CPU_SUBTYPE_ANY` and must return 0 with `ocount == 1`; otherwise reject. Spawn failure still rejects. |
| ADV-D101-T3-01 | SHOULD-FIX | Remains accepted: disposition matches the numbered sequence. |
| D101-T2-SF-1 | SHOULD-FIX | Remains accepted: split check on both platforms. |
...
1. Parent `posix_spawnattr_setflags(POSIX_SPAWN_START_SUSPENDED)`
   ...
   `CPU_TYPE_X86_64` for `macos-15-intel`. The call
   passes subtype `CPU_SUBTYPE_ANY` and must return 0
   with `ocount == 1`; a non-zero return or
   `ocount != 1` → reject. Close-match fallback
   (`arch(1)` trying a near arch) is forbidden.
...
- Pass a NULL subtype to `setarchpref_np`. Rejected:
  D102-T1-SF-1; EINVAL, no preference set, spawn
  unconstrained.
...
| D-102 turn-1 subject | `2bcb6c4aee1d0d8a7a9e456f1011c95febac81bf39caa89238d3f9a8367d4f9f` |
| D-102 Claude 2 turn 1 | `eba606f1a2ade9fc3ae2a4d267bf3516f7e2be01b5e66f83aeaddd3a0a95ec8e` |
| D-102 Codex turn 1 | `c34e9fb633c7b7a270238145cf52175b4e173efd1feb47f8bae1b6276ac8900b` |
