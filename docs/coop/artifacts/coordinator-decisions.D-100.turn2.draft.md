...
> Frozen turn-1 subject is not edited. Frozen D-096 /
...
held frozen. Claude 2 CONSENT. Codex OBJECTIONS, 1
MUST-FIX ADV-D100-01.

| ID | Sev | Disposition |
|---|---|---|
| ADV-D100-01 | MUST-FIX | ACCEPTED. The 106/109/113 candidate is a shared draft. Each later owner-recording names exactly one of those rows, has its own cycle and commit, and does not adopt or overturn the other two. |
...
2. **Preconditions for one later owner-recording entry.**
   ...
   - an applicable user-made grant (D-096 (A)) is ADOPTED,
     ...
     role for the **row** being recorded (for the
     106/109/113 candidate, the selected row's roles);
...
4. **Required pins on each later owner-recording entry.**
   ...
   - candidate path and sha256; both verdict paths and
     sha256s; the single file-08 row being recorded
     (for 106/109/113, exactly one of those three);
     file-08 owner role(s) for that row; preview scope;
     Route A remainder if any;
...
5. **What a later owner-recording may do.** Record the
   ...
   explicit deferral for the roles of the **named row**
   (one row; for 106/109/113, the selected row only).
   It may not mark the row `SATISFIED`. It may not edit
   file 08. SATISFIED, if ever, is a **separate** later
   D-000-reviewed MF-6. Owner-recording precedes that
   MF-6 and is not it. It may not authorize
   `docs/v2/implementation/`. It may not apply a V1
   successor, move the freeze, or move the claim
   register. It may not bundle DR-106, DR-109, and
   DR-113 into one owner-recording.
...
| D-100 Claude 2 turn 1 | `48f70c9dbcb021464d8bc4fa63688cb178206e4e04508741dcb79bf423710e02` |
| D-100 Codex turn 1 | `41bbf584dcab6c0bee2a42e6b91599fab3194d23514c254d2a74bf50405a1a71` |
