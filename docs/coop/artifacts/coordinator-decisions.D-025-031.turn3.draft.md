# D-025–D-031 draft — new cycle, turn 3 (final)
...
> `docs/coop/artifacts/coordinator-decisions.D-025-031.turn2.draft.md`
> `05f9055631b4fc43ada9cb385d4c4b75560c48ee52de8f70551ffd43efb0001f`
> **Turn-2 verdicts:**
> - Claude 2 `a2052bcf7eb6357b91a6791a20a547ad954bc9e873c2c3e7337508e2eb565efa`
> - Codex `947f6db14a42ba1bb53c889a9f3c5fc2a382d1eba523ceb1e9cc20ad3008cee4`
...
- **Decision type:** RULE-GOVERNED. Restates file 08's only-checklist rule, file 07/10 competing-list disclaimers, file 11's header, and D-001's A/B/C routes. Adds no fourth route and no new closed set.
...
3. **An item in file 11 becomes live work only by travelling a route
   ...
   - **Route C** — a product decision through the
     product-disposition process, recorded in a form whose authority derives from the
     product-disposition process, D-000, or the user, including
     but not limited to these corpus-used forms: (i) in the
     product-disposition packet by the product authority; (ii) a
     decision made directly by the user and recorded in the
     coordinator register; (iii) a coordinator decision made on
     the user's behalf under D-000 and recorded as a D-000 entry.
...
**Reversibility class:** total before any dependent lands.
Overturn: supersession plus `git revert` of C-D025 is then
sufficient. After D-031 (or any other dependent) is adopted,
overturn also requires that dependent's supersession or revert.
C-D025 alone does not leave D-031's accounting rule well-defined
once D-031 exists.
...
- **Supersedes:** CONTESTED D-019 draft only. Adopting or overturning this entry does not change D-020–D-024.
- **Subject:** DR-002 only.
...
- **Decision type:** PREFERENCE-LADEN.
- **Supersedes:** CONTESTED D-020 draft only. Adopting or overturning this entry does not change the other five contested predecessors.
- **Subject:** DR-004 only.
...
3. Preview-scoped. The authoritative Phase-1A packet remains owed
   whether or not D-031 is adopted, as does the §3.1
   item-to-supplier binding instrument D-001 already named as
   Route A work. This entry writes neither.
...
- **Decision type:** PREFERENCE-LADEN.
- **Supersedes:** CONTESTED D-021 draft only.
- **Subject:** DR-005 only.
...
5. Writes no disposition. Marks nothing `SATISFIED`. Authorizes no
   blueprint. A completed, reviewed disposition recorded by the
   owning authority may discharge condition 1 for DR-005 within
   the preview scope it names. Conditions 2–5 remain independently
...
- **Decision type:** PREFERENCE-LADEN.
- **Supersedes:** CONTESTED D-022 draft only.
...
4. The coordinator selects. Evidence/retention authority records.
   ...
   Coordinator-composed `SATISFIED` unlawful (DR-204).
5. Writes no disposition. Marks nothing `SATISFIED`. Authorizes no
   blueprint. Adoption permits authoring the later disposition; it
   does not by itself make preview design of this row lawful. Only
   after a completed, reviewed, evidence/retention-authority-recorded
   disposition may condition 1 be discharged within its scope.
   Conditions 2–5 remain independently required. Condition 5 remains
   the only authorization for `docs/v2/implementation/`.
...
authorization to *author* the later scoped disposition. Join and
Phase-1A remain owed. Adoption is not itself preview-design
...
- **Decision type:** PREFERENCE-LADEN.
- **Supersedes:** CONTESTED D-023 draft only.
...
- **Decision type:** PREFERENCE-LADEN.
- **Depends on, but is severable from:** D-018 (ADOPTED), D-025
  (if adopted; accounting rule), and D-026..D-030 (if adopted).
- **Supersedes:** CONTESTED D-024 draft only.
...
and Phase-1A insertion; the freeze §3.1 item-to-supplier binding
instrument (D-001 Route A work; not a file-11 authority). Preview-only
Route B dispositions do **not** terminate Lane R's authoritative
obligations. This lane starts with
...
Scheduling is not consumption as live work. Including an item in
this adopted sequence authorizes drafting only. The resulting
artifact becomes live work only by travelling D-001 routes A, B, or
C. If adopted, authorizes drafting of file 11 item 1 remainder and
items 2, 3, 4, 5. Unscheduled as file-11 items: 6 and 8. Item 7
(the §3.1 supplier-coverage instrument) is not a file-11 live-work
item; it is standing Lane R / D-001 Route A work. D-018 consumed
...
- Gate this sequence on an effort model. Rejected as a blocker of
  these entries: D-001 already names the completion predicate, so
  an estimate informs capacity but does not change what done is.
  A lightweight DAG may be drafted later as file-11 item 8; it is
  not a prerequisite of this sequence.

### Honesty about the trade

Given up: neither preview nor Route A is the unique critical path;
parallel lanes spend coordinator capacity on both. When owners
conflict, Lane R (authoritative V1 closures) is not paused for Lane
P, and Lane P is not paused for Lane R. Gained: they run together.
One-revert total reversal exists only before execution begins.
...
**Reversibility class:** total before execution of Lane P or Lane R
work under this entry begins. Overturn then: supersession plus
`git revert` of C-D031. After dependent drafting or decisions land,
supersession is prospective; independently live downstream acts
remain standing unless separately overturned under their own
procedures. Leaves D-018 and D-025–D-030 standing if those were
adopted.
