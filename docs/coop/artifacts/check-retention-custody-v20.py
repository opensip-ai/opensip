#!/usr/bin/env python3
"""Exact architecture conformance checker for retention-tiers.v20.json.
...
STARTUP_REFUSAL = (
    "RT20-CHECKER-UNSUPPORTED-INVOCATION: use python3 -I -B "
    "check-retention-custody-v20.py"
)
...
BINDING = "retention-tiers.v20.json"
RT20_STATUS = "CANDIDATE-NOT-APPLIED/AWAITING-INDEPENDENT-REVIEW"
...
RT19 = "retention-tiers.v19.json"
RT19_CHECKER = "check-retention-custody-v19.py"
RT19_ADJUDICATION = "retention-tiers.v19.adjudication-v18-review-response.json"
RT19_REVIEW = "retention-tiers.v19.review-independent-prefreeze.json"
ARCHITECTURE_PLAN = "ARCHITECTURE-TO-IMPLEMENTATION-PLAN.md"

PINS: dict[str, str] = {
    RT19: "c81004e42efc6653c59e53460d75f4a6904704461d64ee25b657d8f54f021a50",
    RT19_CHECKER: "ba945983272edb83fb9f06dc9f808e87456ddf59d5ae5866ebde6d1ecfd876d3",
    RT19_ADJUDICATION: "09eea9feae54146c99026817d137c0384ce2c7b63578db658655323d00b44bdb",
    RT19_REVIEW: "b850695323e5a00e46b3e5abfb0a50bcb5520cc3ae4ec87c64f139909007e3b4",
...
class FrozenAuthority:
    ...
    def freeze(self) -> None:
        ...
        for name in (RT19, RT19_ADJUDICATION, RT19_REVIEW,
    ...
    def _validate_reviews(self) -> None:
        ...
        rt19_review = self.parsed[RT19_REVIEW]
        ...
        verdict19 = rt19_review.get("verdict", {})
        finding_ids19 = [row.get("id") for row in rt19_review.get(
            "blockingFindings", []) if isinstance(row, dict)]
        if rt19_review.get("decision") != "REJECT" or \
                verdict19.get("decision") != "REJECT" or \
                verdict19.get("blockingFindingCount") != 1 or \
                finding_ids19 != [
                    "RT19-PF-01-CLOSED-GRAPH-DENOMINATOR-STILL-INCOMPLETE",
                ]:
            raise AuthorityError("RT19 exact one-blocker review binding drift")
        subject_rows19 = rt19_review.get(
            "exactByteReviewBasis", {}).get("subjects", [])
        subjects19: dict[str, str] = {}
        for row in subject_rows19:
            if not isinstance(row, dict) or \
                    row.get("sha256AtStart") != row.get("sha256AtEnd"):
                raise AuthorityError("RT19 review subject window drift")
            subjects19[row.get("path")] = row.get("sha256AtStart")
        if subjects19 != {
                RT19: PINS[RT19], RT19_CHECKER: PINS[RT19_CHECKER],
                RT19_ADJUDICATION: PINS[RT19_ADJUDICATION]}:
            raise AuthorityError("RT19 review exact subjects drift")
        if rt19_review.get("exactByteReviewBasis", {}).get(
                "inputHashDrift") is not False or \
                rt19_review.get("exactByteReviewBasis", {}).get(
                "inputIdentityDrift") is not False:
            raise AuthorityError("RT19 review input window drift")
        retained19 = rt19_review.get("retainedGateResults", {})
        authored19 = retained19.get("authoredRT19Suite", {})
        independent19 = retained19.get("candidateBlindIndependentSuite", {})
        if authored19.get("normal") != "PASS" or \
                independent19.get("retainedPredecessorInputPins") != "55" or \
                independent19.get("retainedPredecessorPinCorruptions") != "55/55":
            raise AuthorityError("RT19 retained 55-pin pass binding drift")
...
def expected_candidate(authority: FrozenAuthority) -> dict[str, Any]:
    candidate = copy.deepcopy(authority.parsed[RT19])
    candidate["version"] = 20
    candidate["status"] = RT20_STATUS
    candidate["supersedesAsArchitectureCandidate"] = RT19
    candidate["authority"] = {
        "candidateState": "NOT-APPLIED",
        "authorityClaim": "NONE",
        "semanticSourceRule": "The exact RT19 semantic object is inherited only for its independently retained narrow passing evidence: exact 55-pin authority checks, EP8 API/receipt commitments, real D9 v1.10 dataclass semantics, current concrete return graph, semantic data/reducers, unchanged 7/2 and 5/3 raw-byte fixtures, nine leases, 13/7 root delta, executed D9 consumers, and strong-reference identity custody. Its one rejected semantic-child-denominator claim is replaced by a generic CPython referent oracle plus exact memoryview/code/frame exception registry, not by another predecessor-diff edge patch.",
    ...
    ownership["rootDelta"] = {
        "predecessor": RT19, "predecessorRootCount": 20,
        ...
        "equation": "13 independently reviewed unchanged roots + 7 metadata/verifier-only roots = 20; status, semanticLeaseProtocolV3, and every semantic/data/fixture root are exact RT19 bytes as parsed data",
    }

    candidate["invariants"] = copy.deepcopy(authority.parsed[RT19]["invariants"]) + [
        "RT20-30: the retained RT19 55-input window is replaced lineage-for-lineage with exact RT19 artifact/checker/adjudication/one-blocker review while remaining exactly 55 unique alias-collapsed, read-once, hash-bound, end-stat-checked inputs; all 55 single-pin corruptions and physical aliases refuse before callback",
        f"RT20-31: every admitted traverse kind either expands the CPython 3.14 gc.get_referents direct-strong-referent oracle or matches the exact memoryview/code/frame exception registry; an independently declared {len(GRAPH_SEMANTIC_CHILD_DENOMINATOR)}-kind semantic-child denominator closes those modes while unknown runtime kinds refuse",
        f"RT20-32: exact slice start/stop/step, code co_consts, function code/builtins/doc/module/type-params/annotate, property doc/name, frame builtins/code/trace/generator, and suspended generator/coroutine/async-generator code/continuation links are named within a {len(GRAPH_EDGE_REGISTRY)}-edge registry and {len(GRAPH_RUNTIME_KIND_REGISTRY)}-kind registry",
        f"RT20-33: the separately declared exact {len(GRAPH_POSITIVE_CASE_IDS)}-case positive denominator passes both as helper roots and after injection beneath the actual VerifiedSemanticRTSnapshotV2 operation-function map; every edge, kind, semantic-child row, oracle exception, and positive case has an independent deletion refusal",
        "RT20-34: cycle deduplication retains a strong id-to-object reference for every expanded identity until the complete scan ends; mappingproxy id-reuse remains 20/20 for helper and actual roots, and traversal/budget/oracle/registry failures remain named refusals without traceback escape",
    ]
    candidate["retainedResiduals"] = [
        "RT19 is independently REJECTED for exactly RT19-PF-01-CLOSED-GRAPH-DENOMINATOR-STILL-INCOMPLETE with zero input hash/identity drift. Its narrow passes for 55-pin corruption/alias refusal, EP8 nine-API commitments, real D9 v1.10 dataclass, current concrete snapshot, 7/2 and 5/3 byte fixtures, nine leases, D9 6/16/10, the 13/7 root delta, declared graph checks and strong-reference identity custody are inherited without semantic change; RT20 itself remains unreviewed and NOT-APPLIED.",
    ...
    api["constructor"]["signature"] = \
        "validate_semantic_rt_snapshot_v5(FrozenDependencySnapshotV6, VerifiedD9SnapshotV1) -> VerifiedSemanticRTSnapshotV2"
    ...
    api["returnedSnapshot"] = {
        ...
        "retainsExactly": [
            "snapshotId",
            "deep-immutable verified RT20 candidate data projection",
        ...
        "objectGraphRule": "Before return, a depth/node-bounded CPython 3.14 traversal expands gc.get_referents for every admitted traverse kind except the exact memoryview/code/frame oracle gaps closed by a separately declared exception registry, validates exact ordered edge/runtime-kind/semantic-child/positive-case denominators, retains every expanded object strongly until scan completion, and rejects every unknown runtime kind. The exact positive denominator passes both as helper roots and beneath the actual VerifiedSemanticRTSnapshotV2 operation-function map. Forbidden identity, oracle/registry/denominator drift, traversal exception, impossible live-id collision, or budget exhaustion is a named refusal without traceback.",
    ...
    api["predecessorRejectionDisposition"] = {
        "reviewArtifact": RT19_REVIEW,
        "reviewSha256": PINS[RT19_REVIEW],
...
def _semantic_findings(candidate: dict[str, Any], expected: dict[str, Any],
    ...
    for key in unchanged:
        ...
        if difference:
            findings.append(f"RT20-RT19-NARROW-PASS-PRESERVATION: {difference}")
    ...
    if candidate.get("status") != RT20_STATUS:
        findings.append("RT20-STATUS: candidate/review boundary drifted")
...
def _snapshot_quarantine_findings(
    ...
    count = scan["visited"]
    findings = [
        f"RT20-GRAPH-FORBIDDEN-IDENTITY: path={row['path']} reaches="
    ...
    if tuple(receipt) != (
            ...
            "fixtureProjectionCommitment", "results", "resultsCommitment",
            "receiptCommitment"):
        findings.append("RT20-EP8-RECEIPT: closed ordered fields drifted")
    if list(receipt.get("requiredCheckerApi", ())) != \
            list(EXPECTED_EP8_REQUIRED_API):
        findings.append("RT20-EP8-RECEIPT: exact nine-name authority drifted")
    expected_counts = {name: (2 if name == "encode_semantic_object_binding" else 1)
                       for name in EXPECTED_EP8_REQUIRED_API}
    if dict(receipt.get("exactCallCounts", {})) != expected_counts:
        findings.append("RT20-EP8-RECEIPT: exact invocation counts drifted")
    try:
        goldens = snapshot.candidate["verifiedSemanticRtApiContract"][
            "authenticatedLoaderBindingV6"]["ep8Closure"]["resultGoldens"]
        if receipt["resultsCommitment"] != goldens["resultsCommitment"] or \
                ...
                snapshot.fixture_projection["projectionCommitment"]:
            findings.append("RT20-EP8-RECEIPT: executable commitment join drifted")
    except (KeyError, TypeError):
        findings.append("RT20-EP8-RECEIPT: executable commitment join absent")
    ...
    if any(callable(value) for _path, value in receipt_children):
        findings.append("RT20-EP8-RECEIPT: direct callable retained")
    if set(snapshot.operations) != {
            "derive_operational_custody_projection_v1",
            "validate_raw_object_resolution_v1", "reduce_semantic_lease_v3",
            "is_retention_local_unavailable_v1"}:
        findings.append("RT20-SNAPSHOT-SURFACE: operation allowlist drifted")
    if count < 200:
        findings.append("RT20-SNAPSHOT-GRAPH-QUARANTINE: traversal was incomplete")
    return findings
...
def _candidate_mutations() -> list[Mutation]:
    ...
    return [
        ...
        ("graph edge omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["graphQuarantine"]["edgeRegistry"].pop()),
        ("runtime kind omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["graphQuarantine"]["runtimeKindRegistry"].pop()),
        ("semantic child omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["graphQuarantine"]["semanticChildDenominator"].pop()),
        ("oracle exception omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["graphQuarantine"]["referentOracle"]["exceptionRegistry"].pop()),
        ("positive case omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["graphQuarantine"]["independentPositiveDenominator"]["caseIds"].pop()),
        ("identity custody mode", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["graphQuarantine"]["identityCustody"].__setitem__("mode", "BARE-ID-SET")),
        ("EP8 required API omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["ep8Closure"]["requiredCheckerApi"].pop()),
        ("EP8 forbidden API extra", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["ep8Closure"]["requiredCheckerApi"].append("check")),
        ("EP8 call count", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["ep8Closure"]["exactCallCounts"].__setitem__("validate_bundle", 0)),
        ("EP8 receipt commitment", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["ep8Closure"]["resultGoldens"].__setitem__("receiptCommitment", "sha256:" + "0" * 64)),
        ("fixture projection commitment", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["ep8Closure"]["resultGoldens"].__setitem__("fixtureProjectionCommitment", "sha256:" + "0" * 64)),
        ("v1.10 real dataclass", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["temporaryVerifiedModuleExecutions"][0].__setitem__("isDataclass", False)),
        ("v1.10 temporary cleanup", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["temporaryVerifiedModuleExecutions"][0].__setitem__("restoredToAbsent", False)),
        ("v1.10 decorator interposition", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV6"]["d9Interpositions"].append({"source": "check-d9-v1.10.py", "kind": "dataclass-decorator"})),
...
def selftest(candidate: dict[str, Any], candidate_source: bytes,
    ...
    if candidate_source != pretty(expected) or candidate_source != pretty(candidate):
        print("REFUSING to self-test: base is not exact canonical RT20 candidate")
        return 1
    ...
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
          f"{len(GRAPH_POSITIVE_CASE_IDS)}/{len(GRAPH_POSITIVE_CASE_IDS)} "
          "helper-root + "
          f"{len(GRAPH_POSITIVE_CASE_IDS)}/{len(GRAPH_POSITIVE_CASE_IDS)} "
          "actual-snapshot-root positive denominator cases")
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
          "20/20 helper + 20/20 actual-root mappingproxy id-reuse probes")
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
          f"{len(GRAPH_RUNTIME_KIND_REGISTRY)}/"
          f"{len(GRAPH_RUNTIME_KIND_REGISTRY)} runtime-kind + "
          f"{len(GRAPH_SEMANTIC_CHILD_DENOMINATOR)}/"
          f"{len(GRAPH_SEMANTIC_CHILD_DENOMINATOR)} semantic-child-row + "
          f"{len(GRAPH_REFERENT_ORACLE_EXCEPTIONS)}/"
          f"{len(GRAPH_REFERENT_ORACLE_EXCEPTIONS)} oracle-exception + "
          f"{len(GRAPH_POSITIVE_CASE_IDS)}/"
          f"{len(GRAPH_POSITIVE_CASE_IDS)} positive-case deletions controlled")
    ...
    print(
        ...
        "attack, budget, exception, and deletion checks passed; exact "
        f"{len(GRAPH_POSITIVE_CASE_IDS)}/{len(GRAPH_POSITIVE_CASE_IDS)} "
        "helper-root and actual-snapshot-root positive denominator plus 20/20 "
        "helper-root and actual-snapshot-root identity-reuse probes passed"
    )
    return 0
...
def main(argv: list[str]) -> int:
    try:
        ...
        print(f"cannot freeze RT20 authority: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"cannot load RT20 candidate: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        findings.append(f"RT20-AUTHORITY-END: {exc}")
    if findings:
    ...
    raw = candidate["verifiedSemanticRtApiContract"][
        "rawObjectResolutionConformance"]["fixtureGoldens"]
    print(
        f"RT20 contract OK - {path.name}; {len(PINS)} frozen inputs read once; "
