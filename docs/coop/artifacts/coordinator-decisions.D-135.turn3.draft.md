...
Turn-1 subject `coordinator-decisions.D-135.draft.md`
`ec908ac522b973fd32af2b4b1cc282a94e4fde6b35979c110c66d38f13093990`
and turn-2 subject `coordinator-decisions.D-135.turn2.draft.md`
`e7ad37b6ef638bdcce5096b5483aeeeb08f75513755d33b665b4130ef218da78`
held frozen. Claude 2 turn 2 OBJECT, 0 MUST-FIX, 1 SHOULD-FIX
D-135-T2-SF-1. Codex turn 2 CONSENT, 0 MUST-FIX, 0 SHOULD-FIX.

| ID | Sev | Disposition |
|---|---|---|
| D-135-T2-SF-1 | SHOULD-FIX | ACCEPTED. Arithmetic now states 63 = 12 inherited + 30 V2 + 5 five-review + 16 DR-011 residuals (gates excluded), verified at commit `b9c2490`. Codex D135-A1's 71 is disposed: it counts 22 gates in place of the 16 residuals and does not reproduce the written 63. 65 is not switched to 71. |
| D135-A1 (Codex turn 1) | advisory | ANSWERED. 71 does not reproduce the written 63. Composition above is the adjudication. |
| CLAUDE-D135-ADV-1 | advisory | DISPOSITIONED, not performed. Neither new row adds an architecture-section pin in this cycle. |
...
| Turn-1 subject (frozen) | `ec908ac522b973fd32af2b4b1cc282a94e4fde6b35979c110c66d38f13093990` |
| Claude 2 turn-1 verdict | `76c2d0f9c4a4f08e8b1bb52b7a3b892e34a1bf90071eb6bba8b9e3b3ecf0b620` |
| Codex turn-1 verdict | `024d8f9b73480e2e7b474da6fee58c94b4138cfc35f98db7f6aa5e38926c93bd` |
| Turn-2 subject (frozen) | `e7ad37b6ef638bdcce5096b5483aeeeb08f75513755d33b665b4130ef218da78` |
| Claude 2 turn-2 verdict | `91bb26d37a668e0639cfb4efdc978f9f0c5d1bf464654b8b5ad8321de289fcc6` |
| Codex turn-2 verdict | `85a3a6c8a2e8bf7bde437d2f6f1c868a6bad88bf11c05a6b868cd3703fa79a8e` |

If a cited file moves in a way that is not append-only COORD
growth with file 08, file 12, the frozen turn-1 subject, the
frozen turn-2 subject, and this draft unmoved,
...
3. **Rewrite only these snapshot bytes:**
   - Heading: `### Current position — measured snapshot, 2026-08-14`
     → `### Current position — measured snapshot, 2026-08-15`.
   - Preamble: `reconstructing it from 63 rows` →
     `reconstructing it from 65 rows`.
...
`| DR-131 | Preview non-authoritative \`analyze\` product contract | Product + CLI / output | [\`COORDINATOR-DECISIONS.md\`](../../coop/COORDINATOR-DECISIONS.md) D-132 (user-made ADOPTED, commit \`d3efe3c…\`); D-134 (CONSENT Claude \`0b672021…\` / Codex \`ec305159…\`); recorded goal [file 12 §3](12-architecture-completion-goal.md) (file 12 has no authority) | Independently reviewed contract that freezes: single first-party bundled declarative pack name/version; sealed facts+Coverage inputs; PlanId membership of the pack; core-evaluated \`policyOutcome\`; typed-indeterminate on a missing required rung; which preview identities are unstable; upgrade path to a sealed Run; citation of D-077 / D-078. The cell excludes human/machine JSON schema and exit generics already SATISFIED at DR-123 (D-092). | **OPEN** — no contract exists. Not eligible in kind today (D-133). Not SATISFIED. | Hard blocker for preview \`analyze\`; condition 2 SATISFIED-requiring per D-134 |`

DR-133:

`| DR-133 | Provider-only TypeScript component output | Semantic / component architecture | [\`COORDINATOR-DECISIONS.md\`](../../coop/COORDINATOR-DECISIONS.md) D-132 (user-made ADOPTED, commit \`d3efe3c…\`); D-134 (CONSENT Claude \`0b672021…\` / Codex \`ec305159…\`); recorded goal [file 12 §4](12-architecture-completion-goal.md) (file 12 has no authority) | Independently reviewed contract stating: the TypeScript component returns semantic facts and Coverage only; the host owns rules, policy, findings, and admission; component-emitted findings are refused, with retained negative tests. | **OPEN** — no contract exists. Not eligible in kind today (D-133). Not SATISFIED. | Hard blocker for preview \`analyze\`; condition 2 SATISFIED-requiring per D-134 |`

## Arithmetic (measured, not a definition)

The live preamble figure `63 rows` is 12 inherited + 30 V2 +
5 five-review + 16 DR-011 residual rows, gates excluded. That
composition is exact at commit `b9c2490`, which introduced
the phrase. Codex turn-1 advisory D135-A1's 71 counts the 22
gate rows in place of the 16 residuals and does not
reproduce the written 63; it is answered, not adopted. This
edit adds two V2 rows, so the preamble becomes 65
(12 + 32 + 5 + 16). Do not write 71.
...
Total only before a dependent DR-131/DR-133 contract, status
re-record, or other act lands. At that stage overturn
**removes** the two inserted rows and restores the snapshot
heading, `63 rows` figure, condition-2 count/clause, and
"What that means" sentence. After a dependent act lands,
overturn also requires that act's owning-entry supersession
and reconciliation of every dependent MF-6 note under its
own reviewed act. Overturn: C-D135. Does not overturn
D-132, D-133, or D-134 unless separately overturned.
