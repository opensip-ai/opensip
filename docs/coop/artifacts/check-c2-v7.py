#!/usr/bin/env python3
"""Retained executable checker for the C-2 plan/stage contract, v7.

WHY v7 EXISTS
-------------
`1.0 == 1` has now defeated four consecutive artifacts written to defeat it,
...
c2-plan-stage-schema.v6.review-independent.json, at
cb2e50e19e38fb786990852758a29d7fb1479688fe81f5851b926e09b448b13b, is a REJECT
on ONE blocking finding, and it is a far narrower rejection than the one before
it: the reviewer attacked `jx_canon` directly with 82,368 distinct tokens of
hostile and random input and could not break its injectivity, confirmed the
primitive stands alone, ran the 109-row matrix to termination GREEN, and swept
137 candidate mutations with 0 admitted.  NONE OF THAT IS REBUILT HERE.  What
is repaired is the one thing none of it touched:

    c2-plan-stage-schema.v6.json
      insert `  "version": 6.0,` immediately BEFORE the existing
      `  "version": 6,`  --  eighteen bytes, no source modification
    python3 -I -B check-c2-v6.py <that file>   ->   exit 0, full green banner,
      including `105 integer leaves of THIS document bound ... with no unbound
      bucket`, over a document whose BYTES carry a float at the one key whose
      integer-ness that checker enforces by an explicit named rule.

`json.loads` keeps the LAST occurrence of a duplicated key, so the parsed object
is byte-for-byte the parse of the unedited file.  Every layer of v6 -- the
primitive included -- operates on that parsed object, and NOTHING read the
candidate's raw bytes.  A reader of the file sees a float; every instrument sees
an int.  That vector is retained here as a live executable case: the mechanism
runs on EVERY invocation (L8), and the full execution of the pinned, rejected
check-c2-v6.py over the eighteen-byte edit is a retained --selftest row.

WHAT WENT WRONG FOUR TIMES, AND THE TWO THINGS v7 DOES DIFFERENTLY
-----------------------------------------------------------------
...
layer inherits without choosing to.  So v7 keeps v6's design entirely and adds
exactly one thing to it: the parse is a gate too, `jx_loads` is that gate, it
lives INSIDE the portable primitive so an adopter cannot take the comparison and
leave the parse behind, and a STRUCTURAL scan of this file's own tree requires
EVERY json.load/json.loads call to pass through it -- not a list of the inputs
somebody remembered.

v7 does not add another guard.  It builds ONE primitive -- the `jx` block in
...
The last three rows are v7's.  The truthiness and stringification rows were
omitted from v6's table, which called itself the operator space, and the
...
truthiness is emptiness and not numeric value.  That scope is RES-C2V7-09.
...
v7 does NOT repair them and does not grade them.  To adopt:
...
instead of a defect a reader has to discover by trying it.
  ...
  2. PARSE EVERY JSON INPUT WITH `jx_loads`, NOT ONLY THE ONE YOU CALL THE
     ...
     `problems` is visible in its own source.  Then copy `json_load_sites` and
     require every `json.load`/`json.loads` in YOUR tree to pass an
     `object_pairs_hook` -- the property has to be structural, because a parse
     defence applied to one input and not its siblings is the list-of-places
     failure this lineage exists to escape.  This step is IR-C2V6-01.
...
----------------------------------------------------------------------------
This was posed as an open design question and v7 answers it in code.  The
comparison exists to catch contract drift and it is also what creates the
false-accept surface.  v7 keeps the comparison but removes the surface: the
...
as RES-C2V7-04, and is not repaired here.
...
1. `jx_canon` diverges from Python `==` in exactly two places, both measured and
   ...
   only the equality half of that.  v7 does not disclose the ordering half; it
...
2. L4 is still SYNTACTIC and its excusal test is still textual.  MEASURED THIS
   ...
   MODEL, not the excusal test.  v7 extends taint to comprehension, generator,
...
4. L4 cannot see `operator.ne` unless the name is visible.  v7 treats
...
6. v7 does not re-run check-c2-v4.py --selftest, the predecessor's ~24k-case
   contract-root execution matrix.  Retained as RES-C2V7-03.
7. The type lock closes JSON TYPE.  A type-correct integer with a wrong VALUE at
   a position no layer measures is outside it.  v7 narrows that gap by requiring
   ...
   value drift outside the counter register.  Retained as RES-C2V7-05.
...
10. L8 covers ONE class -- "the bytes on disk and the object every guard reads
   ...
   are their own reviewed bytes and are not v7's to change.  RES-C2V7-08.
11. L9 closes "a declared string that names something that exists NOWHERE".  It
   ...
   visible rather than described.  RES-C2V7-10.
12. Everything this run does NOT recompute is in RES-C2V7-11, in the artifact,
...
Usage: python3 -I -B artifacts/check-c2-v7.py [contract]  ·  --selftest
...
BINDING = "c2-plan-stage-schema.v7.json"
DECLARED_FLAGS = ("--selftest",)
DECLARED_EXIT_CODES = frozenset({"0", "1", "2", "3"})
HERE = pathlib.Path(__file__).resolve().parent
ARTIFACT_ID = "opensip.c2-plan-stage-schema.v7"
...
def _jx_duplicate_paths(node, steps, marks, out) -> None:
    ...
    kind = jx_type(node)
    if kind == "object":
        for key in jx_get(marks, id(node), []):
            out.append({"kind": "duplicate-key", "path": list(steps) + [key],
                        "key": key, "offset": -1, "token": "",
                        "detail": "is published more than once in the bytes; the host "
                                  "parser keeps the LAST occurrence, so the parsed "
                                  "object cannot say what the document says"})
        for key in list(node):
            _jx_duplicate_paths(jx_at(node, key), list(steps) + [key], marks, out)
    elif kind == "array":
        for index, item in enumerate(node):
            _jx_duplicate_paths(item, list(steps) + [index], marks, out)
...
def jx_selftest() -> dict:
    ...
    for label, produce, expected in (
            ...
            # v7: ordering now agrees with equality at every pair, and the two
...
# load, not reported.  v7 discharges TWO dispositions and both are pinned.
...
# The ONE blocking finding of the v6 REJECT.  v7 exists to discharge it, and
...
class Authority:
    ...
    def __init__(self, snapshots, parsed, v4, v4_authority, directory):
        ...
        self.candidate_number_lock = {"executedCases": 0, "namedRejections": 0,
                                      "admissions": 0, "escapes": []}
...
def load_authority(directory: pathlib.Path = HERE) -> Authority:
    ...
    with redirect_stdout(sink):
        v4 = _execute_snapshot("opensip_c2v7_pinned_v4_checker", V4_CHECKER,
                               snapshots[V4_CHECKER], directory)
        v4._OWN_TREE_CACHE = ast.parse(snapshots[V4_CHECKER])
        v3_module = v4._execute_snapshot("opensip_c2v7_pinned_v3_checker", V3_CHECKER,
...
def _refuse_softened_provenance(parsed) -> None:
    """Both dispositions v7 discharges are read from pinned bytes, by name."""
    ...
    # The v6 REJECT.  v7 exists to discharge ONE blocking finding and refuses to
...
#
# What L8 does NOT cover is in the contract's L8 blindSpots and in RES-C2V7-08:
# string-escape spellings, and the parses performed INSIDE the pinned v4 and v5
# checkers, which are their own bytes and are not v7's to change.
# =============================================================================

# (id, pinned document, the line to duplicate, the shadowing line to insert
#  BEFORE it, the position v7 must name, note)
...
_PARSE_FINDING_IDS = {"duplicate-key": "C2V7-DUPKEY",
                      "non-rfc-constant": "C2V7-NONRFC",
                      "number-text": "C2V7-NUMBER-TEXT"}
...
def parse_problem_findings(problems, label) -> list:
    ...
    for problem in problems:
        ...
        prefix = jx_get(_PARSE_FINDING_IDS, kind, "C2V7-PARSE")
        findings.append(
...
# The ONLY places this file is permitted to parse JSON the way the REJECTED
# predecessor does -- with the LAST duplicate key silently winning.  Both exist
# to reproduce the predecessor's own behaviour as evidence, and both are named,
# counted and published, exactly as DECLARED_EXEC_SITES is.  Anywhere else, a
# bare parse is a finding.
...
def parse_scan_findings(tree, authority) -> dict:
    ...
    declared = [site for site in own if site["inDeclaredHazardTable"]]
    reproductions = [site for site in own if site["inDeclaredPredecessorParse"]
                     and not site["hooked"]]
    predecessors = {}
    for name in (V4_CHECKER, V5_CHECKER, V6_CHECKER):
    ...
    return {"sites": len(own), "ungated": ungated, "declaredHazardSites": len(declared),
            "declaredPredecessorParses": len(reproductions),
            "hooked": len([site for site in own if site["hooked"]]),
...
def parse_probe(label="probe") -> dict:
    ...
    for name, text, kind, position in JX_PARSE_CORPUS:
        ...
        if not kind:
            ...
            continue
        expected = jx_get(_PARSE_FINDING_IDS, kind, "C2V7-PARSE")
...
def parse_differential(authority) -> dict:
    ...
    for vector, document, needle, insert, position, note in PARSE_DIFFERENTIAL_VECTORS:
        ...
        hit = [item for item in findings
               if item.startswith("C2V7-DUPKEY:") and position in item]
        if hit:
            named += 1
        else:
...
def parse_integrity_findings(authority, tree) -> list:
    ...
    for site in scan["ungated"]:
        findings.append(
            "C2V7-PARSE-SCAN: line " + str(site["line"]) + " parses JSON without an "
    ...
    if not jx_int_in_range(scan["sites"], 1, 10 ** 6):
        findings.append("C2V7-PARSE-SCAN: this file appears to parse no JSON at all, so "
    ...
    if not jx_int_in_range(scan["declaredHazardSites"], 1, 10 ** 6):
        findings.append("C2V7-PARSE-SCAN: the operator space no longer demonstrates the "
                        "bare-parse hazard live, so the row that covers it proves "
                        "nothing")
    if not jx_int_in_range(scan["declaredPredecessorParses"], 1, 10 ** 6):
        findings.append("C2V7-PARSE-SCAN: this file no longer reproduces the "
    ...
    for name in jx_sorted(list(scan["predecessors"])):
        ...
        if not jx_int_in_range(entry["sites"], 1, 10 ** 6):
            findings.append("C2V7-PARSE-SCAN: the pinned " + name + " reports no JSON "
                            "parse site at all, so the scan cannot be shown to see the "
                            "shape it exists to catch")
        if jx_bind(entry["hooked"], 0) is not None:
            findings.append("C2V7-PARSE-SCAN: the pinned " + name + " now hooks " +
                            str(entry["hooked"]) + " of its parses, so the external "
                            "anchor for IR-C2V6-01 has changed and the differential is "
                            "no longer measuring a known-defective instrument")
    probe = parse_probe(authority.document_name)
    authority.parse_probe = probe
    findings.extend("C2V7-PARSE-PROBE: " + item for item in probe["escapes"])
    if not jx_int_in_range(probe["executedCases"], 1, 10 ** 6):
        findings.append("C2V7-PARSE-PROBE: the parse layer probed no case, so it is a "
                        "claim over an unobserved region")
    if jx_bind(probe["admissions"], 0) is not None:
        findings.append("C2V7-PARSE-PROBE: " + str(probe["admissions"]) + " byte/parse "
    ...
    findings.extend("C2V7-PARSE-DIFFERENTIAL: " + item for item in differential["escapes"])
    if jx_bind(differential["predecessorParseBlind"], differential["vectors"]) is not None:
        findings.append(
            "C2V7-PARSE-DIFFERENTIAL: " + str(differential["predecessorParseBlind"]) +
    ...
    if jx_bind(differential["successorNamedByPosition"],
        ...
        findings.append(
            "C2V7-PARSE-DIFFERENTIAL: " + str(differential["successorNamedByPosition"]) +
...
# =============================================================================
# Section 3.  The derivation.  The effective v7 contract is the VERIFIED v4
        ...
        try:
            if jx_equal(kind, "set"):
                ...
                if not jx_equal(current, op.get("from")):
                    findings.append(
                        "C2V7-DERIVATION: operation " + str(index) + " at " + path +
                ...
                if jx_has_at(parent, steps[-1]):
                    findings.append("C2V7-DERIVATION: operation " + str(index) + " adds " +
...
def project_to_v4_identity(effective, projection):
    ...
    if jx_type(fields) != "object" or not fields:
        return projected, ["C2V7-PROJECTION: v4InheritanceProjection.fields must be a "
                           "non-empty JSON object"]
    if not jx_equal(fields, dict(DECLARED_PROJECTION_FIELDS)):
        findings.append(
            "C2V7-PROJECTION: the declared projection is " + repr(fields) + "; this "
    ...
    for path in jx_sorted(list(fields)):
        try:
            ...
            findings.append("C2V7-PROJECTION: " + path + " does not resolve (" +
                            type(exc).__name__ + ")")
    drift = jx_sorted(_wire_diff_paths(effective, projected))
    declared = jx_sorted(list(fields))
    if not jx_equal(drift, declared):
        findings.append("C2V7-PROJECTION: the projection handed to the inherited oracle "
...
def measure(effective, authority):
    ...
    v7 authors the comparison; v4 -- reviewed, pinned, and confirmed sound at
    ...
    a trust relationship in RES-C2V7-04.
    """
    v4 = authority.v4
...
def census_comparison_findings(effective, measurement) -> list:
    ...
    for position in jx_sorted(list(positions)):
        ...
        if not present:
            findings.append("C2V7-CENSUS: " + position + " is measured by this run at " +
        ...
        if not jx_int(measured_value):
            findings.append("C2V7-INSTRUMENT: " + position + ": " + reason)
        elif not jx_int(published):
            findings.append("C2V7-TYPE: " + position + ": " + reason)
        else:
            findings.append("C2V7-CENSUS: " + position + ": " + reason)
    return findings
...
def register_findings(effective, measurement, authority) -> list:
    ...
    spec = spec.get("integerConstantRegisterV7") if jx_type(spec) == "object" else None
    if jx_type(spec) != "object":
        findings.append("C2V7-REGISTER: the effective contract declares no "
                        "planIntent.integerConstantRegisterV7; the census counters the "
    ...
    if not jx_equal(jx_sorted(jx_unique(declared_inherited)),
        ...
        findings.append(
            "C2V7-REGISTER: the inherited half of the register is " +
    ...
    if missing:
        findings.append("C2V7-REGISTER: " + str(len(missing)) + " published counter "
                        "position(s) are measured and compared but are not registered, "
                        "so a site could be added silently: " + repr(missing[:6]))
    if extra:
        findings.append("C2V7-REGISTER: " + str(len(extra)) + " registered position(s) "
    ...
    if count_reason is not None:
        findings.append("C2V7-REGISTER: censusCounterPositionCount: " + count_reason)
    ...
    if uncovered or not jx_int_in_range(len(covered), 1, 10 ** 9):
        findings.append(
            "C2V7-REGISTER: the per-comparison-site partition covers " +
    ...
    if "type" not in rule or "before" not in rule:
        findings.append("C2V7-REGISTER: the register does not state plainly that the "
...
def _number_leaf_steps(node, steps=()) -> list:
    """Every path holding a JSON NUMBER -- a float -- as a list of steps.

    v6's totality over the candidate enumerated INTEGER leaves only, so adding
    `"reviewerInjectedLeaf": 17` was refused by C2V6-UNBOUND and adding
    `"reviewerInjectedLeaf": 17.0` was admitted to a full green run.  The rule
    was honestly scoped and the scope was wrong: this artifact exists because of
    int/float equivalence, and the one JSON type it left unbound was float.  v7
    closes the gap rather than rewording the blind spot.  The candidate holds no
    JSON number at all, so this is a totality assertion and not a table.
    """
    out = []
    if jx_type(node) == "object":
        for key in list(node):
            out.extend(_number_leaf_steps(jx_at(node, key), list(steps) + [key]))
    elif jx_type(node) == "array":
        for index, value in enumerate(node):
            out.extend(_number_leaf_steps(value, list(steps) + [index]))
    elif jx_type(node) == "number":
        out.append(list(steps))
    return out
...
def document_type_findings(effective, locked, prefix="C2V7-TYPE") -> list:
    findings = []
    for steps in locked:
...
def document_type_probe(effective, locked, prefix="C2V7-TYPE") -> dict:
...
def document_type_lock_findings(base, effective, authority) -> list:
    ...
    for label, document in (("verified predecessor", base),
        ...
        if missing:
            findings.append(
                "C2V7-DOCLOCK: " + str(len(missing)) + " integer leaf/leaves of the " +
    ...
    findings.extend("C2V7-DOCLOCK: " + item for item in probe["escapes"])
    if not jx_int_in_range(probe["executedCases"], 1, 10 ** 9):
        findings.append("C2V7-DOCLOCK: the document-wide type lock probed no position, "
                        "so it is a claim over an unobserved region")
    if jx_bind(probe["admissions"], 0) is not None:
        findings.append("C2V7-DOCLOCK: " + str(probe["admissions"]) + " float "
    ...
    if jx_bind(probe["namedTypeRejections"], probe["executedCases"]) is not None:
        findings.append("C2V7-DOCLOCK: " + str(probe["namedTypeRejections"]) + " of " +
...
CANDIDATE_COUNTER_ALIASES = (
    ...
    (["theSuccessorDefect", "measuredThisRun", "v7RejectedByName"],
     "successorV7RejectedByName"),
    ...
    (["theParseDefect", "measuredThisRun", "v7NamedByPosition"],
...
def candidate_bindings(c, live, authority, base, measurement):
    ...
    bind(["version"], "identity", 7, "this checker's declared successor identity")
    bind(["supersedes"], "identity", 6,
    ...
    for index, op in enumerate(operations):
        ...
        for sub in _integer_leaf_steps(op.get("from")):
            try:
                ...
                findings.append(
                    "C2V7-UNBOUND: derivedFrom/operations/" + str(index) + "/from/" +
        ...
        for sub in _integer_leaf_steps(op.get("value")):
            ...
            if expected is None:
                findings.append(
                    "C2V7-UNBOUND: the derivation writes a JSON integer into the "
    ...
    for key in jx_sorted(list(live)):
        bind(["v7MeasuredCounters", key], "measured", jx_get(live, key),
             "the value this run measured for " + key)
    return bindings, findings
...
def candidate_lock_findings(c, live, authority, base, measurement) -> list:
    ...
    for steps in unbound:
        findings.append(
            "C2V7-UNBOUND: the candidate publishes a JSON integer at " +
    ...
    for steps in jx_sorted(jx_difference(_number_leaf_steps(c), bound_paths)):
        findings.append(
            "C2V7-UNBOUND: the candidate publishes a JSON number at " +
            _steps_text(steps) + " that no layer of this run binds; v6 bound its "
            "INTEGER leaves and left the float ones outside the totality, so a new "
            "leaf spelled 17 was refused and the same leaf spelled 17.0 reached a "
            "full green run - in the artifact that exists because 1.0 equals 1")
    ...
    for steps in jx_sorted(jx_unique(duplicates)):
        findings.append("C2V7-UNBOUND: " + _steps_text(steps) + " is bound twice, so "
                        "one of the two bindings is not the one being enforced")
    for steps in jx_sorted(jx_difference(bound_paths, leaves)):
        findings.append(
            "C2V7-UNBOUND: this run binds " + _steps_text(steps) + " but the candidate "
    ...
    for record in jx_sorted_by(bindings, "steps"):
        ...
        if reason is None:
            continue
        prefix = "C2V7-TYPE" if not jx_int(published) else "C2V7-CANDIDATE"
        findings.append(prefix + ": " + _steps_text(steps) + " (" + kind + "): " +
                        reason + "; the expectation is " + source)
    number_probe = candidate_number_probe(c, bindings)
    authority.candidate_number_lock = number_probe
    findings.extend("C2V7-CANDNUM: " + item for item in number_probe["escapes"])
    if not jx_int_in_range(number_probe["executedCases"], 1, 10 ** 9):
        findings.append("C2V7-CANDNUM: the candidate's float-leaf totality probed no "
                        "position, so the claim that a NEW leaf spelled 17.0 is refused "
                        "is a claim over an unobserved region - which is exactly what it "
                        "was in v6, where it was false")
    if jx_bind(number_probe["admissions"], 0) is not None:
        findings.append("C2V7-CANDNUM: " + str(number_probe["admissions"]) + " JSON "
                        "number leaf/leaves nobody binds were ADMITTED")
    probe = candidate_type_probe(c, bindings)
    authority.candidate_lock = probe
    findings.extend("C2V7-CANDLOCK: " + item for item in probe["escapes"])
    if not jx_int_in_range(probe["executedCases"], 1, 10 ** 9):
        findings.append("C2V7-CANDLOCK: the candidate lock probed no position, so it is "
                        "a claim over an unobserved region")
    if jx_bind(probe["admissions"], 0) is not None:
        findings.append("C2V7-CANDLOCK: " + str(probe["admissions"]) + " hostile "
                        "spelling(s) at the candidate's own integer leaves were admitted")
    return findings


CANDIDATE_NUMBER_PROBE_KEY = "c2v7ProbeNumberLeaf"
CANDIDATE_NUMBER_PROBE_VALUES = (17.0, -0.0, 1.5)
CANDIDATE_NUMBER_PROBE_TARGETS = ([], ["theDefect"], ["v7MeasuredCounters"])


def candidate_number_probe(c, bindings) -> dict:
    """Behavioural, reads no source.  A JSON NUMBER at a key nobody binds must
    be a named finding, at the root and at depth.

    v6's totality enumerated INTEGER leaves, so this exact probe would have been
    ADMITTED there: `"reviewerInjectedLeaf": 17` was refused and
    `"reviewerInjectedLeaf": 17.0` reached a full green run.  The clean candidate
    holds no JSON number at all, so without this probe the float half of the
    totality would have no positive case and breaking it would be silent.
    """
    bound_paths = [record["steps"] for record in bindings]
    cases = named = admitted = 0
    escapes = []
    for target in CANDIDATE_NUMBER_PROBE_TARGETS:
        try:
            node = _resolve_steps(c, target)
        except MALFORMED_SHAPE_EXCEPTIONS:
            escapes.append(_steps_text(target) + ": the probe target does not resolve in "
                           "this candidate, so the float half of the totality is "
                           "unobserved there")
            continue
        if jx_type(node) != "object" or jx_has(node, CANDIDATE_NUMBER_PROBE_KEY):
            ...
            continue
        for value in CANDIDATE_NUMBER_PROBE_VALUES:
            cases += 1
            expected = list(target) + [CANDIDATE_NUMBER_PROBE_KEY]
            jx_put(node, CANDIDATE_NUMBER_PROBE_KEY, value)
            try:
                unbound = jx_difference(_number_leaf_steps(c), bound_paths)
                if jx_in(expected, unbound):
                    named += 1
                else:
                    admitted += 1
                    escapes.append(_steps_text(expected) + ": a JSON number " +
                                   repr(value) + " that no layer binds was ADMITTED; "
                                   "OBS-C2V6-01 measured exactly this against v6 and it "
                                   "reached a full green run")
            finally:
                del node[CANDIDATE_NUMBER_PROBE_KEY]
...
def _gate_dominates(site, wire_node, gates, parents) -> bool:
    ...
    nothing and that the real hazard was the taint model.  v7 publishes the
...
def bare_truthiness_sites(tree) -> list:
    ...
    the honest scope is in RES-C2V7-09 rather than in a guard that would have to
...
def scan_findings(authority, self_tree=None) -> list:
    ...
    self_scan = wire_comparison_scan(tree, "check-c2-v7.py")
    ...
    for site in self_scan["sites"]:
        findings.append(
            "C2V7-SCAN: " + site["function"] + " line " + str(site["line"]) + " (" +
    ...
    if not jx_int_in_range(self_scan["gateCallSites"], 8, 10 ** 6):
        findings.append("C2V7-SCAN: only " + str(self_scan["gateCallSites"]) +
    ...
    for key, allowed, why in (
        ...
        if jx_bind(jx_get(primitives, key), allowed) is not None:
            findings.append("C2V7-SCAN: this file carries " +
    ...
    if jx_bind(self_scan["functionLikeNodes"], len(walked) - len(gated)) is not None:
        findings.append(
            "C2V7-SCAN-BLIND: the scan reports " + str(self_scan["functionLikeNodes"]) +
    ...
    if not jx_int_in_range(len(adjudicated), 1, 10 ** 6):
        findings.append(
            "C2V7-SCAN-BLIND: the pinned adjudication yields no comparison line at all, "
    ...
    if unseen:
        findings.append(
            "C2V7-SCAN-BLIND: run over the pinned predecessor this scan does NOT report "
    ...
    if not jx_in(V5_DEFECT_LINE, v5_found):
        findings.append(
            "C2V7-SCAN-BLIND: run over the pinned check-c2-v5.py this scan does NOT "
            ...
            "all; if v7's model cannot see it, v7's model is v5's model")
    if not jx_int_in_range(predecessor_scan["ungatedComputedOperandComparisons"],
                           1, 10 ** 6):
        findings.append("C2V7-SCAN-BLIND: the scan reports no computed-operand "
    ...
    if not jx_int_in_range(jx_get(truthiness, V6_CHECKER), 1, 10 ** 6):
        findings.append("C2V7-SCAN-BLIND: the bare-truthiness model reports nothing at "
...
# somewhere in the file.  That residual is RES-C2V7-10.
# =============================================================================
...
def declared_string_findings(c, authority, tree) -> list:
    ...
    findings = ["C2V7-DECLARED-STRING: " + item for item in result["escapes"]]
    if not jx_int_in_range(result["rows"], 1, 10 ** 6):
        findings.append("C2V7-DECLARED-STRING: there are no retained assertion rows to "
                        "check, so this layer is a statement about an empty set")
    if not jx_int_in_range(result["namesChecked"], 1, 10 ** 6):
        findings.append("C2V7-DECLARED-STRING: no primitive name is named by the "
                        "docstring or the candidate, so the adoption-instruction check "
                        "detects nothing")
    if not jx_int_in_range(result["atEmitSite"], 1, 10 ** 6):
        findings.append("C2V7-DECLARED-STRING: not one declared subject is proved at "
    ...
    if jx_bind(result["unresolved"], 0) is not None:
        findings.append("C2V7-DECLARED-STRING: " + str(result["unresolved"]) +
                        " declared subject(s) name a string that exists nowhere")
    if jx_bind(result["namesUndefined"], 0) is not None:
        findings.append("C2V7-DECLARED-STRING: " + str(result["namesUndefined"]) +
...
def behavioural_layer(effective, measurement) -> dict:
    ...
    for position in jx_sorted(list(positions)):
        ...
        for label in HOSTILE_SPELLINGS:
            ...
            hit = [item for item in findings
                   if item.startswith("C2V7-TYPE:") and position in item]
            if hit:
                named += 1
                continue
            if not findings:
            ...
            escapes.append(position + "/" + label + ": rejected, but by " +
                           findings[0].split(":")[0] + " rather than a C2V7-TYPE "
    ...
    for position in jx_sorted(list(positions)):
        ...
        if [item for item in findings
                if item.startswith("C2V7-INSTRUMENT:") and position in item]:
            probes_named += 1
        else:
            probes_escaped += 1
...
def behavioural_findings(effective, measurement, authority, tree) -> list:
    ...
    if not jx_int_in_range(result["executedCases"], 1, 10 ** 9):
        findings.append("C2V7-BEHAVIOUR: the behavioural layer executed no case at all")
    if jx_bind(result["admissions"], 0) is not None:
        findings.append("C2V7-BEHAVIOUR: " + str(result["admissions"]) +
    ...
    if jx_bind(result["rejectedWithoutNamingThePosition"], 0) is not None:
        findings.append(
            "C2V7-BEHAVIOUR: " + str(result["rejectedWithoutNamingThePosition"]) +
            " case(s) were rejected without a C2V7-TYPE finding naming the position; "
    ...
    for key, expected in (("tokenCollisions", 0), ("crossTypeAdmissions", 0),
        ...
        if jx_bind(jx_get(primitive, key), expected) is not None:
            findings.append("C2V7-PRIMITIVE: " + key + " is " +
    ...
    if jx_bind(primitive["roundTrips"], primitive["corpusValues"]) is not None or \
        ...
        findings.append("C2V7-PRIMITIVE: the canonical encoding is not invertible over "
    ...
    if jx_bind(primitive["operatorSpaceHazardsReproduced"],
        ...
        findings.append("C2V7-PRIMITIVE: the operator space is not fully demonstrated "
...
def predecessor_differential(authority) -> dict:
    ...
    for vector, position, spelling, note in DIFFERENTIAL_VECTORS:
        ...
        v7_findings = census_comparison_findings(mutant, measurement)
        v7_named = [item for item in v7_findings
                    if item.startswith("C2V7-TYPE:") and position in item]
        if not v4_named:
        ...
        if v7_named:
            ...
            escapes.append(vector + ": this checker did NOT reject " + position +
                           " spelled " + spelling + " with a C2V7-TYPE finding naming "
                           "the position")
        rows.append({"vector": vector, "position": position, "spelling": spelling,
                     ...
                     "successorNamedThePosition": bool(v7_named)})
...
def sweep_findings(authority) -> list:
    ...
    if jx_bind(sweep["integerLeavesInjected"], live_leaves) is not None:
        findings.append("C2V7-SWEEP: the sweep reports " +
    ...
    if jx_bind(total, sweep["integerLeavesInjected"]) is not None:
        findings.append("C2V7-SWEEP: admitted plus rejected is " + str(total) + " but " +
    ...
    if jx_bind(parts, sweep["admittedToAFullGreenRun"]) is not None:
        findings.append("C2V7-SWEEP: the census and non-census admissions sum to " +
    ...
    if not jx_int_in_range(sweep["admittedOutsideTheCensusBlock"], 1, 10 ** 6):
        findings.append("C2V7-SWEEP: no leaf OUTSIDE the census block drives the pinned "
    ...
    if missing:
        findings.append("C2V7-SWEEP: the sweep no longer finds " + repr(missing) +
...
def successor_differential(authority, candidate) -> dict:
    ...
    admits every one of these to a fully green run.  v7 must name each.
    """
    ...
    rows, escapes = [], []
    admitted = rejected = 0
    for vector, v5_path, v7_path, spelling, note in SUCCESSOR_DIFFERENTIAL_VECTORS:
        ...
        named = False
        try:
            current = _resolve_steps(mutant, v7_path)
        except MALFORMED_SHAPE_EXCEPTIONS:
            escapes.append(vector + ": " + _steps_text(v7_path) + " does not resolve in "
                           "this candidate, so the vector is not retained as an "
                           "executable case")
            rows.append({"vector": vector, "path": _steps_text(v7_path), "note": note,
                         "v5FullyGreen": v5_green, "v7NamedThePosition": False})
            continue
        value = float(current) if jx_equal(spelling, "float-equal") else \
            (True if jx_equal(spelling, "boolean-true") else 1)
        _assign_steps(mutant, v7_path, value)
        findings = candidate_probe_findings(mutant, authority)
        named = bool([item for item in findings if _steps_text(v7_path) in item])
        if named:
            rejected += 1
        else:
            escapes.append(vector + ": this checker did NOT name " +
                           _steps_text(v7_path) + " when it was spelled " + spelling)
        rows.append({"vector": vector, "path": _steps_text(v7_path), "note": note,
                     "v5FullyGreen": v5_green, "v7NamedThePosition": named})
    return {"vectors": len(SUCCESSOR_DIFFERENTIAL_VECTORS), "rows": rows,
            "v5AdmittedToAFullGreenRun": admitted, "v7RejectedByName": rejected,
            "escapes": escapes}
...
def _v5_admissions(authority) -> list:
    ...
    try:
        with redirect_stdout(sink):
            v5 = _execute_snapshot("opensip_c2v7_pinned_v5_checker", V5_CHECKER,
    ...
    for vector, v5_path, _v7_path, spelling, _note in SUCCESSOR_DIFFERENTIAL_VECTORS:
        mutant = copy.deepcopy(contract)
        try:
...
#
# In v7 there is no f-string in the banner.  Every line is a TEMPLATE rendered
...
def live_register(authority) -> dict:
    ...
    return {
        ...
        "candidateNumberProbeCases": authority.candidate_number_lock["executedCases"],
        "candidateNumberProbeNamed":
            authority.candidate_number_lock["namedRejections"],
        "candidateNumberProbeAdmissions":
            authority.candidate_number_lock["admissions"],
        ...
        "successorV7RejectedByName": succ["v7RejectedByName"],
...
BANNER_TEMPLATES = (
    "C-2 v7 contract OK - {documentName}; IR-C2V4-01 and the four blocking findings "
    "of the v5 REJECT repaired at the comparison primitive, IR-C2V6-01 repaired at the "
    "PARSE, {pinnedInputs} inputs hash-verified before execution, and the pinned "
    "check-c2-v4.py executed from its verified snapshot as the inherited oracle over "
    "the v4-identity projection",
    ...
    "named {successorV7RejectedByName} at the analogous position of its own document",
    ...
    "{parseJsonLoadSites} JSON parse call site(s) of which {parseHookedLoadSites} pass "
    "an object_pairs_hook and {parseUngatedLoadSites} do not, against "
    "{parsePredecessorHookedLoadSites} hooked of {parsePredecessorLoadSites} in the "
    "pinned check-c2-v6.py; {parseProbeCases} live parse probes including "
    "{parseProbeControls} controls, of which {parseProbeAdmissions} were admitted",
    ...
    "silently, and the scope is RES-C2V7-09 rather than a guard nobody could keep green",
...
def banner_findings(live, runtime) -> list:
    ...
    if not jx_int_in_range(len(banner_digit_tokens(BANNER_DETECTOR_PROBE)), 1, 10 ** 6):
        findings.append(
            "C2V7-BANNER: the bare-digit detector does not fire on " +
    ...
    if "{" in rendered[0] or "}" in rendered[0]:
        findings.append(
            "C2V7-BANNER: the renderer left an uninterpolated field in " +
    ...
    for token in jx_sorted(jx_unique(banner_digit_tokens(BANNER_TEMPLATES))):
        findings.append(
            "C2V7-BANNER: the banner carries the literal figure " + repr(token) +
    ...
    for key in unknown:
        findings.append("C2V7-BANNER: the banner interpolates " + key + ", which this "
                        "run does not measure")
    if not jx_int_in_range(len(jx_unique(referenced)), 1, 10 ** 6):
        findings.append("C2V7-BANNER: the banner interpolates nothing, so every figure "
                        "in it would be source text")
    declared = jx_sorted(list(RUNTIME_ONLY_FIELDS))
    if not jx_equal(jx_sorted(list(runtime)), declared):
        findings.append("C2V7-BANNER: the run-only banner fields are " +
...
REQUIRED_RESIDUAL_IDS = ("RES-C2V7-01", "RES-C2V7-02", "RES-C2V7-03", "RES-C2V7-04",
                         "RES-C2V7-05", "RES-C2V7-06", "RES-C2V7-07", "RES-C2V7-08",
                         "RES-C2V7-09", "RES-C2V7-10", "RES-C2V7-11")
REQUIRED_LAYER_IDS = ("L0", "L1", "L2", "L2b", "L2c", "L3", "L4", "L5", "L6", "L6b",
                      "L6c", "L7", "L8", "L9")
DISCHARGED_FINDING_IDS = ("IR-C2V4-01", "IR-C2V5-01", "IR-C2V5-02", "IR-C2V5-03",
                          "IR-C2V5-04", "IR-C2V6-01")


def _identity_findings(c) -> list:
    findings = []
    if jx_bind_text(c.get("artifact"), ARTIFACT_ID) is not None:
        findings.append("C2V7: candidate is not " + ARTIFACT_ID)
    if not jx_exact_int(c.get("version"), 7) or not jx_exact_int(c.get("supersedes"), 6):
        findings.append("C2V7: candidate must declare version 7 superseding 6 as JSON "
    ...
    if "CANDIDATE-NOT-APPLIED" not in status or "AWAITING-INDEPENDENT-REVIEW" not in status:
        findings.append("C2V7: candidate status must remain CANDIDATE-NOT-APPLIED / "
    ...
    for finding_id in DISCHARGED_FINDING_IDS:
        if not jx_has(resolves, finding_id):
            findings.append("C2V7: the candidate does not name " + finding_id +
...
def _inventory_findings(c) -> list:
    ...
    for layer in layers:
        ...
        if jx_type(spots) != "array" or not spots:
            findings.append("C2V7-INVENTORY: layer " + repr(layer.get("id")) +
    ...
    if not jx_equal(jx_sorted(jx_unique(seen)), jx_sorted(list(REQUIRED_LAYER_IDS))):
        findings.append("C2V7-INVENTORY: the guard inventory declares layers " +
    ...
    if not jx_int_in_range(len(behind), 80, 10 ** 6):
        findings.append("C2V7-INVENTORY: the inventory does not state what stands behind "
    ...
    if "nothing" not in terminal.lower():
        findings.append("C2V7-INVENTORY: the inventory does not state plainly that the "
...
def _residual_findings(c) -> list:
    ...
    if missing:
        findings.append("C2V7-RESIDUAL: the candidate does not retain " + repr(missing) +
    ...
    for item in residuals:
        ...
        if not jx_in(status, ["RETAINED", "RETAINED-OPEN"]):
            findings.append("C2V7-RESIDUAL: " + repr(item.get("id")) + " is not RETAINED")
        why = item.get("whyNotClosed")
        why = why if jx_type(why) == "string" else ""
        if not jx_int_in_range(len(why), 40, 10 ** 6):
            findings.append("C2V7-RESIDUAL: " + repr(item.get("id")) + " does not say "
    ...
    if jx_type(accounting) != "object":
        findings.append("C2V7-RESIDUAL: the candidate publishes no residualAccounting "
...
def _recording_findings(c) -> list:
    ...
    if missing:
        findings.append("C2V7-RECORD: " + str(len(missing)) + " pinned input(s) are "
    ...
    for item in recorded:
        ...
        if jx_has(PINS, name):
            ...
            if reason is not None:
                findings.append("C2V7-RECORD: " + str(name) + ": " + reason +
        ...
        if not jx_int_in_range(len(role), 12, 10 ** 6):
            findings.append("C2V7-RECORD: " + repr(name) + " is recorded without a role, "
...
def _mode_findings(c) -> list:
    ...
    if jx_bind_text(mode.get("checker"), "check-c2-v7.py") is not None or \
            not jx_equal(jx_sorted(list(codes)), jx_sorted(list(DECLARED_EXIT_CODES))) or \
            "REFUSED" not in refusal:
        findings.append("C2V7: the checker mode contract does not declare exactly the "
...
def _repro_findings(c, authority) -> list:
    ...
    if jx_bind_text(repro.get("sourceModification"), "NONE") is not None:
        findings.append("C2V7-REPRO: the minimal reproduction is not recorded as "
    ...
    if jx_bind_text(repro.get("predecessorSha256"), jx_get(PINS, V4_CHECKER)) is not None:
        findings.append("C2V7-REPRO: the reproduction is recorded against " +
    ...
    for index, site in enumerate(sites):
        ...
        if not jx_int(value):
            findings.append(
                "C2V7-TYPE: theDefect/repairedComparisonSites/" + str(index) +
    ...
    if not jx_subset(adjudicated, lines):
        findings.append("C2V7-REPRO: the candidate enumerates comparison sites " +
    ...
    if not jx_equal(jx_sorted(jx_unique(declared)), jx_sorted(jx_unique(expected))):
        findings.append("C2V7-REPRO: the candidate records successor false-accept "
...
def _parse_repro_findings(c, authority) -> list:
    ...
    if jx_bind_text(defect.get("findingId"), "IR-C2V6-01") is not None or \
        ...
        findings.append("C2V7-PARSE-REPRO: the candidate does not record IR-C2V6-01 as "
                        "the BLOCKING finding this successor exists to discharge")
    repro = defect.get("minimalReproduction")
    repro = repro if jx_type(repro) == "object" else {}
    if jx_bind_text(repro.get("sourceModification"), "NONE") is not None:
        findings.append("C2V7-PARSE-REPRO: the minimal reproduction is not recorded as "
                        "requiring no source modification, which is the whole reason "
    ...
    if jx_bind_text(repro.get("predecessorSha256"), jx_get(PINS, V6_CHECKER)) is not None:
        findings.append("C2V7-PARSE-REPRO: the reproduction is recorded against " +
                        repr(repro.get("predecessorSha256")) + ", not the verified "
    ...
    if jx_bind_text(perrun, "MECHANISM") is not None:
        findings.append("C2V7-PARSE-REPRO: the candidate must say plainly WHICH HALF of "
    ...
    if not jx_equal(jx_sorted(jx_unique(declared)), jx_sorted(jx_unique(expected))):
        findings.append("C2V7-PARSE-REPRO: the candidate records parse vectors " +
...
def differential_findings(authority) -> list:
    ...
    findings = ["C2V7-DIFFERENTIAL: " + item for item in result["escapes"]]
    if jx_bind(result["successorRejectedByName"], result["vectors"]) is not None:
        findings.append("C2V7-DIFFERENTIAL: " + str(result["successorRejectedByName"]) +
    ...
    if not jx_int_in_range(result["predecessorAdmittedThePosition"], 1, 10 ** 6):
        findings.append("C2V7-DIFFERENTIAL: the pinned predecessor named every vector "
    ...
    if not jx_int_in_range(result["predecessorFullyGreenRuns"], 1, 10 ** 6):
        findings.append("C2V7-DIFFERENTIAL: no retained vector reproduces the "
    ...
    findings.extend("C2V7-SUCCESSOR: " + item for item in successor["escapes"])
    if jx_bind(successor["v7RejectedByName"], successor["vectors"]) is not None:
        findings.append("C2V7-SUCCESSOR: " + str(successor["v7RejectedByName"]) + " of " +
    ...
    if not jx_int_in_range(successor["v5AdmittedToAFullGreenRun"], 1, 10 ** 6):
        findings.append("C2V7-SUCCESSOR: the pinned check-c2-v5.py admits none of its "
...
_EMPTY_PRIMITIVE = {
    ...
    "callShapedRowsDeclared": 0, "escapes": [],
}
...
def _check(c, authority, self_tree=None) -> list:
    ...
    if jx_bind_text(derived.get("artifact"), V4_CONTRACT) is not None or \
        ...
        return findings + [
            "C2V7-DERIVATION: the candidate must derive from " + V4_CONTRACT + " at " +
    ...
    findings.extend("C2V7-INHERITED: " + str(item) for item in inherited)
    ...
    for name in authority.portability["names"]:
        findings.append("C2V7-PRIMITIVE: the primitive block references " + name +
    ...
    if not jx_int_in_range(probe["freeNames"], 1, 10 ** 6):
        findings.append(
            "C2V7-PRIMITIVE: the portability detector reports a block that plainly "
...
def _execute_tree(tree):
    module = types.ModuleType("opensip_c2v7_mutated")
...
GATE_MUTATIONS = tuple(
    (name, function, body, expected,
     function if expected == "C2V7-PRIMITIVE" and not subject else subject)
...
LAYER_MUTATIONS = (
    ...
    ("disable the behavioural layer", "behavioural_layer",
     ...
     "C2V7-BEHAVIOUR", "executed no case", False),
    ("confine the document lock to the census block", "locked_integer_leaves",
     "return []", "C2V7-DOCLOCK", "outside the lock", False),
    ("lock only the base direction, so a derived leaf escapes",
     ...
     "C2V7-DOCLOCK", "outside the lock", False),
    ("probe only the first leaf of the document lock", "document_type_probe",
     ...
     "C2V7-DOCLOCK", "probed no position", False),
    ("stop binding the candidate's own integer leaves", "candidate_bindings",
     "return [], []", "C2V7-UNBOUND", "no layer of this run binds", False),
    ("stop probing the candidate's own integer leaves", "candidate_type_probe",
     ...
     "C2V7-CANDLOCK", "probed no position", False),
    ("let the candidate binding table be narrowed silently", "_effective_expected",
     "return None, 'unmeasured'", "C2V7-UNBOUND", "the derivation writes a JSON integer",
     False),
    ("hollow out the whole-document sweep", "predecessor_sweep",
     ...
     "'elapsedSeconds': 0}", "C2V7-SWEEP", "integer leaves injected", True),
    ...
    ("stop executing the primitive's own exhaustive test", "jx_selftest",
     ...
     "'escapes': []}", "C2V7-PRIMITIVE", "empty set", False),
    ("collapse the differential against the pinned v5 REJECT", "_v5_admissions",
     "return []", "C2V7-SUCCESSOR", "collapsed", True),
    ("stop measuring the census comparison sites", "census_site_positions",
     "return {}", "C2V7-REGISTER", "partition", False),
    # v7's new layers.  Every one of them is broken here and every break must be
    # caught by the layer it breaks, after every counter has been republished.
    ("stop reporting the byte/parse divergences at all", "parse_problem_findings",
     "return []", "C2V7-PARSE-PROBE", "duplicate at the root", False),
    ("stop looking for unhooked JSON parses in this file's own tree",
     "json_load_sites", "return []", "C2V7-PARSE-SCAN", "detects nothing", False),
    ("hollow out the parse probe", "parse_probe",
     "return {'executedCases': 0, 'namedAtThePosition': 0, 'controls': 0, "
     "'admissions': 0, 'escapes': []}", "C2V7-PARSE-PROBE", "probed no case", False),
    ("collapse the differential against the REJECTED v6's own parse",
     ...
     "C2V7-PARSE-DIFFERENTIAL", "unable to distinguish", False),
    ("stop enumerating the candidate's JSON NUMBER leaves (OBS-C2V6-01's gap)",
     "_number_leaf_steps", "return []", "C2V7-CANDNUM", "ADMITTED", False),
    ("hollow out the candidate float-leaf probe", "candidate_number_probe",
     "return {'executedCases': 0, 'namedRejections': 0, 'admissions': 0, "
     "'escapes': []}", "C2V7-CANDNUM", "probed no position", False),
...
def _m_unregister_a_census_position(c):
    for op in c["derivedFrom"]["operations"]:
        if jx_equal(op.get("path"), "planIntent.integerConstantRegisterV7"):
...
def _m_widen_the_projection(c):
    c["v4InheritanceProjection"]["fields"]["planIntent.integerConstantRegisterV7"] = None
...
def _m_absorb_a_residual(c):
    c["residuals"] = [item for item in c["residuals"]
                      if not jx_equal(item.get("id"), "RES-C2V7-03")]
...
def _m_understate_a_v7_counter(c):
    c["v7MeasuredCounters"]["behaviouralExecutedCases"] = 1


def _m_float_a_v7_counter(c):
    counters = c["v7MeasuredCounters"]
    counters["registeredCensusPositions"] = float(counters["registeredCensusPositions"])


def _m_publish_an_unmeasured_counter(c):
    c["v7MeasuredCounters"]["counterNobodyMeasures"] = 7
...
def _s_inject_computed_operand_comparison(tree):
    ...
    subject.body.extend(ast.parse(
        "def _c2v7_computed_probe(row, measured):\n"
...
def _s_inject_comprehension_subset(tree):
    ...
    subject = copy.deepcopy(tree)
    subject.body.extend(ast.parse(
        "def _c2v7_subset_probe(sites, authority):\n"
...
def _s_inject_operator_call(tree):
    ...
    subject = copy.deepcopy(tree)
    subject.body.extend(ast.parse(
        "def _c2v7_operator_probe(row, measured):\n"
...
def _s_inject_dict_key_lookup(tree):
    ...
    subject = copy.deepcopy(tree)
    subject.body.extend(ast.parse(
        "def _c2v7_keylookup_probe(row, table):\n"
...
def _s_inject_named_constant_comparison(tree):
    ...
    subject.body.extend(ast.parse(
        "_LAW18_PROBE = 1\n"
        "def _c2v7_named_constant_probe(row):\n"
...
def _s_inject_membership_comparison(tree):
    ...
    subject = copy.deepcopy(tree)
    subject.body.extend(ast.parse(
        "def _c2v7_membership_probe(row):\n"
...
SCAN_MUTATIONS = (
    ("inject a wire value compared against a COMPUTED integer",
     _s_inject_computed_operand_comparison, "C2V7-SCAN", "_c2v7_computed_probe"),
    ("inject IR-C2V5-01's comprehension-result subset test verbatim",
     _s_inject_comprehension_subset, "C2V7-SCAN", "_c2v7_subset_probe"),
    ("inject an operator.ne call, which is never an ast.Compare",
     _s_inject_operator_call, "C2V7-SCAN", "_c2v7_operator_probe"),
    ("inject a dict key lookup on a wire key", _s_inject_dict_key_lookup,
     "C2V7-SCAN", "_c2v7_keylookup_probe"),
    ("inject a set() dedup that collapses 1, 1.0 and True",
     _s_inject_dedup_cardinality, "C2V7-SCAN", "_c2v7_dedup_probe"),
    ("inject a wire value compared against a module-level named integer",
     _s_inject_named_constant_comparison, "C2V7-SCAN", "_c2v7_named_constant_probe"),
    ("inject membership in a numeric literal container",
     _s_inject_membership_comparison, "C2V7-SCAN", "_c2v7_membership_probe"),
    ("inject a max() over a wire sequence", _s_inject_sorted_launder,
     "C2V7-SCAN", "_c2v7_sorted_probe"),
)
...
def parse_mutation_findings(text, authority):
    ...
    try:
        ...
        return ["C2V7-PARSE: the mutant did not parse (" + type(exc).__name__ + ": " +
                str(exc) + ")"]
...
def pinned_v6_over_a_duplicate_key(authority):
    ...
    try:
        with redirect_stdout(sink):
            v6 = _execute_snapshot("opensip_c2v7_pinned_v6_checker", V6_CHECKER,
    ...
    result = {"vector": vector, "document": document, "position": position,
              ...
              "v7Findings": parse_mutation_findings(mutant, authority)}
...
RETAINED_ROWS = (len(PARSED_ROOT_CASES) + len(CONTRACT_MUTATIONS) +
                 ...
                 1 + len(DIFFERENTIAL_VECTORS) + len(SUCCESSOR_DIFFERENTIAL_VECTORS))
...
def selftest(candidate, authority, path):
    ...
    print("C-2 v7 mutation self-test over " + path.name + " - each row must be REJECTED "
    ...
    for name, root in PARSED_ROOT_CASES:
        ...
        named = hit_for(findings, "C2V7-TOTALITY-ROOT", "non-empty JSON object")
        if not named:
            escaped.append("parsed-JSON root " + name + ": no named C2V7-TOTALITY-ROOT "
                           "finding")
    ...
    for name, mutate, expected, subject in SCAN_MUTATIONS:
        try:
            ...
            findings = ["C2V7-SCAN: " + site["function"] + " line " +
        ...
        report(bool(hit), name, hit[0][:180] if hit else
               "NO C2V7-SCAN finding naming " + subject)
    ...
    v7_hit = hit_for(external["v7Findings"], "C2V7-DUPKEY", external["position"])
    ok = external["v6FullyGreen"] and bool(v7_hit)
    ...
    report(ok, "IR-C2V6-01 against the PINNED check-c2-v6.py [" + V6_CHECKER + "]",
           ...
           (v7_hit[0][:140] if v7_hit else "NO NAMED FINDING"))
    ...
    for row in clean_successor["rows"]:
        ok = row["v7NamedThePosition"] and row["v5FullyGreen"]
        if not ok:
        ...
        report(ok, "retained v5 blocking vector " + row["vector"] + " at " + row["path"],
               ...
               "of its own document: " + str(row["v7NamedThePosition"]))
...
def main(argv):
    try:
        ...
        print("C2V7-UNSUPPORTED-INVOCATION: " + str(exc), file=sys.stderr)
        return 2
    try:
        authority = load_authority()
    except AuthorityLoadError as exc:
        print("C2V7-PINNED-INPUT-REFUSED: " + type(exc).__name__ + ": " + str(exc),
              file=sys.stderr)
        return 2
    ...
    try:
        ...
        print("cannot load C-2 v7 candidate " + str(path) + ": " + type(exc).__name__ +
