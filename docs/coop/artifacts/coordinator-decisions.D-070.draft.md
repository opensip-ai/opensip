...
> **Date:** 2026-08-14
> **Protocol:** D-000 new cycle, turn 1 of 3. New cycle after CONTESTED D-067. Not a fourth turn.
...
Predecessor: CONTESTED D-067 turn 3
`coordinator-decisions.D-067.turn3.draft.md`
`dc1fd93184f93e510997a2489f5d2e1004da04ad119d6697514a0ccc2733810f`.
Claude 2 C2-D067-T3-SF1 and Codex ADV-D067-T3-01 accepted:
this file's title is D-070, a new cycle, not turn 2 or 3 of D-067.
Merits of the file-08 edit are unchanged from those turn-3 bytes
except the title/protocol and the COORD pin below.
...
| ADV-D067-T2-01 | SHOULD-FIX | ACCEPTED. Snapshot heading becomes 2026-08-14. |
| ADV-D067-T2-02 | SHOULD-FIX | ACCEPTED. Qualifying set vs explicitly disposed subset. |

Measured inputs:

| Path | sha256 |
|---|---|
| COORD | `71fbaaf0e2438584bc4cb006875c9f1df2f55e23b502cd79a992b19b3b6cde3e` |
...
2. **Amend the snapshot preamble** so condition-1 accounting is a
   ...
   Condition 1's qualifying set is a separate union: (a) rows whose
   ...
   undisposed one. The two components are counted separately
   in the snapshot: one SATISFIED and six explicitly disposed,
   with set-union deduplication if a future row has both."
3. **Advance the snapshot heading** from
   `### Current position — measured snapshot, 2026-08-13` to
   `### Current position — measured snapshot, 2026-08-14`.
   Do not rewrite the rest of the snapshot outside this act
   except the condition-1 row and the six cell notes.
...
5. **Append the exact notes below** to the six status cells. Use
...
C-D070 plus restore of the prior file-08 cells and prior snapshot
preamble.
