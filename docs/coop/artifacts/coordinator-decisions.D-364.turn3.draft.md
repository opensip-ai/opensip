...
> **Date:** 2026-09-01
> **Protocol:** D-000, turn 3 of 3, the last turn. Turn 1 drew OBJECT from both
> independent reviewers; turn 2 drew CONSENT 0/0 from Codex, which returned no
> finding identifiers, and OBJECT from Claude 2 at 0 MUST-FIX and 1 SHOULD-FIX.
> This turn lands every turn-1 identifier — **CODEX-D364-MF1**,
> **CODEX-D364-SF1**, **CLAUDE-D364-MF1**, **CLAUDE-D364-SF1**,
> **CLAUDE-D364-SF2**, **CLAUDE-D364-SF3**, **CLAUDE-D364-ADV-1**,
> **CLAUDE-D364-ADV-2**, **CLAUDE-D364-ADV-3** — and every turn-2 identifier:
> **CLAUDE-D364-T2-SF1**, **CLAUDE-D364-T2-ADV-1**, **CLAUDE-D364-T2-ADV-2**,
> **CLAUDE-D364-T2-ADV-3**, **CLAUDE-D364-T2-ADV-4**. The turn-1 subject
> `coordinator-decisions.D-364.draft.md`
> `5da3c7d3b1923f71f9c94b672003105c830fb7c764d7c562152bd59a01db54e2` and the
> turn-2 subject `coordinator-decisions.D-364.turn2.draft.md`
> `a56a67cb0b72088d886dd59770732a2fc6f2ef9aaaf615c818f0fdd872766a66` remain
...
3. **A D-294 Decision 2(b) successor of a citing artifact is not a D-056 gate-2
   ...
   actor-joins, missing design, still-UNDECIDED numbers. D-294 Decision 3 states the
   refresh duty of a successor "issued for any reason": it "refreshes its
   cross-lineage citations to the versions current at its dispatch and labels the
   superseded ones as not current", and "No frozen artifact is edited to achieve
   this." For a successor owed on ground (b) alone — the class this entry names —
   that refresh is its whole content. Such a successor
   supplies no acceptance-evidence member, no design content, no fixture, no
   schema and no number; it keeps the citing artifact's own currency sentences
   accurate. The same pinned D-056 subject fixes that set expressly: its
   Decision 1 bullet 3 reads file 08's SATISFIED legend, for eligible rows only,
   "as: the independently reviewed design contract or the already-recorded D-000
   decision, plus the named C4/DR-G* remainder list", and adds that "The
   execution of that remainder is not part of the architecture evidence set." A
   citation-refresh successor is in neither half of that set. It is therefore
   outside the set gate 2 quantifies over. Measured on the accepted contract's
   own bytes, such a refresh leaves the fourteen classes' invariant, input and
   pass content, the seven dispositions and p1p2g3Mapping unchanged, and moves
   only the cross-lineage currency and partition sentences. This is a
   classification of an artifact class, not an exception to gate 2, and D-056
   stands unamended. D-315 item 4 (G3-HOSTILE) recognises a mechanical join
   remasurement that "adds no semantic choice" as proceeding under D-293's
   existing delegation; that is a delegation ruling about who may author such a
   remasurement, not an eligibility classification, and it is noted here as
   showing the register already recognises the no-semantic-choice category. It is
   not a second ground and the classification does not rest on it.
...
each of the four lineages — plus file 08, the accepted contract, the D-056
turn-2 subject and this draft are the fixed measurement set. The two
intermediate remasurements pinned above, g29-leftover-join.v6 (D-320) and
g30-leftover-join.v8 (D-321), are historical inputs to the Ground column and are
outside that fixed set; the D-294 Decision 2 (b) test itself runs cited version
to current version. If a cited
