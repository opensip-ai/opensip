#!/usr/bin/env python3
"""Exact architecture conformance checker for retention-tiers.v22.json.
...
STARTUP_REFUSAL = (
    "RT22-CHECKER-UNSUPPORTED-INVOCATION: use python3 -I -B "
...
BINDING = "retention-tiers.v22.json"
RT22_STATUS = "CANDIDATE-NOT-APPLIED/AWAITING-INDEPENDENT-REVIEW"
...
RT21 = "retention-tiers.v21.json"
RT21_CHECKER = "check-retention-custody-v21.py"
RT21_ADJUDICATION = "retention-tiers.v21.adjudication-v20-review-response.json"
RT21_REVIEW = "retention-tiers.v21.review-independent-prefreeze.json"
ARCHITECTURE_PLAN = "ARCHITECTURE-TO-IMPLEMENTATION-PLAN.md"

PINS: dict[str, str] = {
    RT21: "ae2a7ba120650be61b77a3fbfb5f8e3cf74404d4d6421bd95c146480ba4de245",
    RT21_CHECKER: "a5d374a64e2ecfca0e469817ca67d21be799cc077e984d067c6bf4e9a66bb095",
    RT21_ADJUDICATION: "abbf6aa2457697a4ecc483786234d60131e1d251067514de34589b027567f074",
    RT21_REVIEW: "76fb4d21f801e5ceaaa71fe68a678aee98adfd7764704eee2faae86cb94ce097",
...
# Exact frozen route-A returned-value vocabulary.  RT22 returns no general
# Python object and no callable.  The reflective RT20 graph denominator below
# is retained only as historical predecessor evidence.
INERT_KIND_REGISTRY: tuple[dict[str, str], ...] = (
    {"id": "mapping", "pythonType": "types.MappingProxyType"},
    {"id": "tuple", "pythonType": "tuple"},
    {"id": "null", "pythonType": "NoneType"},
    {"id": "bool", "pythonType": "bool-exact"},
    {"id": "int", "pythonType": "int-exact-bounded"},
    {"id": "str", "pythonType": "str-exact-utf8"},
    {"id": "bytes", "pythonType": "bytes-exact"},
)
INERT_KIND_IDS = tuple(row["id"] for row in INERT_KIND_REGISTRY)
INERT_SNAPSHOT_FIELDS = (
    "schemaVersion", "kind", "snapshotId", "candidate", "rt13",
    "ep8DerivationReceipt", "fixtureProjection", "hostApiBinding",
)
INERT_HOST_BINDING_FIELDS = (
    "schemaVersion", "checkerSourceSha256", "publicApi",
)
INERT_PUBLIC_BINDING_FIELDS = (
    "publicName", "implementationName", "implementationAstSha256",
)
INERT_PUBLIC_API_BINDINGS = (
    ("derive_operational_custody_projection_v1",
     "derive_operational_projection"),
    ("validate_raw_object_resolution_v1",
     "validate_raw_object_resolution"),
    ("reduce_expiry_v1", "reduce_semantic_lease_v3"),
    ("reduce_release_v1", "reduce_semantic_lease_v3"),
    ("reduce_crash_reclaim_v3", "reduce_semantic_lease_v3"),
    ("reduce_resolve_and_pin_v2", "reduce_semantic_lease_v3"),
    ("is_retention_local_unavailable_v1",
     "is_retention_local_unavailable_v1"),
)
INERT_PUBLIC_SIGNATURES = MappingProxyType({
    "derive_operational_custody_projection_v1": (
        "snapshot", "evaluation_proof_bundle", "semantic_capability_closure"),
    "validate_raw_object_resolution_v1": (
        "snapshot", "raw_bytes", "request"),
    "reduce_expiry_v1": ("snapshot", "state", "event"),
    "reduce_release_v1": ("snapshot", "state", "event"),
    "reduce_crash_reclaim_v3": ("snapshot", "state", "event"),
    "reduce_resolve_and_pin_v2": ("snapshot", "state", "event"),
    "is_retention_local_unavailable_v1": ("snapshot", "output"),
})
INERT_INT_MIN = -(2 ** 63)
INERT_INT_MAX = 2 ** 64 - 1
INERT_MAX_NODES = 500_000
INERT_MAX_DEPTH = 256
INERT_MAX_BYTES = 64 * 1024 * 1024
...
class FrozenAuthority:
    ...
    def freeze(self) -> None:
        ...
        for name in (RT21, RT21_ADJUDICATION, RT21_REVIEW,
    ...
    def _validate_reviews(self) -> None:
        ...
        rt21_review = self.parsed[RT21_REVIEW]
        ...
        verdict21 = rt21_review.get("verdict", {})
        finding_ids21 = [row.get("id") for row in rt21_review.get(
            "blockingFindings", []) if isinstance(row, dict)]
        if rt21_review.get("decision") != "REJECT" or \
                verdict21.get("decision") != "REJECT" or \
                verdict21.get("blockingFindingCount") != 3 or \
                finding_ids21 != [
                    "RT21-PF-01-FROZEN-ROUTE-A-ROOT-AND-HOST-BINDING-NOT-IMPLEMENTED",
                    "RT21-PF-02-INERT-GRAMMAR-CONTROLS-INCOMPLETE",
                    "RT21-PF-03-ACTIVE-OPERATIONS-READ-REMAINS",
                ]:
            raise AuthorityError("RT21 exact three-blocker review binding drift")
        subject_rows21 = rt21_review.get(
            "exactByteReviewBasis", {}).get("subjects", [])
        subjects21: dict[str, str] = {}
        for row in subject_rows21:
            if not isinstance(row, dict) or \
                    row.get("sha256AtStart") != row.get("sha256AtEnd"):
                raise AuthorityError("RT21 review subject window drift")
            subjects21[row.get("path")] = row.get("sha256AtStart")
        if subjects21 != {
                RT21: PINS[RT21], RT21_CHECKER: PINS[RT21_CHECKER],
                RT21_ADJUDICATION: PINS[RT21_ADJUDICATION]}:
            raise AuthorityError("RT21 review exact subjects drift")
        if rt21_review.get("exactByteReviewBasis", {}).get(
                "inputHashDrift") is not False or \
                rt21_review.get("exactByteReviewBasis", {}).get(
                "inputIdentityDrift") is not False:
            raise AuthorityError("RT21 review input window drift")
        window21 = rt21_review.get("exactByteReviewBasis", {}).get(
            "transitiveWindow", {})
        retained21 = rt21_review.get("retainedGateResults", {})
        lineage21 = retained21.get("predecessorAndPinLineage", {})
        raw21 = retained21.get("rawFixturesLeasesPurgeAndRoots", {})
        if window21.get("inputCount") != 55 or \
                window21.get("unchangedCommonPins") != 51 or \
                window21.get("individualPinCorruptionsRejectedBeforeCallback") != "55/55" or \
                lineage21.get("pinCorruptions") != \
                "55/55 refused before validation callback" or \
                lineage21.get("physicalAlias") != "REFUSED" or \
                lineage21.get("result") != "PASS" or \
                retained21.get("ep8", {}).get("result") != "PASS-RETAINED" or \
                retained21.get("d9", {}).get("result") != "PASS-RETAINED" or \
                raw21.get("result") != "PASS-RETAINED":
            raise AuthorityError("RT21 retained narrow-pass binding drift")
...
def expected_candidate(authority: FrozenAuthority) -> dict[str, Any]:
    candidate = copy.deepcopy(authority.parsed[RT21])
    candidate["version"] = 22
    candidate["status"] = RT22_STATUS
    candidate["supersedesAsArchitectureCandidate"] = RT21
    candidate["authority"] = {
        "candidateState": "NOT-APPLIED",
        "authorityClaim": "NONE",
        "semanticSourceRule": "The exact RT21 semantic object is inherited only for its independently retained narrow passing evidence: exact 55-pin authority checks, EP8 API/receipt commitments, real D9 v1.10 dataclass semantics, semantic data/reducers, unchanged 7/2 and 5/3 raw-byte fixtures, nine leases, 13/7 root delta, and executed D9 consumers. RT22 repairs all three independently identified RT21 blockers: the actual returned root is the frozen V3 inert shape, it commits an exact source/AST-derived seven-row host API binding, its total authenticator enforces alias/range/byte/identity/registry controls, and active successor source contains no operations consumer.",
    ...
    ownership["ownsExactly"] = [
        row.replace("VerifiedSemanticRTSnapshotV2", "VerifiedSemanticRTSnapshotV3")
        for row in ownership["ownsExactly"]
    ]
    ownership["rootDelta"] = {
        "predecessor": RT21, "predecessorRootCount": 20,
        ...
        "equation": "13 independently reviewed unchanged roots + 7 metadata/verifier-only roots = 20; status, semanticLeaseProtocolV3, and every semantic/data/fixture root are exact RT21 bytes as parsed data",
    }

    retained_invariants = [
        copy.deepcopy(row) for row in authority.parsed[RT21]["invariants"]
        if not any(row.startswith(prefix) for prefix in (
            "RT21-35:", "RT21-36:", "RT21-37:", "RT21-38:",
            "RT21-39:"))
    ]
    candidate["invariants"] = retained_invariants + [
        "RT22-40: the retained RT21 55-input window is replaced lineage-for-lineage with exact RT21 artifact/checker/adjudication/three-blocker review while remaining exactly 55 unique alias-collapsed, read-once, hash-bound, end-stat-checked inputs; all 55 single-pin corruptions and physical aliases refuse before callback",
        f"RT22-41: VerifiedSemanticRTSnapshotV3 is exactly a deep-immutable value graph over the ordered {len(INERT_KIND_REGISTRY)}-kind registry mapping/tuple/null/bool/int/str/bytes; mappings and nonempty tuples are alias-sensitive, exact integers are bounded to signed-64 through unsigned-64, and strings plus bytes share an exact 64 MiB production budget",
        "RT22-42: the actual returned V3 snapshot contains no callable and no operation-name tuple; hostApiBinding commits the captured checker source SHA-256 and seven ordered public/implementation/implementation-AST rows, while the seven exact public wrappers remain outside the returned value",
        f"RT22-43: every one of the {len(INERT_KIND_REGISTRY)} inert-kind declarations, every one of the {len(INERT_SNAPSHOT_FIELDS)} root fields, all host-binding rows, exact forbidden identities, scalar boundaries, composite aliases, malformed controls, and node/depth/byte budgets have independent named refusal probes",
        "RT22-44: the snapshot3 commitment covers every returned field except itself under the frozen V3 framing; public wrappers authenticate the exact V3 root before exactly one implementation call and return the implementation result unchanged; the active successor checker has zero operations attribute/subscript/get/getattr consumers",
    ]
    candidate["retainedResiduals"] = [
        "RT21 is independently REJECTED for exactly three blockers: absent frozen V3 host binding, incomplete inert alias/range/byte/identity/totality controls, and one active operations read. RT22 repairs those exact surfaces while inheriting RT21 narrow passes for 55-pin corruption/alias refusal, EP8 nine-API commitments, real D9 v1.10 dataclass, 7/2 and 5/3 byte fixtures, nine leases, D9 6/16/10, purge/custody roots, and the 13/7 root delta without semantic change. RT22 itself remains unreviewed and NOT-APPLIED.",
        ...
        "Any downstream consumer must pin exact independently reviewed RT22 artifact/checker bytes and the exact committed data-only receipt/fixture projection, keep reducers outside the returned snapshot, and receive its own fresh integration review.",
    ...
    api["constructor"]["signature"] = \
        "validate_semantic_rt_snapshot_v7(FrozenDependencySnapshotV8, VerifiedD9SnapshotV1, CapturedCheckerSourceV1) -> VerifiedSemanticRTSnapshotV3"
    ...
    api["returnedSnapshot"] = {
        "type": "VerifiedSemanticRTSnapshotV3",
        "retainsExactly": [
            "schemaVersion=3 and kind=VERIFIED_SEMANTIC_RT_SNAPSHOT_V3",
            "snapshotId committing every returned field except itself under opensip.verified-semantic-rt-snapshot.v3 framing",
            "deep-immutable verified RT22 candidate data projection",
            ...
            "closed deep-immutable hostApiBinding containing the captured checker source SHA-256 and seven ordered public-name/implementation-name/implementation-AST-SHA-256 rows",
        ],
        "mustNotRetain": [
        ...
        "objectGraphRule": "The returned value is constructed only by a node/depth/byte-bounded copier over the exact seven-kind inert registry mapping/tuple/null/bool/int/str/bytes. Its complete semantic-child denominator is mapping value and tuple item; keys must be exact UTF-8 strings. Mappings and nonempty tuples are alias-sensitive; exact integers are bounded to [-2^63, 2^64-1]; exact forbidden identities, malformed registries/maps/budgets, cycles, unknown values, non-string keys, budget exhaustion, and commitment drift refuse with named findings.",
        "hostOperationRule": "The seven exact pure call signatures are source-compiled authenticated host functions outside the returned graph. hostApiBinding commits their exact public names, implementation names, implementation AST digests, and captured checker-source digest. Every wrapper authenticates VerifiedSemanticRTSnapshotV3 before one exact implementation call and returns its result unchanged.",
    ...
    raw_contract["pureFunctionSignature"] = raw_contract[
        "pureFunctionSignature"].replace(
            "VerifiedSemanticRTSnapshotV2", "VerifiedSemanticRTSnapshotV3")
    ...
    api["predecessorRejectionDisposition"] = {
        "reviewArtifact": RT21_REVIEW,
        "reviewSha256": PINS[RT21_REVIEW],
        "decision": "REJECT", "blockingFindingCount": 3,
        "repairedExactly": [
            "RT21-PF-01-FROZEN-ROUTE-A-ROOT-AND-HOST-BINDING-NOT-IMPLEMENTED",
            "RT21-PF-02-INERT-GRAMMAR-CONTROLS-INCOMPLETE",
            "RT21-PF-03-ACTIVE-OPERATIONS-READ-REMAINS",
    ...
    api["authenticatedLoaderBindingV8"] = {
        ...
        "inertBoundary": {
            ...
            "kindRegistry": [copy.deepcopy(row)
                             for row in INERT_KIND_REGISTRY],
            "snapshotOrderedFields": list(INERT_SNAPSHOT_FIELDS),
            "hostBindingOrderedFields": list(INERT_HOST_BINDING_FIELDS),
            "hostBindingRowOrderedFields": list(INERT_PUBLIC_BINDING_FIELDS),
            "publicApiBindings": [
                {"publicName": public, "implementationName": implementation}
                for public, implementation in INERT_PUBLIC_API_BINDINGS
            ],
            "constructorRule": "Copy only exact dict/list/tuple/MappingProxyType/null/bool/int/str/bytes inputs into fresh MappingProxyType/tuple/null/bool/int/str/bytes values. Each composite path is copied independently, so caller-held mutable backing and repeated input aliases do not survive. Dict keys must be exact strings; integers and total string/byte payload are bounded. No custom protocol, iterator, attribute, descriptor, reducer, or callable is consulted.",
            "validatorRule": "Validate exact-type mapping/tuple/null/bool/int/str/bytes dispatch, retain strong object identities for the complete walk, refuse a second path to any mapping or nonempty tuple, and accept only canonical empty-tuple reuse. Unknown kinds, exact forbidden identities, out-of-range integers, non-UTF-8 strings, cycles, non-string keys, registry drift, malformed controls, and node/depth/byte exhaustion produce named findings without exception escape.",
            "canonicalCommitment": {
                "domain": "opensip.verified-semantic-rt-snapshot.v3",
                "grammar": "frozen N/T/F and I/S/B/E/K/V/M type tags plus u64be framing; mapping and tuple order preserved; snapshotId excluded from its own preimage",
                "covers": "schemaVersion, kind, candidate, rt13, ep8DerivationReceipt, fixtureProjection, and the complete hostApiBinding",
            },
            "deletionMutationDenominator": {
                "kindRows": len(INERT_KIND_REGISTRY),
                "snapshotFields": len(INERT_SNAPSHOT_FIELDS),
                "hostApiRows": len(INERT_PUBLIC_API_BINDINGS),
                "expected": "all deletions and all source/AST/implementation mutations refuse",
            },
            ...
            "budgets": {"maxNodes": INERT_MAX_NODES,
                        ...
                        "maxBytes": INERT_MAX_BYTES,
                        "integerMinimum": INERT_INT_MIN,
                        "integerMaximum": INERT_INT_MAX},
            "namedRefusalCodes": [
                "RT22-INERT-REGISTRY-MALFORMED",
                "RT22-INERT-REGISTRY-MISSING",
                "RT22-INERT-REGISTRY-UNKNOWN",
                "RT22-INERT-REGISTRY-ORDER-OR-DECLARATION",
                "RT22-INERT-FORBIDDEN-KIND",
                "RT22-INERT-FORBIDDEN-IDENTITY",
                "RT22-INERT-COMPOSITE-ALIAS",
                "RT22-INERT-INT-RANGE",
                "RT22-INERT-STRING-UTF8",
                "RT22-INERT-BYTE-BUDGET",
                "RT22-INERT-MAPPING-KEY",
                "RT22-INERT-CYCLE",
                "RT22-INERT-NODE-BUDGET",
                "RT22-INERT-DEPTH-BUDGET",
                "RT22-SNAPSHOT-CLOSED-SHAPE",
                "RT22-SNAPSHOT-COMMITMENT",
            ],
        },
...
def _semantic_findings(candidate: dict[str, Any], expected: dict[str, Any],
    ...
    findings: list[str] = []
    rt13 = authority.parsed[RT13]
    predecessor = authority.parsed[RT21]
    ...
    for key in unchanged:
        difference = exact_recursive_equal(
            candidate.get(key), predecessor[key], f"$.{key}")
        if difference:
            findings.append(
                f"RT22-RT21-NARROW-PASS-PRESERVATION: {difference}")
    ...
    if candidate.get("status") != RT22_STATUS:
        findings.append("RT22-STATUS: candidate/review boundary drifted")
...
def _inert_u64(value: int) -> bytes:
    return value.to_bytes(8, "big")


def _inert_blob(tag: bytes, raw: bytes) -> bytes:
    return tag + _inert_u64(len(raw)) + raw


def _inert_kind(value: Any) -> str | None:
...
def _encode_inert(value: Any) -> bytes:
    """Frozen one-to-one framing for an already authenticated inert value."""
    kind = _inert_kind(value)
    if kind == "null":
        return b"N"
    if kind == "bool":
        return b"T" if value else b"F"
    if kind == "int":
        return _inert_blob(b"I", str(value).encode("ascii"))
    if kind == "str":
        return _inert_blob(b"S", value.encode("utf-8"))
    if kind == "bytes":
        return _inert_blob(b"B", value)
    if kind == "tuple":
        encoded = b"".join(
            _inert_blob(b"E", _encode_inert(item)) for item in value)
        return b"T" + _inert_u64(len(value)) + encoded
    if kind == "mapping":
        rows: list[bytes] = []
        for key, item in value.items():
            rows.append(_inert_blob(b"K", key.encode("utf-8")) +
                        _inert_blob(b"V", _encode_inert(item)))
        return b"M" + _inert_u64(len(rows)) + b"".join(rows)
    raise AuthorityError(
        f"RT22-INERT-FORBIDDEN-KIND:{type(value).__name__}")


def _freeze_inert(value: Any, *, max_nodes: int = INERT_MAX_NODES,
                  max_depth: int = INERT_MAX_DEPTH,
                  max_bytes: int = INERT_MAX_BYTES) -> Any:
    """Copy exact input into V3 inert data and sever every composite alias."""
    if type(max_nodes) is not int or max_nodes < 1 or \
            type(max_depth) is not int or max_depth < 1 or \
            type(max_bytes) is not int or max_bytes < 0:
        raise AuthorityError("RT22-INERT-BUDGET-MALFORMED")
    active: set[int] = set()
    nodes = 0
    bytes_seen = 0

    def visit(item: Any, depth: int) -> Any:
        nonlocal nodes, bytes_seen
        nodes += 1
        if nodes > max_nodes:
            raise AuthorityError("RT22-INERT-NODE-BUDGET")
        if depth > max_depth:
            raise AuthorityError("RT22-INERT-DEPTH-BUDGET")
        if item is None or type(item) is bool:
            return item
        if type(item) is int:
            if not INERT_INT_MIN <= item <= INERT_INT_MAX:
                raise AuthorityError("RT22-INERT-INT-RANGE")
            return item
        if type(item) is str:
            try:
                raw = item.encode("utf-8")
            except UnicodeEncodeError as exc:
                raise AuthorityError("RT22-INERT-STRING-UTF8") from exc
            bytes_seen += len(raw)
            if bytes_seen > max_bytes:
                raise AuthorityError("RT22-INERT-BYTE-BUDGET")
            return item
        if type(item) is bytes:
            ...
            if bytes_seen > max_bytes:
                raise AuthorityError("RT22-INERT-BYTE-BUDGET")
            return bytes(item)
        if type(item) not in (dict, list, tuple, MappingProxyType):
            raise AuthorityError(
                f"RT22-INERT-FORBIDDEN-KIND:{type(item).__module__}."
                f"{type(item).__qualname__}")
        identity = id(item)
        if identity in active:
            raise AuthorityError("RT22-INERT-CYCLE")
        active.add(identity)
        try:
            ...
            for key, child in item.items():
                if type(key) is not str:
                    raise AuthorityError("RT22-INERT-MAPPING-KEY")
                if key in result:
                    raise AuthorityError("RT22-INERT-MAPPING-DUPLICATE")
...
def _thaw_inert(value: Any, depth: int = 0) -> Any:
    if depth > INERT_MAX_DEPTH:
        raise AuthorityError("RT22-INERT-DEPTH-BUDGET")
    if value is None or type(value) in (bool, int, str, bytes):
        return value
    if type(value) is tuple:
        return [_thaw_inert(item, depth + 1) for item in value]
    if type(value) is MappingProxyType:
        return {key: _thaw_inert(item, depth + 1)
                for key, item in value.items()}
    raise AuthorityError(
        f"RT22-INERT-FORBIDDEN-KIND:{type(value).__name__}")


def _inert_registry_findings(registry: Any) -> list[str]:
    try:
        rows = tuple(registry)
    except BaseException as exc:
        return [f"RT22-INERT-REGISTRY-MALFORMED:{type(exc).__name__}"]
    ids: list[str] = []
    for index, row in enumerate(rows):
        if type(row) is not dict or set(row) != {"id", "pythonType"} or \
                type(row.get("id")) is not str or \
                type(row.get("pythonType")) is not str:
            return [f"RT22-INERT-REGISTRY-MALFORMED:index={index}"]
        ids.append(row["id"])
    if len(ids) != len(set(ids)):
        return ["RT22-INERT-REGISTRY-DUPLICATE"]
    missing = [name for name in INERT_KIND_IDS if name not in ids]
    extra = [name for name in ids if name not in INERT_KIND_IDS]
    if missing:
        return [f"RT22-INERT-REGISTRY-MISSING:kind={name}"
                for name in missing]
    if extra:
        return [f"RT22-INERT-REGISTRY-UNKNOWN:kind={name}"
                for name in extra]
    if tuple(ids) != INERT_KIND_IDS or rows != INERT_KIND_REGISTRY:
        return ["RT22-INERT-REGISTRY-ORDER-OR-DECLARATION"]
    return []


def _inert_snapshot_without_id(
        snapshot: Mapping[str, Any]) -> MappingProxyType:
    return MappingProxyType({key: value for key, value in snapshot.items()
                             if key != "snapshotId"})


def _inert_snapshot_id(snapshot: Mapping[str, Any]) -> str:
    preimage = (b"opensip.verified-semantic-rt-snapshot.v3\x00" +
                _encode_inert(_inert_snapshot_without_id(snapshot)))
    return "snapshot3:sha256:" + sha256(preimage)


def _inert_snapshot_schema_findings(snapshot: Any) -> list[str]:
    if type(snapshot) is not MappingProxyType:
        return ["RT22-SNAPSHOT-ROOT:exact mappingproxy required"]
    if tuple(snapshot) != INERT_SNAPSHOT_FIELDS:
        return ["RT22-SNAPSHOT-FIELDS:exact ordered fields required"]
    findings: list[str] = []
    if type(snapshot.get("schemaVersion")) is not int or \
            snapshot.get("schemaVersion") != 3:
        findings.append("RT22-SNAPSHOT-VERSION")
    if snapshot.get("kind") != "VERIFIED_SEMANTIC_RT_SNAPSHOT_V3":
        findings.append("RT22-SNAPSHOT-KIND")
    snapshot_id = snapshot.get("snapshotId")
    if type(snapshot_id) is not str or not re.fullmatch(
            r"snapshot3:sha256:[0-9a-f]{64}", snapshot_id):
        findings.append("RT22-SNAPSHOT-ID-SHAPE")
    else:
        try:
            expected = _inert_snapshot_id(snapshot)
        except BaseException as exc:
            findings.append(
                f"RT22-SNAPSHOT-ID-DERIVATION:{type(exc).__name__}")
        else:
            if snapshot_id != expected:
                findings.append("RT22-SNAPSHOT-COMMITMENT")
    for name in ("candidate", "rt13", "ep8DerivationReceipt",
                 "fixtureProjection", "hostApiBinding"):
        if type(snapshot.get(name)) is not MappingProxyType:
            findings.append(f"RT22-SNAPSHOT-FIELD-TYPE:{name}")
    binding = snapshot.get("hostApiBinding")
    if type(binding) is MappingProxyType:
        if tuple(binding) != INERT_HOST_BINDING_FIELDS:
            findings.append("RT22-HOST-BINDING-FIELDS")
        elif binding.get("schemaVersion") != 1 or \
                type(binding.get("checkerSourceSha256")) is not str or \
                not re.fullmatch(r"[0-9a-f]{64}",
                                 binding["checkerSourceSha256"]):
            findings.append("RT22-HOST-BINDING-HEADER")
        rows = binding.get("publicApi")
        if type(rows) is not tuple or \
                len(rows) != len(INERT_PUBLIC_API_BINDINGS):
            findings.append("RT22-HOST-BINDING-API-COUNT")
        else:
            for index, (row, expected) in enumerate(zip(
                    rows, INERT_PUBLIC_API_BINDINGS)):
                if type(row) is not MappingProxyType or \
                        tuple(row) != INERT_PUBLIC_BINDING_FIELDS or \
                        row.get("publicName") != expected[0] or \
                        row.get("implementationName") != expected[1] or \
                        type(row.get("implementationAstSha256")) is not str or \
                        not re.fullmatch(r"[0-9a-f]{64}", row[
                            "implementationAstSha256"]):
                    findings.append(
                        f"RT22-HOST-BINDING-API-ROW:index={index}")
    return findings


def _scan_inert_snapshot_v3(
        root: Any, forbidden: Any = None, *,
        registry: Any = INERT_KIND_REGISTRY,
        max_nodes: Any = INERT_MAX_NODES,
        max_depth: Any = INERT_MAX_DEPTH,
        max_bytes: Any = INERT_MAX_BYTES,
        require_snapshot_schema: bool = True) -> dict[str, Any]:
    registry_findings = _inert_registry_findings(registry)
    if registry_findings:
        return {"findings": registry_findings, "forbiddenHits": [],
                "visited": 0, "clean": False,
                "identityCustodyMode": "STRONG-REFERENCE"}
    if type(max_nodes) is not int or max_nodes < 1 or \
            type(max_depth) is not int or max_depth < 1 or \
            type(max_bytes) is not int or max_bytes < 0:
        return {"findings": ["RT22-INERT-BUDGET-MALFORMED"],
                "forbiddenHits": [], "visited": 0, "clean": False,
                "identityCustodyMode": "STRONG-REFERENCE"}
    if forbidden is None:
        forbidden = {}
    if type(forbidden) not in (dict, MappingProxyType):
        return {"findings": ["RT22-INERT-FORBIDDEN-MAP-MALFORMED"],
                "forbiddenHits": [], "visited": 0, "clean": False,
                "identityCustodyMode": "STRONG-REFERENCE"}
    ...
    def visit(value: Any, path: str, depth: int) -> None:
        ...
        identity = id(value)
        if identity in active:
            findings.append(f"RT22-INERT-CYCLE:path={path}")
            return
        kind = _inert_kind(value)
        composite = kind in ("mapping", "tuple")
        alias_sensitive = kind == "mapping" or \
            (kind == "tuple" and len(value) > 0)
        if alias_sensitive and identity in finished:
            findings.append(f"RT22-INERT-COMPOSITE-ALIAS:path={path}")
            return
        retained[identity] = value
        if len(retained) > max_nodes:
            findings.append(f"RT22-INERT-NODE-BUDGET:path={path}")
            return
        try:
            label = forbidden.get(identity)
        except BaseException as exc:
            findings.append(
                f"RT22-INERT-FORBIDDEN-MAP-ERROR:{type(exc).__name__}")
            return
        if label is not None:
            if type(label) is not str:
                findings.append("RT22-INERT-FORBIDDEN-LABEL")
            else:
                hits.append({"path": path, "label": label})
            return
        if kind is None:
            findings.append(
                f"RT22-INERT-FORBIDDEN-KIND:path={path}:"
                f"type={type(value).__module__}.{type(value).__qualname__}")
            return
        if kind == "int" and not INERT_INT_MIN <= value <= INERT_INT_MAX:
            findings.append(f"RT22-INERT-INT-RANGE:path={path}")
        elif kind == "str":
            try:
                raw = value.encode("utf-8")
            except UnicodeEncodeError:
                findings.append(f"RT22-INERT-STRING-UTF8:path={path}")
            else:
                bytes_seen += len(raw)
        elif kind == "bytes":
            bytes_seen += len(value)
        if bytes_seen > max_bytes:
            findings.append(f"RT22-INERT-BYTE-BUDGET:path={path}")
            return
        if not composite:
            return
        if depth >= max_depth:
            findings.append(f"RT22-INERT-DEPTH-BUDGET:path={path}")
            return
        active.add(identity)
        try:
            if kind == "tuple":
                ...
                try:
                    ...
                    findings.append(
                        f"RT22-INERT-MAPPING-ERROR:path={path}:"
                        f"{type(exc).__name__}")
                    return
                for index, pair in enumerate(rows):
                    try:
                        ...
                        findings.append(
                            f"RT22-INERT-MAPPING-PAIR:path={path}:"
                            f"index={index}:{type(exc).__name__}")
                        continue
                    if type(key) is not str:
                        findings.append(
                            f"RT22-INERT-MAPPING-KEY:path={path}:index={index}")
                        continue
    ...
    visit(root, "$", 0)
    if require_snapshot_schema:
        findings.extend(_inert_snapshot_schema_findings(root))
    return {"findings": findings, "forbiddenHits": hits,
            "visited": len(retained), "clean": not findings and not hits,
            "identityCustodyMode": "STRONG-REFERENCE",
            "bytesVisited": bytes_seen}


def authenticate_inert_snapshot_v3(
        root: Any, *, forbidden: Any = None,
        registry: Any = INERT_KIND_REGISTRY,
        max_nodes: Any = INERT_MAX_NODES,
        max_depth: Any = INERT_MAX_DEPTH,
        max_bytes: Any = INERT_MAX_BYTES) -> MappingProxyType:
    """Total reviewer-facing adapter for the exact V3 authenticator."""
    try:
        scan = _scan_inert_snapshot_v3(
            root, forbidden, registry=registry, max_nodes=max_nodes,
            max_depth=max_depth, max_bytes=max_bytes)
        codes = tuple(scan["findings"] + [
            f"RT22-INERT-FORBIDDEN-IDENTITY:path={row['path']}:"
            f"label={row['label']}" for row in scan["forbiddenHits"]])
        return MappingProxyType({
            "accepted": scan["clean"],
            "findingCodes": codes,
            "escaped": False,
        ...
        return MappingProxyType({
            "accepted": False,
            "findingCodes": (f"RT22-INERT-TOTALITY:{type(exc).__name__}",),
            "escaped": False,
        })


def _require_inert_snapshot_v3(snapshot: Any) -> None:
    result = authenticate_inert_snapshot_v3(snapshot)
    if not result["accepted"]:
        raise AuthorityError(result["findingCodes"][0])


def _function_ast_sha256(source: bytes, name: str) -> str:
    ...
    if len(matches) != 1:
        raise AuthorityError(
            f"RT22-HOST-IMPLEMENTATION-DEFINITION:{name}:{len(matches)}")
    normalized = ast.dump(matches[0], annotate_fields=True,
                          include_attributes=False).encode("utf-8")
    return sha256(normalized)


def _build_host_api_binding(checker_source: bytes) -> MappingProxyType:
    if type(checker_source) is not bytes:
        raise AuthorityError("RT22-HOST-CHECKER-SOURCE-TYPE")
    rows = [{
        "publicName": public_name,
        "implementationName": implementation_name,
        "implementationAstSha256": _function_ast_sha256(
            checker_source, implementation_name),
    } for public_name, implementation_name in INERT_PUBLIC_API_BINDINGS]
    return _freeze_inert({
        "schemaVersion": 1,
        "checkerSourceSha256": sha256(checker_source),
        "publicApi": rows,
    })


def VerifiedSemanticRTSnapshotV3(
        candidate: dict[str, Any], authority: FrozenAuthority,
        checker_source: bytes) -> MappingProxyType:
    """Return the actual frozen V3 inert root; no post-return translation."""
    ...
    provisional = _freeze_inert({
        "schemaVersion": 3,
        "kind": "VERIFIED_SEMANTIC_RT_SNAPSHOT_V3",
        "snapshotId": "snapshot3:sha256:" + "0" * 64,
        "candidate": candidate,
        "rt13": authority.parsed[RT13],
        "ep8DerivationReceipt": authority.ep8_receipt_data,
        "fixtureProjection": authority.fixture_projection_data,
        "hostApiBinding": _build_host_api_binding(checker_source),
    })
    snapshot = _freeze_inert({
        **dict(provisional),
        "snapshotId": _inert_snapshot_id(provisional),
    })
    _require_inert_snapshot_v3(snapshot)
    return snapshot


def derive_operational_custody_projection_v1(
        snapshot: Any, evaluation_proof_bundle: Mapping[str, Any],
        semantic_capability_closure: dict[str, Any]) -> tuple[
            dict[str, Any], bytes, str, str]:
    _require_inert_snapshot_v3(snapshot)
    if not isinstance(evaluation_proof_bundle, Mapping):
        raise AuthorityError("RT22-HOST-BUNDLE-SHAPE")
    try:
        bundle_ref = evaluation_proof_bundle["proofBundleCasRef"]
        closure_ref = evaluation_proof_bundle["semanticClosureCasRef"]
    except (KeyError, TypeError) as exc:
        raise AuthorityError("RT22-HOST-BUNDLE-REFS") from exc
    return derive_operational_projection(
        semantic_capability_closure, bundle_ref, closure_ref)
...
def validate_raw_object_resolution_v1(
    ...
    _require_inert_snapshot_v3(snapshot)
    return validate_raw_object_resolution(raw_bytes, request)
...
def _host_reduce(snapshot: Any, state: dict[str, Any],
    ...
    if type(event) is not dict or event.get("kind") != expected_kind:
        raise AuthorityError(f"RT22-HOST-LEASE-EVENT-KIND:{expected_kind}")
    return reduce_semantic_lease_v3(state, event)
...
def host_is_retention_local_unavailable_v1(
    ...
    _require_inert_snapshot_v3(snapshot)
    return is_retention_local_unavailable_v1(output)
...
def _snapshot_quarantine_findings(
        snapshot: MappingProxyType, authority: FrozenAuthority,
        checker_source: bytes,
) -> list[str]:
    findings: list[str] = []
    authenticated = authenticate_inert_snapshot_v3(snapshot)
    if not authenticated["accepted"]:
        return list(authenticated["findingCodes"])
    ...
    try:
        goldens = snapshot["candidate"]["verifiedSemanticRtApiContract"][
            "authenticatedLoaderBindingV8"]["ep8Closure"]["resultGoldens"]
    ...
    try:
        expected_binding = _build_host_api_binding(checker_source)
    except (AuthorityError, SyntaxError, UnicodeError) as exc:
        findings.append(
            f"RT22-HOST-BINDING-RECOMPUTE:{type(exc).__name__}")
    else:
        if snapshot["hostApiBinding"] != expected_binding:
            findings.append("RT22-HOST-BINDING: source/AST binding drifted")
    expected_public_names = tuple(
        public for public, _implementation in INERT_PUBLIC_API_BINDINGS)
    if tuple(HOST_OPERATION_REGISTRY) != expected_public_names:
        findings.append("RT22-PUBLIC-API: exact ordered names drifted")
    for public_name, parameters in INERT_PUBLIC_SIGNATURES.items():
        function = HOST_OPERATION_REGISTRY.get(public_name)
        if type(function) is not types.FunctionType or \
                function.__globals__ is not globals() or \
                function.__closure__ is not None or \
                tuple(inspect.signature(function).parameters) != parameters:
            findings.append(f"RT22-PUBLIC-API-SIGNATURE:{public_name}")
    ...
    forbidden_scan = _scan_inert_snapshot_v3(
        snapshot, {identity: "frozen-authority-or-host-function"
                   for identity in forbidden_identities})
    if forbidden_scan["forbiddenHits"]:
        findings.extend(
            f"RT22-INERT-FORBIDDEN-IDENTITY:{row['path']}"
            for row in forbidden_scan["forbiddenHits"])
    findings.extend(forbidden_scan["findings"])
    return findings


def check_contract(candidate: Any, authority: FrozenAuthority,
                   candidate_source: bytes | None = None,
                   checker_source: bytes | None = None) -> list[str]:
    ...
    try:
        ...
        if not findings:
            if type(checker_source) is not bytes:
                findings.append("RT22-CHECKER-SOURCE: captured bytes required")
            else:
                snapshot = VerifiedSemanticRTSnapshotV3(
                    candidate, authority, checker_source)
                findings.extend(_snapshot_quarantine_findings(
                    snapshot, authority, checker_source))
...
def _candidate_mutations() -> list[Mutation]:
    ...
    return [
        ...
        ("inert kind omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["inertBoundary"]["kindRegistry"].pop()),
        ("snapshot field omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["inertBoundary"]["snapshotOrderedFields"].pop()),
        ("host binding field omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["inertBoundary"]["hostBindingOrderedFields"].pop()),
        ("host API row omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["inertBoundary"]["publicApiBindings"].pop()),
        ("independent challenge count", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["inertBoundary"]["independentChallengeBinding"].__setitem__("runtimeKindsChallenged", 24)),
        ("inert commitment domain", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["inertBoundary"]["canonicalCommitment"].__setitem__("domain", "wrong")),
        ("EP8 required API omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["ep8Closure"]["requiredCheckerApi"].pop()),
        ("EP8 forbidden API extra", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["ep8Closure"]["requiredCheckerApi"].append("check")),
        ("EP8 call count", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["ep8Closure"]["exactCallCounts"].__setitem__("validate_bundle", 0)),
        ("EP8 receipt commitment", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["ep8Closure"]["resultGoldens"].__setitem__("receiptCommitment", "sha256:" + "0" * 64)),
        ("fixture projection commitment", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["ep8Closure"]["resultGoldens"].__setitem__("fixtureProjectionCommitment", "sha256:" + "0" * 64)),
        ("v1.10 real dataclass", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["temporaryVerifiedModuleExecutions"][0].__setitem__("isDataclass", False)),
        ("v1.10 temporary cleanup", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["temporaryVerifiedModuleExecutions"][0].__setitem__("restoredToAbsent", False)),
        ("v1.10 decorator interposition", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV8"]["d9Interpositions"].append({"source": "check-d9-v1.10.py", "kind": "dataclass-decorator"})),
...
def _inert_boundary_selftest(
        ...
        checker_source: bytes) -> tuple[int, int, list[str]]:
    """Exercise the complete V3 route-A boundary independently of JSON claims."""
    results: list[tuple[str, bool]] = []

    def record(name: str, passed: bool) -> None:
        results.append((name, passed))

    def accepted(root: Any, **kwargs: Any) -> bool:
        result = authenticate_inert_snapshot_v3(root, **kwargs)
        return bool(result["accepted"] and not result["escaped"])

    def refused(root: Any, **kwargs: Any) -> bool:
        result = authenticate_inert_snapshot_v3(root, **kwargs)
        return bool(not result["accepted"] and
                    not result["escaped"] and result["findingCodes"])

    if snapshot is None:
        return 0, 1, ["snapshot-construction"]
    record("base-exact-v3", accepted(snapshot))
    record("base-exact-mappingproxy", type(snapshot) is MappingProxyType)
    record("base-exact-fields", tuple(snapshot) == INERT_SNAPSHOT_FIELDS)
    record("host-binding-source", snapshot.get("hostApiBinding") ==
           _build_host_api_binding(checker_source))

    for index, row in enumerate(INERT_KIND_REGISTRY):
        registry = list(copy.deepcopy(INERT_KIND_REGISTRY))
        registry.pop(index)
        record(f"kind-delete-{row['id']}", refused(
            snapshot, registry=registry))
    for field in INERT_SNAPSHOT_FIELDS:
        changed = MappingProxyType({key: value for key, value in
                                    snapshot.items() if key != field})
        record(f"field-delete-{field}", refused(changed))
    record("field-unknown", refused(MappingProxyType({
        **dict(snapshot), "unknown": None})))
    record("field-reorder", refused(MappingProxyType(dict(
        reversed(tuple(snapshot.items()))))))
    record("v2-name-only-root", refused(MappingProxyType({
        "schemaVersion": 2,
        "kind": "VERIFIED_SEMANTIC_RT_SNAPSHOT_V2",
        "snapshotId": "snapshot2:sha256:" + "0" * 64,
        "candidate": snapshot["candidate"],
        "rt13": snapshot["rt13"],
        "ep8DerivationReceipt": snapshot["ep8DerivationReceipt"],
        "fixtureProjection": snapshot["fixtureProjection"],
        "hostOperationIds": tuple(
            public for public, _implementation in INERT_PUBLIC_API_BINDINGS),
    })))

    def recommit(**changes: Any) -> MappingProxyType:
        rows = dict(snapshot)
        rows.update(changes)
        rows["snapshotId"] = "snapshot3:sha256:" + "0" * 64
        provisional = MappingProxyType(rows)
        rows["snapshotId"] = _inert_snapshot_id(provisional)
        return MappingProxyType(rows)

    shared = MappingProxyType({"shared": True})
    record("composite-alias", refused(recommit(
        candidate=shared, rt13=shared)))
    for name, value in (
        ("integer-2pow80", 2 ** 80),
        ("integer-below-min", INERT_INT_MIN - 1),
        ("integer-above-max", INERT_INT_MAX + 1),
        ("surrogate-string", "\ud800"),
    ):
        try:
            changed = recommit(candidate=value)
        except BaseException:
            changed = MappingProxyType({
                key: (value if key == "candidate" else item)
                for key, item in snapshot.items()})
        record(name, refused(changed))
    record("bytes-64mib-plus-one", refused(recommit(
        candidate=b"x" * (INERT_MAX_BYTES + 1))))
    record("forbidden-identity", refused(
        snapshot, forbidden={id(snapshot["candidate"]): "candidate-root"}))

    cycle_backing: dict[str, Any] = {}
    cycle = MappingProxyType(cycle_backing)
    cycle_backing["self"] = cycle
    record("cycle", refused(MappingProxyType({
        key: (cycle if key == "candidate" else value)
        for key, value in snapshot.items()})))
    record("nonstring-key", refused(MappingProxyType({
        key: (MappingProxyType({1: "value"})
              if key == "candidate" else value)
        for key, value in snapshot.items()})))
    ...
    forbidden_values: list[Any] = [
        IntSubclass(1), StrSubclass("x"), BytesSubclass(b"x"),
        TupleSubclass((1,)), DictSubclass({"x": 1}),
        [], {}, set(), bytearray(b"x"), memoryview(b"x"),
        lambda: None, (lambda: None).__code__, sys._getframe(),
        types.ModuleType("rt22_forbidden"), object(), ValueError("x"),
        slice(1), range(1), 1.0, complex(1, 2), type,
    ]
    ...
    forbidden_values.extend((generator, coroutine, async_generator))
    try:
        for index, value in enumerate(forbidden_values):
            changed = MappingProxyType({
                key: (value if key == "candidate" else item)
                for key, item in snapshot.items()})
            record(f"forbidden-kind-{index:02d}-{type(value).__name__}",
                   refused(changed))
    finally:
        generator.close()
        coroutine.close()
    ...
    record("node-budget", refused(recommit(
        candidate=tuple(range(40))), max_nodes=12))
    deep: Any = None
    for _index in range(8):
        deep = (deep,)
    record("depth-budget", refused(
        recommit(candidate=deep), max_depth=4))
    record("byte-budget-override", refused(
        recommit(candidate=b"0123456789"), max_bytes=5))
    record("malformed-registry", refused(snapshot, registry=None))
    record("malformed-forbidden", refused(snapshot, forbidden=[]))
    record("malformed-node-budget", refused(snapshot, max_nodes=True))
    record("malformed-depth-budget", refused(snapshot, max_depth="x"))
    record("malformed-byte-budget", refused(snapshot, max_bytes=-1))

    mutable_shared = {"x": [1, 2]}
    frozen = _freeze_inert({"left": mutable_shared, "right": mutable_shared})
    mutable_shared["x"].append(3)
    record("builder-severs-caller-alias",
           frozen["left"] == frozen["right"] and
           frozen["left"] is not frozen["right"] and
           tuple(frozen["left"]["x"]) == (1, 2))

    candidate = _thaw_inert(snapshot["candidate"])
    fixtures = candidate["verifiedSemanticRtApiContract"][
        "rawObjectResolutionConformance"]["fixtureGoldens"]
    lease_rows = candidate["semanticLeaseProtocolV3"]["goldenScenarios"]
    lease_by_kind = {row["event"]["kind"]: row for row in lease_rows}
    vectors: dict[str, tuple[tuple[Any, ...], tuple[Any, ...]]] = {}
    fixture = fixtures[0]
    vectors["derive_operational_custody_projection_v1"] = (
        (fixture, fixture["semanticClosure"]),
        (fixture["semanticClosure"], fixture["proofBundleCasRef"],
         fixture["semanticClosureCasRef"]),
    )
    raw_row = fixture["resolutionRows"][0]
    raw = bytes.fromhex(raw_row["rawBytesHex"])
    vectors["validate_raw_object_resolution_v1"] = (
        (raw, raw_row["request"]), (raw, raw_row["request"]),
    )
    event_by_wrapper = {
        "reduce_expiry_v1": "expiry",
        "reduce_release_v1": "release",
        "reduce_crash_reclaim_v3": "crash-reclaim",
        "reduce_resolve_and_pin_v2": "resolve-and-pin",
    }
    for wrapper, kind in event_by_wrapper.items():
        row = lease_by_kind[kind]
        vectors[wrapper] = (
            (row["preState"], row["event"]),
            (row["preState"], row["event"]),
        )
    vectors["is_retention_local_unavailable_v1"] = (
        (lease_rows[0]["expectedOutput"],),
        (lease_rows[0]["expectedOutput"],),
    )
    token = object()
    for public_name, implementation_name in INERT_PUBLIC_API_BINDINGS:
        public = HOST_OPERATION_REGISTRY[public_name]
        public_args, expected_args = vectors[public_name]
        original = globals()[implementation_name]
        calls: list[tuple[Any, ...]] = []

        def marker(*args: Any) -> Any:
            calls.append(args)
            return token

        globals()[implementation_name] = marker
        try:
            try:
                output = public(snapshot, *public_args)
            except BaseException:
                output = None
            valid = output is token and calls == [expected_args]
            before = len(calls)
            invalid_refused = False
            try:
                public(None, *public_args)
            except BaseException:
                invalid_refused = len(calls) == before
            record(f"wrapper-binding-{public_name}", valid)
            record(f"wrapper-invalid-first-{public_name}", invalid_refused)
        finally:
            globals()[implementation_name] = original

    projection_ok = raw_ok = True
    raw_count = 0
    for fixture in fixtures:
        try:
            projection, encoded, ref, digest = \
                derive_operational_custody_projection_v1(
                    snapshot, fixture, fixture["semanticClosure"])
            projection_ok = projection_ok and (
                projection == fixture["operationalProjection"] and
                encoded.hex() == fixture["operationalProjectionCanonicalBytesHex"] and
            ...
            for row in fixture["resolutionRows"]:
                actual = validate_raw_object_resolution_v1(
                    snapshot, bytes.fromhex(row["rawBytesHex"]),
                    row["request"])
                raw_ok = raw_ok and actual == row["expectedResult"]
                raw_count += 1
        except (AuthorityError, KeyError, ResolutionError, TypeError,
                ValueError):
    ...
    reducer_names = {value: key for key, value in event_by_wrapper.items()}
    ...
    record("host-lease-goldens-9", lease_ok and len(lease_rows) == 9)
    record("host-predicate-goldens-9",
           predicate_ok and len(lease_rows) == 9)
...
def _trust_falsifiers(
        authority: FrozenAuthority, checker_source: bytes,
    ...
    try:
        snapshot = VerifiedSemanticRTSnapshotV3(
            expected_candidate(authority), authority, checker_source)
        graph_clean = not _snapshot_quarantine_findings(
            snapshot, authority, checker_source)
        frozen_rejected = False
        try:
    ...
    edge_passed, edge_total, edge_failures = _inert_boundary_selftest(
        snapshot, checker_source)
...
def selftest(candidate: dict[str, Any], candidate_source: bytes,
             authority: FrozenAuthority, checker_source: bytes) -> int:
    expected = expected_candidate(authority)
    if candidate_source != pretty(expected) or candidate_source != pretty(candidate):
        print("REFUSING to self-test: base is not exact canonical RT22 candidate")
        return 1
    base = check_contract(
        candidate, authority, candidate_source, checker_source)
    if base:
    ...
    for name, mutate in mutations:
        ...
        try:
            ...
            if after == before:
                ...
                rejected = bool(check_contract(
                    changed, authority, after, checker_source))
        except Exception:
            rejected = False
    ...
    for _name, value in hostile:
        try:
            source = pretty(value)
            rejected = bool(check_contract(
                value, authority, source, checker_source))
        except Exception:
            rejected = False
    ...
    (trust_passed, trust_total, trust_failures,
     edge_passed, edge_total, edge_failures) = _trust_falsifiers(
        authority, checker_source)
    ...
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
          f"{len(INERT_KIND_REGISTRY)}/{len(INERT_KIND_REGISTRY)} "
          "kind deletion mutations refuse before root")
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
          f"{len(INERT_SNAPSHOT_FIELDS)}/{len(INERT_SNAPSHOT_FIELDS)} "
          "snapshot-field deletion mutations refuse")
    print(f"  {'pass' if not edge_failures else 'FAIL':>6}  "
    ...
    failed = (
        ...
        or legacy_findings or d9_findings or trust_failures or edge_failures
        or not dirty_refused
    )
    print()
    if failed:
        print(
            "RT22 selftest failures: "
    ...
    print(
        ...
        f"passed, including {len(INERT_KIND_REGISTRY)}/"
        f"{len(INERT_KIND_REGISTRY)} kind and "
        f"{len(INERT_SNAPSHOT_FIELDS)}/{len(INERT_SNAPSHOT_FIELDS)} field deletions, "
...
def main(argv: list[str]) -> int:
    try:
        ...
        checker_before = FrozenAuthority._identity(checker_path)
        checker_source = checker_path.read_bytes()
        if FrozenAuthority._identity(checker_path) != checker_before:
            raise AuthorityError("RT22 checker changed during source capture")
        ...
        print(f"cannot freeze RT22 authority: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"cannot load RT22 candidate: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    if "--selftest" in argv[1:]:
        if not isinstance(candidate, dict):
            print("selftest requires an object root", file=sys.stderr)
            return 1
        return selftest(candidate, source, authority, checker_source)
    findings = check_contract(
        candidate, authority, source, checker_source)
    try:
        authority.end_stat_check()
    except AuthorityError as exc:
        findings.append(f"RT22-AUTHORITY-END: {exc}")
    if findings:
    ...
    raw = candidate["verifiedSemanticRtApiContract"][
        "rawObjectResolutionConformance"]["fixtureGoldens"]
    print(
        f"RT22 contract OK - {path.name}; {len(PINS)} frozen inputs read once; "
        ...
        f"closed {len(INERT_KIND_REGISTRY)}-kind V3 inert data-only return "
        "with source/AST-bound seven-wrapper host API; "
