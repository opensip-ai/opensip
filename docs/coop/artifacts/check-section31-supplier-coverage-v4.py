#!/usr/bin/env python3
"""Fail-closed checker for section31-supplier-coverage.v4.
...
TABLE = ROOT / "docs/coop/artifacts/section31-supplier-coverage.v4.json"
...
def main() -> int:
    ...
    for item in rows:
        ...
        if "supplier" not in item:
            failures.append(f"{item['id']}: BOUND item has no supplier")
            continue
