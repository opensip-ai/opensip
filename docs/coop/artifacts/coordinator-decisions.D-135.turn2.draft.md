...
D-135-SF-1 and D-135-SF-2. Codex OBJECT, 0 MUST-FIX, 2
SHOULD-FIX D135-SF-1 and D135-SF-2.

| ID | Sev | Disposition |
|---|---|---|
| D-135-SF-1 (Claude) | SHOULD-FIX | ACCEPTED. Decision 3 also replaces `63 rows` with `65 rows`. |
| D-135-SF-2 (Claude) | SHOULD-FIX | ACCEPTED. Source-pin cells cite `COORDINATOR-DECISIONS.md` with D-132 commit and D-134 CONSENT digests. |
| D135-SF-1 (Codex) | SHOULD-FIX | ACCEPTED. Snapshot heading date becomes 2026-08-15. |
| D135-SF-2 (Codex) | SHOULD-FIX | ACCEPTED. Overturn *removes* the two rows. Total only before dependents. |
