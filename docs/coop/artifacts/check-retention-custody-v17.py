#!/usr/bin/env python3
"""Exact architecture conformance checker for retention-tiers.v17.json.
...
STARTUP_REFUSAL = (
    "RT17-CHECKER-UNSUPPORTED-INVOCATION: use python3 -I -B "
    "check-retention-custody-v17.py"
)
...
BINDING = "retention-tiers.v17.json"
RT17_STATUS = "CANDIDATE-NOT-APPLIED/AWAITING-INDEPENDENT-REVIEW"
...
class FrozenAuthority:
    ...
    def freeze(self) -> None:
        ...
        for name in (RT16, RT16_ADJUDICATION, RT16_REVIEW,
...
def expected_candidate(authority: FrozenAuthority) -> dict[str, Any]:
    candidate = copy.deepcopy(authority.parsed[RT16])
    candidate["version"] = 17
    candidate["status"] = RT17_STATUS
    candidate["supersedesAsArchitectureCandidate"] = RT16
    candidate["authority"] = {
        "candidateState": "NOT-APPLIED",
        "authorityClaim": "NONE",
        "semanticSourceRule": "The exact RT16 semantic object is inherited only under its independent narrow PASS for semantic data/reducers, unchanged 7/2 and 5/3 raw-byte fixtures, the 47-input outer snapshot, the internal nine-call EP8 control, and executed D9 consumers; its three rejected return-surface/dataclass claims are replaced exactly by this successor.",
    ...
    ownership["rootDelta"] = {
        "predecessor": RT16, "predecessorRootCount": 20,
        ...
        "independentlyReviewedUnchangedRootCount": 11,
        "metadataOrVerifierOnlyChangedRootKeys": [
            "version", "status", "supersedesAsArchitectureCandidate",
            "authority", "semanticLeaseProtocolV3",
            "semanticOwnershipBoundary", "invariants", "retainedResiduals",
            "verifiedSemanticRtApiContract",
        ],
        "metadataOrVerifierOnlyChangedRootCount": 9,
        "addedRootKeys": [], "removedRootKeys": [], "finalRootCount": 20,
        "equation": "11 independently reviewed unchanged roots + 9 status/metadata/verifier-only roots = 20; the only semanticLeaseProtocolV3 delta is the returned snapshot nominal V1-to-V2 substitution, with all state/event/output/reducer/golden data exact",
    }

    candidate["invariants"] = copy.deepcopy(authority.parsed[RT16]["invariants"]) + [
        "RT17-17: the 47-input RT16 verifier window plus exact RT16 artifact/checker/adjudication/review are alias-collapsed into 51 unique read-once, hash-bound, end-stat-checked inputs before successor authority is used",
        "RT17-18: the exact byte-derived RT13 requiredCheckerApi is exactly nine unique EP8 callables with no extras; every callable executes internally over typed authenticated inputs and exact results/call counts are committed before every live EP8 callable/module/loader is excluded from the return",
        "RT17-19: VerifiedSemanticRTSnapshotV2 returns only immutable EP8 derivation/fixture data and its cycle-safe graph quarantine traverses attributes, slots, mappings, containers, bound owners, function globals/closures/defaults/annotations, partials, and module dictionaries against every frozen buffer, parsed root, executed module, and the live FrozenAuthority",
        "RT17-20: exact D9 v1.10 bytes execute with the real @dataclass(frozen=True) semantics under one collision-refusing digest-derived temporary sys.modules registration that is identity-checked and restored to absence",
    ]
    candidate["retainedResiduals"] = [
        "RT16 is independently REJECTED only for RT16-PF-01, RT16-PF-02, and RT16-PF-03. Its explicit narrow PASS for semantic data/reducers, unchanged byte-bearing fixtures, 47-input outer snapshot, internal nine-call EP8 control, and executed D9 consumers is inherited without semantic change; RT17 itself remains unreviewed and NOT-APPLIED.",
        ...
        "Any downstream successor must pin exact independently reviewed RT17 artifact/checker bytes and the exact data-only receipt/fixture projection, rebuild its enclosing values without returned EP8 executable authority, and receive its own fresh independent review.",
    ...
    api["constructor"]["signature"] = \
        "validate_semantic_rt_snapshot_v2(FrozenDependencySnapshotV3, VerifiedD9SnapshotV1) -> VerifiedSemanticRTSnapshotV2"
    ...
    api["returnedSnapshot"] = {
        ...
        "retainsExactly": [
            "snapshotId",
            "deep-immutable verified RT17 candidate data projection",
        ...
        "objectGraphRule": "Before return, a cycle-safe identity traversal covers attributes, __dict__, every slot, mapping keys/values, list/tuple/set/frozenset items, bound method owner/function, function globals/closures/defaults/kwdefaults/annotations/custom attributes, functools.partial state, and module dictionaries; any forbidden identity refuses.",
    ...
    api["verifierTcb"] = {
        ...
        "sourceExecutionOrder": "all 51 local data/checker/review/plan buffers hash and review-bind before any verified local source buffer executes",
    }
    api["predecessorRejectionDisposition"] = {
        "reviewArtifact": RT16_REVIEW,
        "reviewSha256": PINS[RT16_REVIEW],
        ...
        "preservedNarrowPass": [
            ...
            "47 unique read-once outer-snapshot pins",
    ...
    old_loader = api.pop("authenticatedLoaderBindingV2", None)
    if not isinstance(old_loader, dict):
        raise AuthorityError("RT16 authenticatedLoaderBindingV2 absent")
    ...
    api["authenticatedLoaderBindingV3"] = {
        "ep8Closure": {
        ...
        "graphQuarantine": {
            ...
            "edgeKinds": [
                "attributes and __dict__", "slots", "mapping keys and values",
                "list/tuple/set/frozenset items", "bound method owner and function",
                "function globals/closures/defaults/kwdefaults/annotations/custom attributes",
                "functools.partial function/args/keywords", "module dictionaries",
            ],
            "failureRule": "any reachable forbidden object identity refuses before return",
        },
...
def _semantic_findings(candidate: dict[str, Any], expected: dict[str, Any],
    ...
    findings: list[str] = []
    rt13 = authority.parsed[RT13]
    rt16 = authority.parsed[RT16]
    if list(candidate) != ROOT_ORDER:
        findings.append("RT17-ROOT-ORDER: exact 20-root order drifted")
    unchanged = [
        "artifact", "claimId", "dependencies",
        "semanticBasisProjection", "operationalCustodyProjectionContract",
        "storageAndLineage", "custodyPolicy",
    ...
    for key in unchanged:
        difference = exact_recursive_equal(candidate.get(key), rt16[key], f"$.{key}")
        if difference:
            findings.append(f"RT17-RT16-NARROW-PASS-PRESERVATION: {difference}")
    expected_lease = copy.deepcopy(rt16["semanticLeaseProtocolV3"])
    expected_lease["pureFunctionSignatures"] = [
        row.replace("VerifiedSemanticRTSnapshotV1", "VerifiedSemanticRTSnapshotV2")
        for row in expected_lease["pureFunctionSignatures"]
    ]
    difference = exact_recursive_equal(
        candidate.get("semanticLeaseProtocolV3"), expected_lease,
        "$.semanticLeaseProtocolV3")
    if difference:
        findings.append(f"RT17-RT16-LEASE-DATA-PRESERVATION: {difference}")
    ...
    if candidate.get("status") != RT17_STATUS:
        findings.append("RT17-STATUS: candidate/review boundary drifted")
...
def _owned_graph_children(value: Any, path: str) -> list[tuple[str, Any]]:
    rows: list[tuple[str, Any]] = []
    if isinstance(value, MappingABC):
        for index, (key, item) in enumerate(value.items()):
            rows.append((f"{path}.key[{index}]", key))
            rows.append((f"{path}[{key!r}]", item))
        return rows
    if isinstance(value, (list, tuple, set, frozenset)):
        return [(f"{path}[{index}]", item)
                for index, item in enumerate(value)]
    if isinstance(value, types.MethodType):
        return [(path + ".__self__", value.__self__),
                (path + ".__func__", value.__func__)]
    if isinstance(value, types.BuiltinMethodType):
        owner = getattr(value, "__self__", None)
        return [] if owner is None else [(path + ".__self__", owner)]
    if isinstance(value, functools.partial):
        return [(path + ".func", value.func), (path + ".args", value.args),
                (path + ".keywords", value.keywords)]
    if isinstance(value, types.FunctionType):
        rows.extend((f"{path}.__dict__[{name!r}]", item)
                    for name, item in vars(value).items())
        if value.__annotations__:
            rows.append((path + ".__annotations__", value.__annotations__))
        if value.__defaults__ is not None:
            rows.append((path + ".__defaults__", value.__defaults__))
        if value.__kwdefaults__ is not None:
            rows.append((path + ".__kwdefaults__", value.__kwdefaults__))
        if value.__closure__ is not None:
            for index, cell in enumerate(value.__closure__):
                try:
                    rows.append((f"{path}.__closure__[{index}]", cell.cell_contents))
                except ValueError:
                    pass
        rows.extend((f"{path}.__globals__[{name!r}]", item)
                    for name, item in value.__globals__.items())
        return rows
    if isinstance(value, types.ModuleType):
        return [(f"{path}.__dict__[{name!r}]", item)
                for name, item in vars(value).items()]
    ...
    for cls in type(value).__mro__:
        ...
        if isinstance(declared, str):
            declared = (declared,)
        slots.update(name for name in declared
                     if name not in ("__dict__", "__weakref__"))
    for name in sorted(slots):
        try:
            rows.append((f"{path}.{name}", object.__getattribute__(value, name)))
...
def _forbidden_identity_paths(
        root: Any, forbidden: Mapping[int, str], max_nodes: int = 500_000,
) -> tuple[list[dict[str, str]], int]:
    queue: deque[tuple[str, Any]] = deque([("$", root)])
    seen: set[int] = set()
    hits: list[dict[str, str]] = []
    atomic = (type(None), bool, int, float, complex, str, bytes, bytearray,
              memoryview, range, types.CodeType)
    while queue:
        ...
        if len(seen) > max_nodes:
            raise AuthorityError(
                f"snapshot graph exceeded {max_nodes} nodes at {path}")
        if isinstance(value, atomic):
            continue
        queue.extend(_owned_graph_children(value, path))
    return hits, len(seen)


def _quarantine_edge_selftest() -> tuple[int, int, list[str]]:
    sentinel = object()
    ...
    module = types.ModuleType("rt17_quarantine_sentinel")
    ...
    cases = {
        "dict": {"value": sentinel},
        "mappingproxy": MappingProxyType({"value": sentinel}),
        "list": [sentinel], "tuple": (sentinel,), "set": {sentinel},
        "frozenset": frozenset({sentinel}), "object-dict": dictionary_box,
        "object-slot": slot_box, "bound-method": method_box.expose,
        "function-globals": namespace["probe"],
        "function-closure": closure_function,
        "function-default": default_function,
        "function-kwdefault": kwdefault_function,
        "function-annotation": annotation_function,
        "module": module, "partial": functools.partial(id, sentinel),
    }
    failures = []
    for name, root in cases.items():
        hits, _count = _forbidden_identity_paths(
            root, {id(sentinel): "sentinel"})
        if not hits:
...
def _snapshot_quarantine_findings(
    ...
    hits, count = _forbidden_identity_paths(snapshot, forbidden)
    findings = [
        f"RT17-SNAPSHOT-GRAPH-QUARANTINE: {row['path']} reaches "
    ...
    if tuple(receipt) != (
            ...
            "fixtureProjectionCommitment", "results", "resultsCommitment",
            "receiptCommitment"):
        findings.append("RT17-EP8-RECEIPT: closed ordered fields drifted")
    if list(receipt.get("requiredCheckerApi", ())) != \
            list(EXPECTED_EP8_REQUIRED_API):
        findings.append("RT17-EP8-RECEIPT: exact nine-name authority drifted")
    expected_counts = {name: (2 if name == "encode_semantic_object_binding" else 1)
                       for name in EXPECTED_EP8_REQUIRED_API}
    if dict(receipt.get("exactCallCounts", {})) != expected_counts:
        findings.append("RT17-EP8-RECEIPT: exact invocation counts drifted")
    try:
        goldens = snapshot.candidate["verifiedSemanticRtApiContract"][
            "authenticatedLoaderBindingV3"]["ep8Closure"]["resultGoldens"]
        if receipt["resultsCommitment"] != goldens["resultsCommitment"] or \
                ...
                snapshot.fixture_projection["projectionCommitment"]:
            findings.append("RT17-EP8-RECEIPT: executable commitment join drifted")
    except (KeyError, TypeError):
        findings.append("RT17-EP8-RECEIPT: executable commitment join absent")
    if any(callable(value) for _path, value in _owned_graph_children(
            receipt, "$.ep8_derivation_receipt")):
        findings.append("RT17-EP8-RECEIPT: direct callable retained")
    if set(snapshot.operations) != {
        ...
        findings.append("RT17-SNAPSHOT-SURFACE: operation allowlist drifted")
    if count < 200:
        findings.append("RT17-SNAPSHOT-GRAPH-QUARANTINE: traversal was incomplete")
    return findings
...
def _candidate_mutations() -> list[Mutation]:
    ...
    return [
        ...
        ("graph edge omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["graphQuarantine"]["edgeKinds"].pop()),
        ("EP8 required API omission", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["ep8Closure"]["requiredCheckerApi"].pop()),
        ("EP8 forbidden API extra", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["ep8Closure"]["requiredCheckerApi"].append("check")),
        ("EP8 call count", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["ep8Closure"]["exactCallCounts"].__setitem__("validate_bundle", 0)),
        ("EP8 receipt commitment", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["ep8Closure"]["resultGoldens"].__setitem__("receiptCommitment", "sha256:" + "0" * 64)),
        ("fixture projection commitment", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["ep8Closure"]["resultGoldens"].__setitem__("fixtureProjectionCommitment", "sha256:" + "0" * 64)),
        ("v1.10 real dataclass", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["temporaryVerifiedModuleExecutions"][0].__setitem__("isDataclass", False)),
        ("v1.10 temporary cleanup", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["temporaryVerifiedModuleExecutions"][0].__setitem__("restoredToAbsent", False)),
        ("v1.10 decorator interposition", lambda root: root["verifiedSemanticRtApiContract"]["authenticatedLoaderBindingV3"]["d9Interpositions"].append({"source": "check-d9-v1.10.py", "kind": "dataclass-decorator"})),
...
def _trust_falsifiers(authority: FrozenAuthority) -> tuple[int, int, list[str]]:
    ...
    legacy_hits, _legacy_nodes = _forbidden_identity_paths(
        legacy_like, {id(authority): "live FrozenAuthority"})
    record("legacy EP8 function closure reaches authority", bool(legacy_hits))
    try:
    ...
    edge_passed, edge_total, edge_failures = _quarantine_edge_selftest()
    record("recursive quarantine edge denominator",
           edge_passed == edge_total == 16 and not edge_failures)
...
def selftest(candidate: dict[str, Any], candidate_source: bytes,
    ...
    if candidate_source != pretty(expected) or candidate_source != pretty(candidate):
        print("REFUSING to self-test: base is not exact canonical RT17 candidate")
        return 1
    ...
    trust_passed, trust_total, trust_failures = _trust_falsifiers(authority)
    ...
    print(
        ...
        "falsifiers rejected or contained"
    )
    return 0
...
def main(argv: list[str]) -> int:
    try:
        ...
        print(f"cannot freeze RT17 authority: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"cannot load RT17 candidate: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        findings.append(f"RT17-AUTHORITY-END: {exc}")
    if findings:
    ...
    raw = candidate["verifiedSemanticRtApiContract"][
        "rawObjectResolutionConformance"]["fixtureGoldens"]
    print(
        f"RT17 contract OK - {path.name}; {len(PINS)} frozen inputs read once; "
        ...
        "recursive graph-clean data-only EP8 return; real v1.10 frozen dataclass; "
