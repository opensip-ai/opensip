...
> Frozen turn-1 subject is not edited. Frozen D-099
> subjects are not edited. Not a fourth turn of D-099.
> Not a SATISFIED re-record.
...
Turn-1 subject `coordinator-decisions.D-101.draft.md`
`54333f8ec16398e0cd3d3bec15afb0b42c4e4a9740bfe7755e3326d083cd7864`
held frozen. Claude 2 OBJECT, 1 MUST-FIX D101-T1-MF-1.
Codex OBJECTIONS, 0 MUST-FIX, 1 SHOULD-FIX ADV-D101-T1-01.
D-099 remains CONTESTED at `90c80fc31b4c4a0dc376da62184d1fd6d82f21a4`.
The contract lives in this file's bytes.

| ID | Sev | Disposition |
|---|---|---|
| D101-T1-MF-1 | MUST-FIX | ACCEPTED. `sysctl.proc_cputype`+PID was measured: it returns the host CPU type (same for native arm64 and Rosetta x86_64). It is not the process-arch field. D099-T3-MF-1's sysctl-family alternative is rejected by that measurement. macOS 15.4 has no public per-process CPU-type API. Target ISA is the fail-closed derivation below. |
| ADV-D101-T1-01 | SHOULD-FIX | ACCEPTED. The unsupported "path swap is detectable because T is suspended" claim is removed. Path/inode/digest are checked before spawn, after observations, and immediately before each timed launch. |
| D101-T1-N-1 | NOTE | ACCEPTED. G04 help/version RSS may share the G03 launch. Peak RSS is `wait4`/`getrusage` after exit. Steady baseline RSS methodology is harness-authoring / DR-G04 evidence, not this naming act. |
| D101-T1-N-4 | NOTE | ACCEPTED. MacOSX15.4.sdk is the API-availability warrant, not the fleet-class definition. |
...
2. Record absolute path P, I0=`(st_dev, st_ino)` of P,
   ...
   The job must not mutate P concurrently. On macOS,
   re-check I and D after observations and immediately
   before each timed launch (item 10).
...
**Path identity (ADV-D101-T1-01).** P is a digest-addressed
...
equal I0 and D0. Immediately before **each** timed
launch of P, repeat that check. Any mismatch → reject
the job and rerun preflight. Suspension of T does **not**
by itself detect a path swap. This path-hash binding is
weaker than Linux's `/proc/T/exe` fd hash. Darwin has
no fd-to-running-image equivalent.
...
| D-101 turn-1 subject | `54333f8ec16398e0cd3d3bec15afb0b42c4e4a9740bfe7755e3326d083cd7864` |
| D-101 Claude 2 turn 1 | `903d4abeddcdee58f4bd0854b709b6432636e4e356b5823cc72d6d667a6a3c54` |
| D-101 Codex turn 1 | `4936f9f24e49d004ef1e6443131cc6bf717e22baefb1f8876fd0f3e63775edef` |
